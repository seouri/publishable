...
| At `freeze`, the run directory is missing `config.yaml`, `config.yaml` does not parse to a mapping, or `environment/repo_root.txt` is missing or empty — the run was started by a build before these artifacts existed, or the directory was edited by hand; it cannot be frozen | `E-FREEZE-NO-CONFIG` |
