...
| A [`sweep.baseline`](#expansion-modes) fixing a value on every `sweep.grid` axis leaves at least one condition differing from it on more than one of them, so that comparison is reported `confounded: true` and its delta mixes two effects. Checked at `validate` over the declared axes alone, and only for a baseline that fixes every one of them — a baseline leaving an axis free is a different shape rather than this fault, expanding to [one baseline per cell](#expansion-modes) of the free axes instead of one reference for the whole run, so the row's condition does not hold and nothing is reported. Silence there is not a verdict that such a design confounds nothing: what each comparison is taken against is [`vs_baseline`](#expansion-modes)'s question, not this row's | `W-SWEEP-BASELINE-CONFOUNDED` |
