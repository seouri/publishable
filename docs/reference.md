...
| A run record `read_run_record` is pointed at is absent, present but unparseable or without a `run_id`, or carries a `schema_version` this build doesn't read — read through [`io.reuse_from`](#lineage-between-runs)'s upstream `run.yaml`, and through [`diff`](#cli-reference)'s own run-record operand, which calls the identical reader | `ContractError` · `E-UPSTREAM-RECORD-MISSING`, `E-UPSTREAM-RECORD-UNREADABLE`, `E-UPSTREAM-RECORD-VERSION` |
