...
**A recorded column carries `resample_draws` too, once `statistics.resample` is declared, but with different provenance and only two of the three values above.** A column has the `t_over_units` fallback the paragraph above contrasts it with, so `resample_draws` is **absent** entirely — not `null` — when no `resample` is declared: nothing was attempted, and the key says so by not appearing, the same way the field is absent from a run predating H4a. Once declared, the value is **`null`** whenever `ci95` is (fewer than two units, or too few draws for the confidence level), and otherwise the **requested** `n` — never a lesser count. A column's draw statistic is a mean, or a weighted mean, over a non-empty sample that is finite throughout, so it is always defined once an interval exists at all: there is no per-draw failure for it to survive-count the way a derived metric's recompute can fail draw by draw, so the `0` bucket above ("attempted, every draw individually degenerate") is unreachable for a column. The field is genuinely two-valued for a column (`null` or the requested `n`) against the derived metric's three-valued scheme above, and that asymmetry is real rather than smoothed over — it is a fact about what a mean can fail at, not an inconsistency in how core reports it.
