...
A warning is a [diagnostic](#exit-codes-and-diagnostics), not an exception: nothing raises one and nothing catches one. It carries a stable `W-` identifier for the same reason an error carries an `E-` one — a message gets clearer over time, and something pinned to the wording breaks when it does. A warning never changes an exit code; it changes what the record says core was unsure of. Some fire at `validate` time, from the declaration alone; others fire at `run` time, from what the declaration met once units and executions were real — the table states each, not when it runs.
...
| [`replication.repeats`](#a-batch-says-when-not-what) declares a `batch` level but no step in the pipeline sets `nondeterministic = True` — under a fully deterministic pipeline a batch recomputes the same answer every time, so its dispersion is a row of zeros bought with *n*× the compute | `W-REPL-DETERMINISTIC` |
...
| A [column](#templates-where-parameters-are-defined) no row of the unit table holds, read off `units` in a template's `aggregate` — reported as [`W-STATS-AGGREGATE-FAILED`](#the-unit-table-is-the-inference-base), since `aggregate` runs after every execution has completed and a metric core can't compute never costs a run its record | `ContractError` · `E-STEP-COLUMN-UNKNOWN` |
...
**Below 80 surviving draws core reports no interval.** That is the fewest at which both percentile ranks are interior: under it the 2.5th percentile *is* the smallest draw, so the interval contains the sample minimum by construction while its upper end keeps shrinking — low-biased and too narrow — and at two survivors the two ranks coincide and the "interval" has zero width. Reporting a point with no interval is honest; a zero-width 95 % interval is not. `run` warns [`W-STATS-AGGREGATE-FAILED`](#warnings-core-reports) or [`W-STATS-RESAMPLE-THIN`](#warnings-core-reports) over the two ways that can go — see the table for which. Neither costs the run its record: every execution had already completed, so the run is `completed`, `run.yaml` is written, the recorded columns keep their own summaries, and what is lost is the derived metric.
