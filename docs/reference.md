...
| `sweep.ablate` is declared (truthy — `null` is not) and `sweep.baseline` is not, so no condition is one change away from anything and the ablated conditions carry only their own change | `E-SWEEP-ABLATE-BASELINE-MISSING` |
| `sweep.ablate` is declared (truthy) alongside a non-empty `sweep.grid`, `sweep.paired` or `sweep.sample` — the axis-shaped modes, read as one set so a later axis mode joins this refusal by existing. `sweep.groups` is not among them and composes freely, varying units rather than parameters | `E-SWEEP-ABLATE-CROSSED` |
| A `sweep.ablate.remove` entry names a path `remove` cannot act on, in either of two ways: the parameter is neither a boolean nor `nullable`, so neither `false` nor `null` is a value it may hold (reported whatever the baseline says); or — for a boolean, and only when a `sweep.baseline` is declared at all — the baseline fixes no value for that path, so `remove` sets `null` rather than `false` and plants it at a parameter that is not nullable. Reported only for a path the template declares, since an unknown one is `E-SWEEP-PATH-UNKNOWN`; the baseline reading is `remove`'s alone, an `override` stating its own value | `E-SWEEP-ABLATE-TARGET` |
