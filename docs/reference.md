...
| [`data.units.weight_by`](#weighted-samples) and [`data.units.cluster_by`](#clustered-units) are both declared and the design resolves to at least one comparison — reading the resolved comparison family, not the declaration, for the same reason a group axis's own guard does: it should not refuse a design that never reaches a comparison. The two unweighted paired clustered constructions exist in this build, deliberately not their weighted counterparts: a weighted clustered interval's df comes from the cluster count, not from Kish's effective size, and the two coincide in any fixture not built to separate them, so the wrong choice would be invisible if approximated rather than refused. Either declaration alone beside a comparison is fine, and a weighted clustered design with no comparison publishes `weighted_t_over_units_clustered` per condition, untouched | `E-DATA-WEIGHT-CLUSTER-CONTRAST` |
