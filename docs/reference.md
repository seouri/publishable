...
| The resolved [`BaseReport`](#a-report-override-renders-one-experiments-own-figures) subclass's own `sections()` body raises while RENDERING — distinguished from every other `E-REPORT-OVERRIDE-*` row above by *when* it happens: those are [override discovery](#a-report-override-renders-one-experiments-own-figures) faults (finding and importing the class), this is the resolved class's own code running. Not a code of its own when the raise is already a `ContractError` — its own code travels instead, on `freeze`'s "the plugin's own code, or a code of last resort" precedent | `E-REPORT-OVERRIDE-RAISED` |
