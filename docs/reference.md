...
| At `resume`, a line of `executions.jsonl` cannot be read as a record of an execution: it is not valid JSON, it parses to something other than an object, or it is missing any of `step`, `scope`, `condition`, `repeat`, `status`. Three faults, one code, one remedy — the ledger was truncated or hand-edited. A reader that skipped such a line would report that triple as *never ran*, so a resume would re-execute work already paid for, or reconstitute nothing for it and publish intervals over the remainder. `returned` and `recorded_columns` are **not** in the required set: a ledger written by an earlier build reads clean here | `E-RESUME-LEDGER-UNREADABLE` |
