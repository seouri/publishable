...
| A `data.units.holdout` field that means nothing under the declared method: `frac` under `by_attribute`, which reads a partition rather than drawing one; `from` under `random`, which draws one rather than reading one; or a non-empty `stratify_by` under `by_attribute`, which names how a draw is balanced when no draw happened. One finding per offending field, so a block declaring more than one earns more than one. The same fault [`E-DATA-ASSIGN-NO-DRAW`](#errors-validate-reports) names one declaration over, including its `!= []` exemption — an empty `stratify_by: []`, what `init` writes, changes no behavior and is not refused | `E-DATA-HOLDOUT-NO-DRAW` |
