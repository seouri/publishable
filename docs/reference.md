...
| The upstream `run.yaml` [`io.reuse_from`](#lineage-between-runs) resolves to: absent, present but unparseable or without a `run_id`, or carrying a `schema_version` this build doesn't read | `ContractError` · `E-UPSTREAM-RECORD-MISSING`, `E-UPSTREAM-RECORD-UNREADABLE`, `E-UPSTREAM-RECORD-VERSION` |
...
| `1` | The thing you asked about is wrong — a config that fails [validation](#validation), a `diff` of runs that don't share a hash, a `resume` whose hashes moved, or a changed [apparatus fact](#the-apparatus-core-can-only-observe) caught before the first execution ran, which leaves nothing to mark `failed` at all |
