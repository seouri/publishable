...
| A step's recorded column is not a number and disagrees across the repeats of at least one unit, so that unit's cell in the collapsed table is `None` rather than a value — the per-repeat `units.parquet` files still hold every observation the disagreement was found across, and the declared route for a within-unit collapse is [`data.units.measurements`](#the-per-unit-tables). Reported at `run` time, once per (condition, step) | `W-STATS-REPEATS-DISAGREE` |
...
**A metric block's `value` is a number, and its `n` counts what it was computed from.** A column in `units.parquet` earns a block in `aggregated` when it carries a number for at least one unit — [§ The per-unit tables](#the-per-unit-tables)' read-side rule — so a column that is non-numeric for every unit reaches `aggregate`'s table and publishes no block, while a column that is a number for some units and `None` for others publishes a block over the units that carried one. **What makes that honest rather than convenient is the count**: the block reports the number of units that contributed, not the run's `completed`, because an interval over five values published beside a `completed` of two hundred is a claim about a precision it does not have — and it is a claim no later reader can catch from the record. A repeat-level disagreement [collapses that unit's cell to `None`](#templates-where-parameters-are-defined), so it counts the unit out of that one metric and out of nothing else. This is the `units` basis's own rule; it says nothing about the `repeats` rows below, whose point estimate is whatever the step itself returned, nor about a `summary` step's own `Estimate`, which core stores rather than computes and which carries `reported: true` for exactly that reason.
