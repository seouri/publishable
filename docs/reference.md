...
| [`study add --as <name>`](#building-one) where `<name>` is already in the bundle — checked BOTH against `study.yaml`'s `runs` keys and against the file `<name>.run.yaml` on disk, since the two can disagree (a hand-edited `study.yaml`, or a copy interrupted between the two writes) and the file is the thing whose overwrite loses data. The load-bearing refusal of the whole command: `main.run.yaml` silently becoming a different run is exactly the overwrite [append-only](design-principles.md#design-goals) forbids, and a bundle beside a manuscript is the last place to allow it. Re-add under a new name, or start a new bundle — adding the same `run_id` twice under two different names is permitted, since the refusal is about the name and a paper legitimately reports one run in two roles | `E-STUDY-NAME-EXISTS` |
...
**Two rows in this table are not raises, and the `Type` cell says so.** A command can also refuse
through a fresh `Collector` — the mechanism `validate` uses — at a point where no step exists to raise
into and no `validate` pass is running: the dirty gate and the empty-file-list gate, both inside
