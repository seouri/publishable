...
| `src/**` or `templates/**` carries uncommitted changes when a command that executes starts. Checked inside `cli._prepare_run` (the run command's phases 1-5, since H9a), before the empty-file-list gate below and before any step runs, for the reason [§ Three hashes](#three-hashes) gives: a `code_hash` folded over a tree that is still moving names no state anyone can return to. **Untracked files count** — the check is `git status --porcelain` over the two trees, which reports them — while **a file the repo's own rules exclude does not**. That question is asked with `-c core.excludesFile=`, [exactly as the hash asks it](#how-the-three-are-computed) and through the same shared constant, so **the gate and the hash honour one exclude chain**, which they did not before: a file a global, system, or repo-local `core.excludesFile` hides is now dirty here *and* hashed there, rather than clean here and hashed there — a recorded identity covering a file no clone of the commit contains. **It is the rules that converged, not the file sets.** The hash additionally drops [the fixed skip set](#how-the-three-are-computed) whatever git says, so a tracked, modified `src/pkg/step.pyc` is dirty at this gate and is read by no hash. One emit site | *(no exception; a `Collector` diagnostic)* · `E-CODE-DIRTY` |
...
would create 20 step directories under /secure/results/cohort-pilot/run_.../
  ...
  conditions/00_baseline/seed137/step03_analyze
  conditions/00_baseline/seed1009/step03_analyze
  conditions/00_baseline/seed2027/step03_analyze
