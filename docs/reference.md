...
| A [hypothesis](#pre-registration) reaches no verdict because the number it names could not be located: either the observation site resolved and the run produced no metric of that name — which core cannot see before the run, since which keys a template's [`aggregate`](#templates-where-parameters-are-defined) or a `summary` step returns is user Python — or `compare: {to: constant}` names no `condition` on a run whose sweep resolved several, leaving no single condition's value to compare against. Reported at `run` time, once per hypothesis, naming which of the two and, for the first, **every metric name the step did record**. The record carries the same fact as [`unevaluable`](#pre-registration) on the entry, because a warning is not written to `run.yaml` and a reader months later has only the record. The step half of a metric name is checked at `validate` instead, by `E-HYPOTHESIS-METRIC` | `W-HYPOTHESIS-UNEVALUABLE` |
...
**A hypothesis that reaches no verdict records why, as `unevaluable`.** `observed: null` alone cannot say which fault it was, and the two have different remedies: `metric_absent` means the observation site resolved and the run produced no metric of that name — a typo, or a template whose `aggregate` never derived the key — while `condition_unresolved` means `compare: {to: constant}` named no `condition` on a run whose sweep resolved several, so there is no single condition's value to compare against. The key is **absent** when there is a verdict rather than present-and-null, on the same rule [`weighted_by`](#weighted-samples) follows: a key that is there and empty reads as *considered, came back nothing*, which is the opposite of what it means. Everything `validate` can see from the declarations alone is refused before a run — an unknown step (`E-HYPOTHESIS-METRIC`), an unknown contrast (`E-HYPOTHESIS-CONTRAST`), an unknown condition label (`E-HYPOTHESIS-CONDITION`) — so these two are what is left, and both need the run to have happened. [`W-HYPOTHESIS-UNEVALUABLE`](#warnings-core-reports) reports the same fact while the config is still in front of you, naming every metric the step *did* record; the field is what survives into the record for whoever reads it later.
