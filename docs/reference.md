...
| `statistics.resample` is declared beside `data.units.cluster_by` and falls in fewer clusters than `limits.min_clusters` — counted over `data.units.holdout`'s realized **test** partition when one is declared, the roster otherwise, because a resample draws from the per-unit table and that is what the table holds; a resample draws whole clusters, so the interval rests on that many independent draws however many units they hold | `W-STATS-RESAMPLE-CLUSTERS` |
