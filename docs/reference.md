...
| `statistics.resample.stratify_by` is declared and validates clean, but no interval construction in this build draws within a stratum — every `percentile_over_units`/`percentile_over_units_clustered` draw in the run is unstratified regardless, for both a recorded column and a derived metric. Reported once per run rather than once per metric, since the gap is a property of the declaration, not of any one interval | `W-STATS-RESAMPLE-STRATIFY-UNHONOURED` |
