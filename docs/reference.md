...
| [`generate step`](#generators) or [`generate report`](#a-report-override-renders-one-experiments-own-figures) naming an `experiment` package whose `src/<pkg>/` does not exist — a different check from `E-EXPERIMENT-EXISTS`'s, which is `generate experiment`'s own overwrite guard on the same directory once it *is* there. Both callers share the one `package_name` helper and its one raise, so a missing package is the identical fault under the identical code regardless of which generator meets it first | `ContractError` · `E-EXPERIMENT-UNKNOWN` |
