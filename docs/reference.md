...
| `src/**` or `templates/**` carries uncommitted changes when a command that executes starts. Checked inside `command_run`, before the empty-file-list gate below and before any step runs, for the reason [§ Three hashes](#three-hashes) gives: a `code_hash` folded over a tree that is still moving names no state anyone can return to. **Untracked files count** — the check is `git status --porcelain` over the two trees, which reports them — while **git-ignored files do not**, and since [the hash honours the same exclude rules](#how-the-three-are-computed) the gate and the hash now consider the same set of files, which they did not before. One emit site | *(no exception; a `Collector` diagnostic)* · `E-CODE-DIRTY` |
...
**One residue, and it is the only machine-dependent input left.** The exclude question is asked with the user's global and system git configuration neutralized, so a `core.excludesFile` on one machine cannot change what another machine hashes — a rule that does not travel with the tree cannot define the tree's identity, which is this section's opening sentence applied to git's own configuration. `.git/info/exclude` is the exception no flag can disable: it lives in one clone and travels with nothing, so a file it excludes is unhashed here and hashed by whoever clones the repo. A file whose hashing status has to be the same everywhere is a file you **commit**.
