...
the two `Param`-construction cases are: unlike them, it also fires for core's own templates and
any that load outside a project's `templates/`, where nothing folds it into `E-TEMPLATE-LOAD` at
all. Each returns because every check after it reads what it just found wrong, and each is
