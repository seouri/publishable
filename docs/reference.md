...
| A [`study.yaml`](#building-one) `report` was pointed at is absent, not valid YAML, does not parse to a mapping, or its `runs` key (or one `runs.<name>` entry) is not a mapping — or a `runs.<name>.file` is absent, not a string, or resolves outside the bundle directory or to something not there. A member's `file` that IS in the bundle but corrupt once read is a distinct, adjacent fault under the shipped `E-UPSTREAM-RECORD-*` family instead — this code is for the bundle DOCUMENT'S own shape, never for a member's contents | `E-STUDY-UNREADABLE` |
