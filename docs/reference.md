...
**The dirty gate moved with the definition, and that is a second behaviour change to `run` (2026-08-23).** The gate is `git status --porcelain` over the two trees asked with the same configuration neutralized, so a repository that relied on a rule the tree does not carry to keep a file out of `src/**` or `templates/**` now **fails `E-CODE-DIRTY` where it used to run**: a global or system `core.excludesFile`, a `core.excludesFile` in the repo's own `.git/config`, the default `~/.config/git/ignore` no config entry names, and a global `status.showUntrackedFiles = no`, which used to hide every untracked file from the gate while the hash folded them in. The remedy is the same one this section already gives — commit the rule. One consequence points the other way and is filed rather than fixed here: `safe.directory` is read from global and system configuration too, so on a repository git considers dubiously owned the gate now gets no answer and reports the tree **clean** — measured, and the run stops two phases later anyway, because the hash asks its own question through the same neutralization and that one fails loudly as [`E-CODE-FILE-LIST`](#errors-core-raises). No record is published either way; what is filed is a gate that answers *no* to a question it could not ask.
