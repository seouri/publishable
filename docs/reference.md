...
| Stratification attribute exists | `stratify_by: label` is not in `data.units.attributes`, or is the column `data.units.measurements.by` names — consumed when a unit's rows collapse, so no resolved unit carries it |
...
| Allocation deltas aren't computed | A resolved comparison — a `vs_baseline`, or a `statistics.contrasts` entry — crosses a [group axis](#expansion-modes) (`00_arm=control` vs. `01_arm=treatment`); the two sides hold disjoint units and no contrast construction computes an unpaired delta — specified, not built in this build. Unlike the two rows below, read per comparison rather than for the whole design: `groups × grid`'s within-arm comparisons stay paired and computed |
...
| Baseline leaves contrasts confounded | `sweep.baseline` fixes a value on every `grid` axis (`analysis.method: pearson`, `analysis.min_samples: 30`) over a grid of `method: [spearman, kendall]` × `min_samples: [30, 50]`, so 2 of the 4 contrasts differ on both and are reported `confounded: true`; leaving one axis unfixed gives a baseline per cell instead (warning). A baseline fixing a [group axis](#expansion-modes) too is outside this check, which reads `grid`'s axes alone — that contrast is still marked where it is formed |
...
**A recorded gap, not yet closed:** [`W-STATS-REPORTBY-THIN`](#warnings-core-reports) counts a `statistics.report_by` level against the roster `validate` can already see — the whole roster, not a group axis's own arm. A design declaring both would have every arm's predicted stratum size overstated by the same whole-roster figure, the validate-time twin of the run-time gap `technical_n` just closed above. **Reachable now**: a declared group axis no longer draws `E-SWEEP-GROUPS-UNSUPPORTED` at `validate`, so a config combining `sweep.groups` with `statistics.report_by` constructs this case for real rather than merely being imagined — recorded in `docs/superpowers/spec-defects.md` as a live gap rather than a latent one.
