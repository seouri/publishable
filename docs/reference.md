...
| A `name` that escapes the directory it is checked against — the step's own on a write, or the target step's on [`io.read_condition`](#steps-that-need-every-condition), [`io.read_upstream`](#step-scope), or a [report override's](#a-report-override-renders-one-experiments-own-figures) own `io.read_condition`, which checks the identical target-step containment through the same `_contained` call — an `io.append` onto anything but `.jsonl`, or an extension [no writer claims](#steps-and-artifacts) handed an object that isn't `bytes` or `str`. Four emit sites for the escape alone: a write checks against the calling step's own directory, and each of the three readers checks against the directory of the step named in the call — one row because the check and the remedy are the same containment rule at all four, never a check on the path's shape | `ArtifactError` · `E-ARTIFACT-NAME`, `E-ARTIFACT-APPEND`, `E-ARTIFACT-UNWRITABLE` |
