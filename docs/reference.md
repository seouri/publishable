...
check. **Not yet true in this build:** the function that would load a resolver has no production
caller yet, so `validate` does not reach either check for any config — a resolver source is still
refused wholesale under `E-DATA-RESOLVER-UNSUPPORTED`. `E-TEMPLATE-LOAD`
...
| A [`@register_*` argument](#creating-a-plugin-publishable-plugin-new) disagreeing with the entry-point key that named it. The entry point is the registration and the decorator is a declaration checked against it, so two spellings of one name with no rule for which is canonical is refused rather than resolved — the [defaults-file argument](#there-is-no-separate-defaults-file) again. The comparison itself exists as a function. **Not yet reached at `validate` in this build:** the function that would load a resolver and run this check has no production caller yet, so a resolver's disagreement is not yet `validate`'s finding, only `run`'s once the resolver actually executes | `ContractError` · `E-PLUGIN-DECORATOR` |
| An entry point whose module raises while importing, or calls `sys.exit()` at module scope. `SystemExit` is a `BaseException` and so needs its own `except` — a plugin building an `argparse` parser at import would otherwise end the command with the plugin's own exit code and no diagnostic at all. The same containment `E-PLUGIN-DECORATOR` describes — reached wherever a plugin is imported, which is a [resolver source](#where-units-come-from)'s dispatch. **Not yet reached at `validate` or `dry-run` in this build**, for the same reason `E-PLUGIN-DECORATOR` is not: the loading function has no production caller yet. The fault names the entry point and the distribution rather than the module, since a distribution is what a reader uninstalls or pins | `ContractError` · `E-PLUGIN-LOAD` |
