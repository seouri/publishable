...
| The mirror of `E-DATA-ALLOCATION-NO-ARMS`: [`sweep.groups`](#expansion-modes) declares an axis, but [`data.units.allocation`](#allocation-within-subjects-or-between-subjects) is `within`, or absent (which defaults to it). `within` says every unit appears in every condition, so a unit can't be in one arm and in all of them — the fault [§ Validation](#validation)'s *Arms need allocation* names, and the pair this row and its mirror form is what makes handing every condition on a group axis the same, whole roster structurally impossible rather than merely avoided. Read from the declarations, so it reports whether or not a roster resolved. Gated on `allocation` being exactly `null`/absent/`within`, not on a bare declared axis, so an out-of-enum `allocation` value is left to `E-DATA-ALLOCATION-UNSUPPORTED`'s blanket refusal rather than being misreported here as `within`. **Temporary in that one respect**: once the slice that retires `E-DATA-ALLOCATION-UNSUPPORTED` lands, an out-of-enum `allocation` beside a declared group axis will draw neither this row nor any other — a gap this row's own gate creates rather than closes, recorded in `docs/superpowers/spec-defects.md` | `E-DATA-ALLOCATION-WITHIN-ARMS` |
...
**A recorded gap, not yet closed:** [`W-STATS-REPORTBY-THIN`](#warnings-core-reports) counts a `statistics.report_by` level against the roster `validate` can already see — the whole roster, not a group axis's own arm. A design declaring both would have every arm's predicted stratum size overstated by the same whole-roster figure, the validate-time twin of the run-time gap `technical_n` just closed above. Unreachable while a declared group axis draws `E-SWEEP-GROUPS-UNSUPPORTED` at `validate`, so nothing today constructs the case; recorded here rather than left silent for whichever slice retires that refusal.
