...
| A [`data.units.from.resolver`](#where-units-come-from) source yields something that is not a `Unit` — a resolver is the second place user code runs inside resolution, and `validate` is contracted never to raise, so a yielded mapping or other object reaching `u.key` as an `AttributeError` would escape it. Raised in `units._from_resolver`, the one place a yielded item is checked, and [reported by `validate`](#errors-core-raises) under the same code, reaching it through `_check_units`'s `except ContractError` the same way `E-DATA-CLUSTER-VARIES` does | `E-RESOLVER-YIELD` |
