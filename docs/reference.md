...
| Allocation strata exist | `assign.arm.stratify_by: [site]` but `site` is neither a unit attribute nor a group axis. Owns every `assign.<axis>.stratify_by`, including a target naming a group axis — unlike *Stratification attribute exists*, whose `fold`/`holdout` stratify targets admit only a unit attribute |
...
| Under `method: random` or `blocked`, `assign.<axis>.stratify_by` names a value that is neither a unit attribute `data.units.attributes` declares nor an earlier-resolved [`sweep.groups`](#expansion-modes) axis — the one target [§ Validation](#validation)'s *Allocation strata exist* admits that a `fold`'s or `holdout`'s `stratify_by` does not, forward-only order being what lets an axis stratify on a group axis resolved before it. `data.units.attributes` and `sweep.groups`, not the roster's realized columns, for the same reason `E-DATA-CLUSTER-UNKNOWN` reads that set: a stratum is read per unit when the assignment is drawn, so it has to survive resolution as an attribute or resolve as an axis. Checked from the declaration alone, so it reports whether or not a roster resolved | `E-DATA-ASSIGN-STRATIFY-UNKNOWN` |
