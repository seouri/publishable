...
| At `resume`, a completed triple's ledger line names a non-empty `recorded_columns` and no `units.parquet` exists beside it. Distinct from the row above and from *no rows at all*: a step that recorded nothing writes no file and records an empty list, which is legitimate and resumes | `E-RESUME-ROWS-MISSING` |
