...
| A fact value a probe returned equals the value core read for a declared credential — refused rather than redacted, because [`provenance.apparatus` is publishable as-is](#the-apparatus-core-can-only-observe) and a redacted value sitting in a block whose whole contract is that it needs no redaction would still be evidence a credential had been there. Checked by **exact value**, never by pattern, against exactly the set [`credential_values`](#secrets--credentials) answers for this config's declared names — and checked before the scalar walk below, since a credential passing as a plain string must be caught here rather than reaching it. Raised in `apparatus.check_facts` | `ContractError` · `E-APPARATUS-FACT-CREDENTIAL` |
