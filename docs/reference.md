...
| Assignment method isn't drawn | `assign.arm.method: random` or `blocked` — both are in the enum "Assignment names a method" above checks against, and both draw an arm rather than reading one already assigned, which is specified, not built in this build; only `by_attribute` executes |
...
| Allocation strata survive clustering | Under `method: random`, `assign.arm.stratify_by: [label]` with `cluster_by: animal_id`, but `label` varies within animal `A3` — a cluster is drawn whole, so an arm can't be balanced on a stratum that cluster carries two of. The same rule and the same reason as *Fold strata survive clustering* below; `blocked` beside any `cluster_by` is refused outright by *Blocked draw excludes clustering* above, so this row reaches `random` alone |
...
loads, before either of the later two returns is possible, and except the one non-container
...
| Under `method: by_attribute`, `assign.<axis>.from` — declared, or defaulted to the axis name the same way its own name check resolves it (see `E-DATA-ASSIGN-UNKNOWN`) — names a column whose value is not the same across one unit's [measurement rows](#what-isnt-a-repeat), the same shape `E-DATA-CLUSTER-VARIES` and `E-DATA-WEIGHT-VARIES` refuse and reported the same way, through the same collapse. **The worst of the three**: a mis-collapsed cluster decides which side of a split a unit lands on and a mis-collapsed weight mis-sizes what one unit stands for, but a mis-collapsed arm decides which *condition* the unit is measured in — so the order the file happens to be in would be silently deciding it. One entry per declared axis feeds the same collapse, and each declaration is checked on its own and still raises its own code when it is the only one declared; a single unit violating more than one gets exactly one code, from whichever declaration's entry is checked first — `assign`, ahead of `cluster_by`/`weight_by`, matching the severity this row states | `E-DATA-ASSIGN-VARIES` |
...
`arm`'s `method: random` carries the same refusal as the single-axis example above — `sex`'s `by_attribute` is unaffected.
...
The first row's two `by_attribute` axes are the only one of the three that executes in this build; the other two describe `random`, refused as [`E-DATA-ASSIGN-DRAWN`](#allocation-within-subjects-or-between-subjects).
