...
are reported by `validate` only where it actually loads a plugin, which is a
[resolver source](#where-units-come-from) and nothing else — resolving a *name* is answered from
package metadata and imports nothing, so a config that names no resolver reaches neither check.
Both are findings rather than early returns: `_check_units` reports what resolution raised and
the pass continues. `E-TEMPLATE-LOAD`
