...
hypotheses:
  ...
  - id: above_chance
    ...
    direction: greater
    threshold: 0.0
    evaluate_on: observed                     # see below: a bound here has no `Member` to correct
```
...
**A constant-referenced hypothesis joins `family_size`, and that is the only correction it gets today.** Core builds no correctable `Member` for a condition's own value the way it does for a `vs_baseline` delta or a declared contrast, so under `correction: none` its `observed` carries no `ci95_corrected` key exactly like every other member of a family nothing was attempted for — but under a real method (`holm`, `bonferroni`, `fdr_bh`), evaluating it on `ci95_lower` or `ci95_upper` reads `supported: null` and `ci95_corrected: null`, the same honest gap [`W-STATS-CORRECTED-THIN`](#warnings-core-reports) reports for a family too large for its resample's draws to support. `evaluate_on: observed` is unaffected — correction only ever tightens a bound, never the point estimate — which is why the example above reads on `observed` rather than a bound.
