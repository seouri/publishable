...
| A [`sweep.baseline`](#expansion-modes) fixing a value on every `sweep.grid` axis leaves at least one condition differing from it on more than one of them, so that comparison is reported `confounded: true` and its delta mixes two effects. Checked at `validate` over the declared axes alone, and only for a baseline that fixes every one of them — a baseline leaving an axis free is the [remedy](#expansion-modes), not the fault, since it expands to one baseline per cell and every comparison then differs in exactly one place | `W-SWEEP-BASELINE-CONFOUNDED` |
