...
| `sweep.baseline` is declared alongside a `sweep.sample` axis, so every draw becomes a `vs_baseline` comparison — while a `sample` draw is [not a comparison](#sweeps-and-repeats) and the correction family skips it, which is specified but not implemented in this build. A `sample` sweep with no baseline is unaffected, since a comparison is generated only against a declared one, and so is a declared [`statistics.contrasts`](#contrasts-claims-that-arent-condition-vs-baseline) entry, whose members are named rather than generated | `E-SWEEP-SAMPLE-BASELINE` |
