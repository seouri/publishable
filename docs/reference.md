...
| No git repository walking up from the path given, raised once, in `provenance.find_repo_root`. Eight paths reach it. `cli._prepare_run` (the run command's phases 1-5, since H9a) and the `generate`/`init` dispatch call it uncaught, surfacing it at `main`'s `except PublishableError` printer on stderr, exit `1` — the creation commands walk up from **`Path.cwd()`** rather than a path argument, being the commands with none to walk up from — the one place `CLAUDE.md` § Invariants' "a walk-up from the path the command was given, not from the working directory" does not apply, and a reader comparing the two without that sentence would conclude one is wrong. Three sites catch it **by code**: `validate._check_data` returns quietly ("not in a repo, so inside-the-repo doesn't arise"); `study._refuse_if_in_repo` catches it as the pass branch of its own in-repo refusal; and [`reproduce`](#reproducing-on-another-device)'s config form catches it and **re-reports it under the same code** through its own `Collector`, at exit `1` — a config outside every repository names no project to restore, and re-reporting rather than re-raising is what keeps the [redaction pass](#secrets--credentials) over it, since `main`'s own handler applies none. The first two are pass branches; the third is a refusal, and that is the difference worth reading. Three more catch it **by type**, testing no code: `reproduce.prepare_checkout`'s walk-up from the derived destination's parent, where a raise IS the ordinary case — a destination outside any repository is what `reproduce` wants, so the exception path is the pass branch and the *quiet* return is the refusal; `validate.validate_config`, under a bare `except ContractError`, leaves `repo_root` `None` so local template discovery is skipped while every other check still runs; `cli._preloaded_experiment`, under `except Exception`, returns `None`. This is why a config outside every repository prints `✓ config valid` and refuses only when a command needs the repository — at `run`, and at `reproduce` | `ContractError` · `E-GIT-NO-REPO` |
