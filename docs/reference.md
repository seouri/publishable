...
limits:
  ...
  min_units_per_cell: 20           # `validate` warns when the design's thinnest cell holds fewer
                                   #   units than this; only for a design that HAS cells, so a
                                   #   small project with no `sweep.groups` axis never fires it
...
| Defaulted parameter left unset | `analysis.confidence` carries a default and isn't set under `parameters`; `cfg.parameters.analysis.confidence` raises `E-STEP-PARAM-UNKNOWN` for a step that reaches for it directly (warning) |
...
| Attribute names aren't reserved | `data.units.attributes` names `paths`, which is a field of [`Unit`](#where-units-come-from) itself — `key`, `paths`, and `attributes` can't also be attributes (`E-UNITS-ATTR-RESERVED`) — or names `unit`, `measurement`, or `by`, each already a column of [a per-unit table](#the-per-unit-tables) or the block [a stratified report](#reporting-strata) keys its rows by (`E-UNITS-ATTR-COLUMN`) |
...
| Cells are populated | `sex × arm` over 40 units gives cells of 10; below `limits.min_units_per_cell`, so `validate` warns (`W-DATA-CELL-THIN`). Reported for the thinnest cell that holds at least one unit, and only for a design whose cells resolve — a design with no `sweep.groups` axis has no cells to be thin, and an arm no unit resolves to is refused as `E-DATA-ASSIGN-LEVELS` rather than reported as a thin cell |
...
| Allocation is coherent | `allocation: between` over 2 arms and 12 units gives cells of 6; below `limits.min_units_per_cell`, so `validate` warns (`W-DATA-CELL-THIN`) — the same check *Cells are populated* above states from the crossed-axes side, a single arm axis being a one-axis product |
...
| Folds fit inside the cells | `{kind: fold, k: 5}` beside a `sweep.groups` axis whose thinnest arm holds 3 units — a fold level runs inside every cell, so `k` is bounded by the **thinnest populated cell's** unit count, or its cluster count under `data.units.cluster_by`, and not by the whole roster's. The same rule *Folds fit inside the clusters* states for one roster, asked of the cell the fold is actually drawn inside; the message names that cell, since the declaration a reader must change is one arm's. A cell no unit falls in bounds nothing — it is skipped, not counted zero |
...
| Contrast has units in common | `statistics.contrasts[0]` compares two conditions whose completed units don't intersect, so no paired difference exists. Covers the case a differing [group axis](#expansion-modes) does not: two same-arm conditions with a `within` stratum, or any pair `resolve_units` happens to leave disjoint, that carry no differing group axis between them — a comparison crossing a group axis is unpaired instead, computed by `welch_t_over_units`/`unpaired_percentile_over_units` and their `_clustered` forms — **for a recorded column**, those constructions closing over the column's own values; a metric the template DERIVES gets no unpaired contrast at all and is reported by [`W-STATS-CONTRAST-UNPAIRED-DERIVED`](#warnings-core-reports) instead, and this row reads the intersection `stats.paired_keys` actually computes for the paired case alone |
...
| Reported when | Code |
|---|---|
| A [declared fact](#the-apparatus-core-can-only-observe) came back `null` from at least one probe call — once per (condition, fact) pair with at least one such call, printed to stdout through a [`Collector`](#exit-codes-and-diagnostics) at run end, read off the accumulated per-(condition, fact) counts rather than emitted per call: under [every execution probed](#the-apparatus-core-can-only-observe), a single flaky fact would otherwise print the same line once per probe that missed it. Never fired for a fact `apparatus_facts` does not declare — an undeclared fact's `null` is recorded in `provenance.apparatus.facts` but warns about nothing, since the warning is what a *declaration* buys. Never changes an exit code, on `W-ENV-UNLOCKED`'s own precedent. **A second surface**: [`freeze`](#cli-reference) fires the identical warning at the end of its own invocation, printed the same way through its own `Collector`. **`freeze`'s own counts** are the run's own accumulated `run_start`/`pre_execution` history, replayed from the ledger as this invocation's starting point, **plus** the one round `freeze` itself just probed — never a fresh accumulator, and never a prior `freeze`'s own round, which the ledger replay excludes. **A third**: [`dry-run`](#before-you-spend-it) fires it from its own probe round, over that round's own in-memory counts alone — one call per resolved condition, so a declared fact it did not answer reads `null` on 1 of 1 probes — printed to stdout through a fresh `Collector` before the transcript, which is the whole point of learning it there rather than after a metered run | `W-APPARATUS-UNANSWERED` |
| The design's thinnest **populated** cell holds fewer units than `limits.min_units_per_cell`, checked at `validate` against the roster it can already see and against the cells it draws for real — every [`sweep.groups`](#expansion-modes) axis realized and intersected, the same decomposition the [fold bound](#validation) and the [holdout](#a-fixed-holdout-split) are computed over. **Gated on a cell structure resolving**, and never fired for a design with no group axis: `limits.min_units_per_cell` is materialized in every generated config, so an ungated floor would fire on every small project that never declared a cell at all — a warning readers learn to skip is worse than none. A design whose cell draw cannot be performed is silent for the same reason, the fault it carries being reported by its own check. Reported **once**, for the thinnest cell, on `W-DATA-CLUSTER-UNDECLARED`'s own shape: the remedy — enrol more units, or drop an axis — is the same for every thin cell in one design. A cell **no** unit falls in is excluded rather than counted zero: a declared level nothing resolves to is refused as `E-DATA-ASSIGN-LEVELS`, and a cell no reader can make thicker is not a floor this can state | `W-DATA-CELL-THIN` |
...
| Checked at `validate`, from the declaration alone: a [`parameter_spec`](#the-importable-surface) path carries a default and this config does not set it under `parameters` — one diagnostic at path `parameters` naming every such path, on [`W-TEMPLATE-VERSION`](#warnings-core-reports)'s own enumerating shape. Fires unconditionally, not gated on any version mismatch: omitting a defaulted parameter is what almost every hand-written config does, and a freshly `init`-ed one sets all of them, so this fires exactly on the hand-trimmed configs the fact is about. States the consequence rather than a guess about intent — a step reaching for the path as `cfg.parameters.<path>` raises [`E-STEP-PARAM-UNKNOWN`](#errors-core-raises), since core never writes a config's declared defaults back into the file it reads. Covers the `parameters` block only; the identical symptom through an omitted **core-schema** key is filed, not built — core reads its own schema keys defensively, so the only casualty is a step reaching for one through `cfg`, and closing it needs either [the defaults structure this document forbids](#there-is-no-separate-defaults-file) or the [greenfield line](design-principles.md#greenfield-only) crossed | `W-PARAM-UNSET` |
...
| A step's recorded column carries a real number for fewer units than `limits.min_reported_n` — the **contributing** count reported as that column's own `n.completed`, which is what an interval over a [partially covered](#the-unit-table-is-the-inference-base) column is computed at. Reported at `run` time, once per (condition, step, recorded column); the same floor `W-STATS-CONTRAST-THIN` and `W-STATS-STRATUM-THIN` read against a realized denominator, and a column no unit carries a number for earns no block and no warning | `W-STATS-COLUMN-THIN` |
| A [hypothesis](#pre-registration) reaches no verdict because the number it names could not be read: the observation site resolved and the run produced no metric of that name — which core cannot see before the run, since which keys a template's [`aggregate`](#templates-where-parameters-are-defined) or a `summary` step returns is user Python; or `compare: {to: constant}` names no `condition` on a run whose sweep resolved several, leaving no single condition's value to compare against; or the metric block is there and **carries no number**, its `delta` or `value` `null`, which is the state that looks healthiest and is not. Reported at `run` time, once per hypothesis, naming which of the three, for the first **every metric name the step did record**, and for the third that the reason the number is null is among the run's own findings. The record carries the same fact as [`unevaluable`](#pre-registration) on the entry, because a warning is not written to `run.yaml` and a reader months later has only the record. The step half of a metric name is checked at `validate` instead, by `E-HYPOTHESIS-METRIC` | `W-HYPOTHESIS-UNEVALUABLE` |
| A [comparison](#contrasts-claims-that-arent-condition-vs-baseline) whose two conditions share no completed units names a metric the template **derives** in [`aggregate`](#templates-where-parameters-are-defined) rather than a recorded column, so no delta and no interval are built: the unpaired constructions `welch_t_over_units`/`unpaired_percentile_over_units` close over a column's own values, and recomputing a derived metric on each side would need `aggregate` evaluated on two independently drawn tables — a construction this build does not have. The entry still carries `n_of` and `n_against`, and a bound-evaluated hypothesis reading it comes back `supported: null`. Reported at `run` time, once per (comparison, step, metric); **not** a `validate` row, because whether a metric name is derived or recorded is a fact about what an `aggregate` returns, which core never reads. Record the quantity as a column, or carry the comparison as a [`summary`-step `Estimate`](#the-unit-table-is-the-inference-base) | `W-STATS-CONTRAST-UNPAIRED-DERIVED` |
...
| A step's recorded column disagrees across the repeats of at least one unit — the per-repeat `units.parquet` files still hold every observation the disagreement was found across, and the declared route for a within-unit collapse is [`data.units.measurements`](#the-per-unit-tables). Reported at `run` time, once per (condition, step, recorded column) | `W-STATS-REPEATS-DISAGREE` |
...
| A step records a column named `by` — the reserved key the reporting strata are attached under — so that column is reported with no contrast delta and no seat in the correction family, and no strata are reported for the step at all. Fired for a `by` column whatever it holds, over the same three mixtures [§ The per-unit tables](#the-per-unit-tables) draws for any recorded column: a number for every unit keeps its own metric block beside the warning, a number for some units and `None` for the rest keeps a block computed over the units that carried one, and no unit recording a number keeps none — the fault is the recorded name either way | `W-STATS-STRATUM-SHADOWED` |
...
| A [`run`- or `condition`-scoped step](#step-scope) returns a **non-empty** mapping, which has nowhere to land: a metric is keyed by unit or reported per repeat, and neither scope has either, so core discards it. Reported once per (step, scope) rather than once per execution — a condition-scoped step returns once per condition and one mistake should not print nine times — with the dropped keys unioned across those executions. **An empty mapping draws nothing**, which is what makes the warning quiet for every wide step that never meant to report: the distinction between a value an author computed and one they never had needs no declaration, since a non-empty return at these scopes IS the first. The route is the one [§ Step scope](#step-scope) already gives — `io.write` for what a wide step produces, a narrower step for what it measures | `W-STEP-RETURN-DISCARDED` |
| Two [bundled](#building-one) runs sharing a `provenance.git.commit` disagree on `provenance.apparatus.hash`. A run whose `provenance.apparatus` is `null`, or a mapping carrying no `hash` at all, is **excluded** from this comparison rather than counted a mismatch — "this experiment declares no probe" is not a deployment claim, unlike [`diff`](#the-apparatus-core-can-only-observe)'s own one-sided-`null` reading, which is a different question over two runs rather than one claim over a group. Never calls [`apparatus.apparatus_hash`](#the-apparatus-core-can-only-observe): recomputing it here could disagree with the one figure `diff`'s own `apparatus` row treats as authoritative, so this compares the two RECORDED `hash` strings only | `W-STUDY-APPARATUS-MISMATCH` |
| Two [bundled](#building-one) runs whose `provenance.git.commit` agree do not share a `code_hash` — a real finding, since the same commit means the same two hashed trees. A run carrying no `code_hash` at all is excluded from this comparison, the identical rule `W-STUDY-APPARATUS-MISMATCH` applies to a missing `hash`. States what was found (both commits, the differing hashes) and diagnoses no cause: a dirty tree, an uncommitted `templates/**` edit, and another experiment's package moving inside [the two hashed trees](#how-the-three-are-computed) are three candidates among others, and naming one as fact would be the comment-claiming-a-guarantee habit `CLAUDE.md` names. `report study.yaml` computes neither side — [`hashes.code_hash`](#how-the-three-are-computed) is never called on this path — it only compares the two RECORDED strings | `W-STUDY-CODE-HASH-MISMATCH` |
| [`study add`](#building-one)'s run being added records a `provenance.git.commit` that differs from `study.yaml`'s existing `code.commit` — a notice, not a refusal, because a sensitivity analysis rerun a month later at a later commit is ordinary. Fires only when the add is neither the bundle's first nor `--as main` — either of those WRITES `code.commit` from this run instead of comparing against it. Names both commits and which run each belongs to, and never computed: it compares the two records' own `provenance.git.commit` strings, on the identical grounds `W-STUDY-CODE-HASH-MISMATCH` already gives for a bundle-level comparison | `W-STUDY-COMMIT-MISMATCH` |
...
| Two conditions [`expand`](#expansion-modes) renders resolve to the same `values` over the same units — the same measurement, run twice under two labels and two directories. Checked at `validate` over `expand`'s own output, so it is blind to which mode produced either condition: a `sweep.baseline` fixing a path the `grid` also lists is one route to it, and two `grid` cells colliding through an unrelated axis is another — the check asks the direct question rather than naming a route. Identical `values` means identical units even across a [`sweep.groups`](#expansion-modes) axis, whatever `data.units.assign`'s method: a group axis is realized once per run — [§ Allocation](#allocation-within-subjects-or-between-subjects)'s draw computes the whole axis's arms in one call — so every condition naming a given level reads the same resolved membership. What this check skips instead is narrower, and scoped to the exact pair a sharper code already names rather than to every pair sharing an axis it mentions: a candidate pair is skipped only when it IS the baseline row against its own axis's matching product row — same fixed axis, same fixed value, exactly one side `is_baseline` — which is `E-SWEEP-BASELINE-GROUP`'s shape (or `E-SWEEP-ABLATE-BASELINE-GROUP`'s, its `sweep.ablate`-declared sibling; both name only the same single axis, `fixed_levels[0]`, never a second one `sweep.baseline` also fixes), or when the shared value is a level that axis's own `levels` list repeats. That clause excuses more than `E-SWEEP-LEVEL-DUPLICATE`'s own single reported finding whenever another axis crosses it, and it is withheld outright once a `grid` axis also repeats a value — but every pair either form excuses sits in a config a repeated `levels` entry already refuses outright as `E-SWEEP-LEVEL-DUPLICATE`: such a config does not run, so nothing here is withheld from a config a reader could otherwise act on. Two measured shapes — `groups: [{by: arm, levels: [c, c]}]` crossed with a repeated `grid` value, bare and baseline-crossed alike — are refused by `E-SWEEP-LEVEL-DUPLICATE` regardless and left unwarned by this check for that reason, parked rather than closed as a separate case. A pair that fails both is not unreachable — `validate` collects rather than aborting, so a config either sharper check refuses still reaches this one — it is a fault neither of them describes, including two baseline rows or two product rows a `grid` repeat alone produced. Reported once, for the first duplicated pair in condition order, on `W-DATA-CLUSTER-UNDECLARED`'s own shape — the remedy is the same sentence whichever pair a reader looks at, and it is [`W-SWEEP-BASELINE-CONFOUNDED`](#warnings-core-reports)'s own remedy: fix the axis you are measuring and leave the ones you are stratifying over free, and each cell gets its own baseline. A warning, not an error: the design is expensive and confusing rather than unexpressible, and a refusal would be a new way for a config that validated yesterday to stop. Core reports; it does not deduplicate — dropping a condition would change what executed without the record saying so | `W-SWEEP-CONDITION-DUPLICATE` |
...
| At [`freeze`](#cli-reference), the repo's `uv.lock` no longer hashes to the copy captured in `environment/uv.lock` at run start. Never a code change — nothing mid-run re-checks the lockfile, so an exit `1` here would flag something that cannot stop the run — and silent when nothing was captured at run start (a project with no lockfile then and none now) | `W-FREEZE-LOCK-MOVED` |
...
code raised at load can be in both, reported here and raised there. **Read that scope sentence
literally rather than the heading**: a command other than `validate` can also collect and report,
through a fresh `Collector` and without raising anything, and those codes are here too — the
`E-RESUME-*`, `E-FREEZE-*` and `E-REPRODUCE-*` families, plus [`diff`](#cli-reference)'s own config
operand. What decides a row's table is whether a *command reports* it or core *raises* it, not which
command it belongs to. Each
...
A code ending `-UNSUPPORTED` is exempt from this table **when it is temporary**: it refuses a
block this build reads but does not yet execute, it retires with the slice that implements the
feature it names, and documenting one here would pin a row the project has committed to deleting.
[§ The one config file](#the-one-config-file) is where that family's rule lives, and where a
currently-refused block is named and marked `NOT BUILT` in the config it shows.

**No member of the family is temporary today, and the exemption therefore covers none of them.**
`E-TEMPLATE-INSTALLED-UNSUPPORTED` is the one that remains, it refuses a *value* rather than a
block, and it is **permanent as this project ships** — the charter is complete and no slice loads
an installed template — so it carries a row below like any other permanent refusal, and its
message names the route rather than a slice. The suffix is kept because an identifier is stable
across everything except its own meaning, and what the suffix says of the *feature* — installed
templates are unsupported — is exactly true. What it stopped saying, on 2026-08-27, is anything
about *when*.
...
template core does not load (`E-TEMPLATE-INSTALLED-UNSUPPORTED`) — the last two decided by
...
are specified to be reported by `validate` only where it actually loads a plugin, which is a
[resolver source](#where-units-come-from) and nothing else — resolving a *name* is answered from
package metadata and imports nothing, so a config that names no resolver would reach neither
check. `E-TEMPLATE-LOAD`
...
or a code to this set. A malformed `parameter_spec` path folds into the same "raises while
importing" shape for a project-local template — the raise is `BaseTemplate.__init_subclass__`'s,
at class-*definition* time, which for such a file happens inside the same import — but it is
carried at its own row below (`E-TEMPLATE-PARAM-PATH`) rather than left out of this set the way
the two `Param`-construction cases are: unlike them, this check also runs for core's own
`generic`, at `registry.py`'s own module-level import, which is not inside `discover_local`'s
try/except at all. Core ships only `generic`, and its path is correct, so that site never
actually raises — there is no reachable command surface where this fires outside a
project-local `templates/*.py`, and this row does not claim one. Each returns because every
check after it reads what it just found wrong, and each is
...
| Under `method: random`, unstratified and unclustered, `frac` apportions the test side zero units — every metric would be over nothing. The denominator is the resolved roster, or, when the design's cells **resolve**, its **thinnest populated cell**, because that is the sub-roster the split is drawn inside. *Resolve*, not merely *are declared*: a cell structure whose own draw faults is reported by that fault's own check, and this bound falls back to the roster rather than reporting a second, derived one on top of it (see [A fixed holdout split](#a-fixed-holdout-split)) and a `frac` that clears the roster need not clear every cell of it; the message names that cell. One code and one remedy either way — widen `frac`, or resolve more units — which is why the cell case is this row rather than a second code. The **test** side only: a cell thin enough to leave the *train* side empty is [§ Errors core raises](#errors-core-raises)' own row. Reported for the unstratified, unclustered draw only, mirroring *Every arm draws units*: a stratified or clustered split is checked where the run performs it, because a cluster is the smallest thing that can move and only the draw knows what it moved | `E-DATA-HOLDOUT-EMPTY` |
| [`data.units.weight_by`](#weighted-samples) and [`data.units.cluster_by`](#clustered-units) are both declared and the design resolves to at least one comparison — reading the resolved comparison family, not the declaration: a `sweep.baseline` with no axis beside it publishes no delta, and this guard should not refuse a design that never reaches a comparison. The two unweighted paired clustered constructions exist in this build, deliberately not their weighted counterparts: a weighted clustered interval's df comes from the cluster count, not from Kish's effective size, and the two coincide in any fixture not built to separate them, so the wrong choice would be invisible if approximated rather than refused. Either declaration alone beside a comparison is fine, and a weighted clustered design with no comparison publishes `weighted_t_over_units_clustered` per condition, untouched | `E-DATA-WEIGHT-CLUSTER-CONTRAST` |
...
| A hypothesis's `compare` declares `to`, and its value is neither `baseline` nor `constant` — or declares `to: constant` and `contrast` together, two comparison targets where only one is ever made | `E-HYPOTHESIS-COMPARE-TO` |
| A hypothesis's `compare.to` is `constant` and `compare.value` is missing or not a number | `E-HYPOTHESIS-COMPARE-VALUE` |
| A hypothesis's `compare.condition` names a label the run's `sweep` does not declare, or names the baseline's own label under `to: baseline` — not under `to: constant`, where the baseline's own value is a real, resolvable claim — checked only once a baseline is declared, since `E-HYPOTHESIS-BASELINE` already covers the case where none is, and only once `sweep` expands cleanly enough to resolve condition labels at all | `E-HYPOTHESIS-CONDITION` |
...
| The resolved template declares an [`apparatus_probe`](#the-apparatus-core-can-only-observe) that no installed distribution registers under the `publishable.probes` entry-point group, **or declares one that is not a usable name at all** — anything other than a non-empty `str`, `None` being the only spelling of "no probe declared." Answered from metadata, the same way and for the same reason a resolver name is. Reported at `experiment_type` — the field that decided which template's declaration applies — since the probe name is the template's rather than the config's, and a reader who cannot install the plugin fixes this by choosing a different template. A template declaring no probe (`None`) is the ordinary case and draws nothing here — a `list`, an empty string, or any other non-`str` value is not that case and is reported rather than silently read as one. **Dual-surface**: `apparatus._probe_for` raises the identical code, from the identical metadata scan, at dispatch — reached wherever a command resolves the declared probe to call it, which is `run` and `draft` through `cli._execute_prepared`, [`dry-run`](#before-you-spend-it) through its own probe round, and [`freeze`](#cli-reference) through its own — `freeze`'s omission from this clause predates H9a and is corrected here rather than left, since it has dispatched through the same `_probe_for` since H8b. `validate` answers this one first in the ordinary case, so the dispatch raise is met only where nothing validated this config first, or where the installed set changed between the two — never through a call this section's own guard forbids, since `validate` itself calls no probe to reach it | `E-PROBE-UNKNOWN` |
...
| A `fold` level's `k` exceeds the number of things a fold cannot divide — the resolved unit count, or the cluster count when `data.units.cluster_by` is declared — which would leave a fold with nothing to test. **Where a design's [`sweep.groups`](#expansion-modes) axes decompose the roster into cells, the count is the thinnest cell's rather than the roster's** — a fold runs in every cell, so the cell with fewest units, or fewest clusters, is what makes the declaration unaffordable — and the message names that cell by its `axis=level` pairs, so a reader is sent to the arm the bound bit on rather than to a roster-wide number no bound used. Ties go to the first cell in declaration order. **One row, three emit sites**, and the clause reaches all three: `validate` reports it as a finding, and `replication._fold_k` raises it twice — once counting units and once counting clusters — which is why a message asserted at one site says nothing about the other two | `E-REPL-FOLD-K-TOO-LARGE` |
...
| A [`Section`](#a-report-override-renders-one-experiments-own-figures)'s `body` is neither `str` nor a mapping — an override's own `self.section(..., body=...)` handed the module something else entirely. Refused rather than reaching `.get` and raising a bare `AttributeError` out of a renderer: `body` is the one value an override supplies freely, the same reason `E-REPORT-FORMAT` refuses rather than defaults. Scoped to exactly this shape — a `sections()` that yields something other than a `Section` at all, bypassing `self.section()` itself, fails on `.title`/`.body` before this check is ever reached, and is a different, unowned fault | `E-REPORT-BODY` |
| This run's own record carries `draft: true` — see [Draft runs](#draft-runs). A refusal, not a watermark: `report` renders nothing rather than a citable page with a banner on it, which is the sentence the whole `draft`-versus-`--allow-dirty` argument rests on. A single run's own refusal only — [`report study.yaml`](#building-one) flags a draft member instead, since refusing a whole bundle render for one draft member would throw away every other legitimate one | `E-REPORT-DRAFT` |
| [`report`](#cli-reference)'s one operand is decided by its file **name** alone, never by parsing the document for a discriminating key and never by `path.is_dir()` succeeding: `run.yaml` is a run, `study.yaml` is a bundle, and any other name — a directory included, since `report`'s two forms are two file names and a directory would make "which did you mean" a question core answers by guessing — is refused | `E-REPORT-FORM` |
| A resolved [`BaseReport`](#a-report-override-renders-one-experiments-own-figures) subclass declares no `format`, or a value other than `"markdown"`/`"html"`. Refused at render rather than silently defaulted: `format` has no base default, because a base default would make "declared" and "omitted" indistinguishable — the same reason `BaseTemplate.aggregate` has no base implementation returning `{}`. No override at all (`report_cls is None`) is not this row's case and is not refused: core renders its own four standard sections as markdown, since there is no class there for "declared" and "omitted" to disagree about | `E-REPORT-FORMAT` |
| [Override discovery](#a-report-override-renders-one-experiments-own-figures) imports the run's own `<root_pkg>.report` and finds it defines no `BaseReport` subclass, or more than one. "More than one" is refused rather than resolved by definition order — order is exactly the proxy this discovery forbids, and a project has one report | `E-REPORT-OVERRIDE-CLASS` |
| [Override discovery](#a-report-override-renders-one-experiments-own-figures) reads the run record's own `config.entrypoint` to find this experiment's root package, and it is absent, empty, not a string, or not shaped `<module>:<attribute>` — a hand-edited record's shape, distinguishable from `E-ENTRYPOINT-REQUIRED`'s by *when* each is checked: `E-ENTRYPOINT-REQUIRED` gates a config at its own `validate`/`run`, where this one is discovery independently re-reading a finished record, never calling `base_experiment.load_experiment` and so never inheriting `E-ENTRYPOINT-IMPORT` either | `E-REPORT-OVERRIDE-ENTRYPOINT` |
| `<root_pkg>.report` exists and raises while importing — distinguished from no such module at all by the import machinery's own answer, `ModuleNotFoundError.name` naming exactly the module [override discovery](#a-report-override-renders-one-experiments-own-figures) tried to import, never by catching every exception alike. `report`'s own discovery re-implements `base_experiment.load_experiment`'s `sys.path` window rather than calling it — it needs `<root_pkg>.report`, not the entrypoint's own attribute — so this is a code of its own rather than sharing `E-ENTRYPOINT-IMPORT`'s | `E-REPORT-OVERRIDE-IMPORT` |
| The resolved [`BaseReport`](#a-report-override-renders-one-experiments-own-figures) subclass's own `sections()` body raises while RENDERING — distinguished from the discovery faults (finding and importing the class, before the resolved class's own code ever runs) by *when* it happens, not by where it sits in this table. Not a code of its own when the raise is already a `ContractError` — its own code travels instead, on `freeze`'s "the plugin's own code, or a code of last resort" precedent | `E-REPORT-OVERRIDE-RAISED` |
| `report <run.yaml>`'s [override discovery](#a-report-override-renders-one-experiments-own-figures) reads `environment/repo_root.txt` from the run directory, and it is missing, empty, or names something that is not a directory. Not a walk-up from the argument: `output_dir` may never resolve inside the git repo, so `provenance.find_repo_root` from there answers a different question and raises `E-GIT-NO-REPO` on a correctly configured project rather than answering it — `environment/repo_root.txt` is the fact, the same one H8b introduced for `freeze`'s identical problem, and `provenance.git.repo_root` is not read here: it is the same value recorded at run end, `study add` redacts it out of a bundle member, and two sources for one fact is how the two drift | `E-REPORT-OVERRIDE-REPO` |
| A run record [`read_record_file`](#errors-core-raises) already parsed clean — a real `run_id`, a schema version this build reads — is still missing or malformed at `execution`, `results.conditions`, or `config.data.input_dir`: the three facts `report` needs to build the read-only artifact accessor an override receives. Distinct from the shipped `E-UPSTREAM-RECORD-*` family, which is `read_record_file`'s OWN checks (existence, parseability, schema version) rather than a fact a caller needed downstream of a clean parse. Also raised over a [bundle](#building-one) member, from the identical shortfall, once that member's own record has parsed clean | `E-REPORT-RECORD-INCOMPLETE` |
| A [`study.yaml`](#building-one) is absent, not valid YAML, does not parse to a mapping, or its `runs` key (or one `runs.<name>` entry) is not a mapping — or, at `report`, a `runs.<name>.file` is absent, not a string, or resolves outside the bundle directory or to something not there. Two callers: `report <study.yaml>` reading the whole bundle document (including every member's `file` reference), and `study add` reading the bundle it is about to add a run into (`_load_study_doc`) before checking anything else — refusing to prompt "run `study new` first" when there is no `study.yaml` there at all. A member's `file` that IS in the bundle but corrupt once read is a distinct, adjacent fault under the shipped `E-UPSTREAM-RECORD-*` family instead — this code is for the bundle DOCUMENT'S own shape, never for a member's contents | `E-STUDY-UNREADABLE` |
| [`study new`](#building-one)'s or [`study add`](#what-study-add-redacts)'s bundle path resolves inside a git repository — a study is a publication artifact, sibling to the paper, and belongs beside the manuscript rather than inside the code repo it cites (§ Why not in the repo). Checked at both commands, not only at creation: a bundle assembled outside a repo and later enclosed by one (a `git init` above it, or a move) must not become writable again just because `study new` already ran. Implemented as the walk-up `provenance.find_repo_root` performed over the **bundle path itself** — the same walk-up `input_dir`/`output_dir` already use, applied here to what each command was given — **succeeding**: `find_repo_root` raises `E-GIT-NO-REPO` rather than returning `None` when none is found, so that one code is caught as the pass branch and every other `ContractError` it might raise propagates | `E-STUDY-IN-REPO` |
| [`data.units.from.resolver`](#where-units-come-from) names a resolver that no installed distribution registers under the `publishable.resolvers` entry-point group. Answered from package **metadata**, so a name that is absent costs no import at all — [§ Creating a plugin](#creating-a-plugin-publishable-plugin-new) makes that the whole argument for entry points, and a check that reached for the object behind the name would have changed the guarantee whatever it returned. The message names every group member it did find, the way an unresolved template names the templates it knows, because the ordinary cause is a spelling and the ordinary remedy is reading the list | `E-RESOLVER-UNKNOWN` |
| [`data.units.measurements.by`](#what-isnt-a-repeat) names a field, and the resolver the roster came from yields no attribute of that name to collapse on. A table source reports the same fault against its columns; a resolver has no columns beyond the attributes it declares, so the field a CSV would simply have carried has to be yielded, and this is where that obligation is checked. Separate from [`E-UNITS-ATTR-MISSING`](#errors-validate-reports), which reports the same field missing from a **table** source's columns: the two name different declarations, and a reader fixing one is not fixing the other | `E-RESOLVER-MEASUREMENT-FIELD` |
| A resolver reads a parameter the [sweep](#expansion-modes) varies. The unit table is one table for the whole run, so conditions that resolved different units could not be paired and `n` would mean something different in each — [§ Where units come from](#where-units-come-from) states the rule. Its own code rather than the [`E-STEP-SWEPT-PARAM`](#errors-core-raises) the read itself raises: that identifier is a step's, reached at run time from `"run"` or `"summary"` scope, and a reader holding it is sent to a section describing a different fault at a different time. Sharing the mechanism — a sentinel substituted for a swept path, raising on the read — is not sharing the fault, the same way a coded `ContractError` from a local template's top level is reported as `E-TEMPLATE-LOAD` rather than under the code it carried. An [apparatus probe](#the-apparatus-core-can-only-observe) carries no such restriction and usually does read a swept parameter. **Dual-surface**: a resolver is nondeterministic user code that may read the parameter only on a later call, so every command that enters phases 1-5 — `run`, `draft` and [`dry-run`](#before-you-spend-it) — can meet this same raise, through `cli._prepare_run`, and reports it there too, through the same redacting collector `E-RESOLVER-RAISED` uses | `E-RESOLVER-SWEPT-PARAM` |
| A [`data.units.from.resolver`](#where-units-come-from) source yields something that is not a `Unit`, **or a `Unit` whose attribute value `resolve_units` cannot coerce to a scalar** (a list, a dict, anything else [`coerce_scalars`](#steps-and-artifacts) refuses) — the same fault in a second shape, on the argument that a `Unit` core cannot build a roster row from is the identical problem whichever half of it is wrong. A resolver is the second place user code runs inside resolution, and `validate` is contracted never to raise, so a yielded mapping reaching `u.key` as an `AttributeError`, or a structural attribute value reaching `finalize`'s write uncoerced, would both escape it the same way. The non-`Unit` half is raised in `units._from_resolver`, the one place a yielded item is checked; the attribute-value half is raised at the end of `units.resolve_units`, over every resolved unit's attribute mapping, after the source, after collapsing measurements, and after the roster's own uniqueness check — a table source hands every value through `csv.DictReader` as a `str` and a glob yields no attributes at all, so only a resolver can produce this half even though the check itself runs over every source. Both [reported by `validate`](#errors-core-raises) under the same code, reaching it through `_check_units`'s `except ContractError` the same way `E-DATA-CLUSTER-VARIES` does | `E-RESOLVER-YIELD` |
| A resolver's own body raises something that is not a `ContractError` — a plugin resolver is user code, and `validate` is contracted never to raise, so `_check_units` contains it under a coded refusal of its own rather than letting an arbitrary exception escape. `cli._prepare_run` — the phases `run`, `draft` and [`dry-run`](#before-you-spend-it) all enter — contains the identical raise at each of those three commands, through a fresh, redacting [`Collector`](#secrets--credentials) rather than letting it end the command in an un-redacted traceback — the one output no redacting surface sees. **Except `KeyboardInterrupt`**, which is re-raised rather than turned into a diagnostic, so Ctrl-C still stops the command; a fresh, argument-less `KeyboardInterrupt` is raised in its place so a resolver body that constructed one carrying a credential cannot reach the traceback with its message intact. See [§ Secrets & credentials](#secrets--credentials) | `E-RESOLVER-RAISED` |
...
| `sweep.baseline` fixes a level of a [`sweep.groups`](#expansion-modes) axis and `sweep.ablate` is not declared — § Expansion modes' *the arms of a group axis are peers*. `_baseline_cells` reads a fixed axis as fixed, so it expands over nothing while `_axes` still emits that level as a product row: the level is rendered **twice**, once as the baseline row and once as its own axis's, and the two conditions hold the same units and the same parameters, with directories identical at every artifact — [*two identical measurements reported as two arms*](experimental-designs.md#mistakes-core-prevents). Where the axis declares two or more levels, the other levels' rows cross the single baseline, a comparison that computes rather than being refused; at one level there is no cross-arm comparison at all. The route a control arm actually wants, once there are two of them, is a [`statistics.contrasts`](#contrasts-claims-that-arent-condition-vs-baseline) entry naming both arms, which core computes — the refusal here is the peers rule, not the comparison's availability. Where `ablate` is declared the sibling code `E-SWEEP-ABLATE-BASELINE-GROUP` reports instead, never both | `E-SWEEP-BASELINE-GROUP` |
...
| `experiment_type` names a template that only an installed distribution's `publishable.templates` entry point registers. Core resolves such a name from package metadata and never imports the package, so there is no class behind it to materialize parameters from, enforce a `parameter_spec` against, or call `validate`/`aggregate` on — and **this is what the project ships, not a stage it passes through**: nothing is chartered that would load one. The route is the second of the three homes [§ Templates](#templates-where-parameters-are-defined) names — keep the template in your repository's `templates/`, where path discovery finds it and [`code_hash`](#three-hashes) covers it, and let the plugin carry the machinery, whose four other registries all dispatch from an install. Three surfaces meet this condition and one shared message governs all three: `validate` reports it as a finding, and [`generate experiment`](#creation-commands) and [`freeze`](#cli-reference) each raise it as a `ContractError`. Decided by the same branch as `E-TEMPLATE-UNKNOWN` and returning at the same site, so a config earns one of the two and never both. The cost of the route is real and is the reason this row says it out loud: a template inside `code_hash` is pinned by each consuming repository rather than by `uv.lock`, so two repositories sharing an experiment type keep two copies and share no template identity | `E-TEMPLATE-INSTALLED-UNSUPPORTED` |
...
| A `parameter_spec` path is not `head.leaf` — not exactly two dotted segments — declared by a template class, core's own or a repo's [`templates/`](#templates-where-parameters-are-defined). [§ Templates](#templates-where-parameters-are-defined) states the constraint: a path is not a `Param` constraint, so `Param.__init__` cannot check it, and it is checked instead the moment the class defining it loads, in `BaseTemplate.__init_subclass__` — before `@register_template` or anything else ever sees the class. For a project-local template this raise happens inside the same import `E-TEMPLATE-LOAD` already covers, so it folds into that code's "raises while importing" shape exactly as a `Param` declaring `default=None` without `nullable=True` already does — but, unlike that one, the message names this code by itself, so it is still legible nested inside `E-TEMPLATE-LOAD`'s. The remedy is one word: rename — a template declaring `"reference_frame"` fixed this by renaming it to `"frame.reference"` | `E-TEMPLATE-PARAM-PATH` |
...
| `experiment_type` is missing, empty, or names a template that neither core, nor any installed distribution's `publishable.templates` entry points, nor this project's own [`templates/`](#templates-where-parameters-are-defined) registers — the installed set read from package metadata, so a name no distribution declares is refused without importing one. A name an installed distribution **does** register is `E-TEMPLATE-INSTALLED-UNSUPPORTED`'s row above, not this one: that name resolves, and what is missing is the class behind it rather than the claim. When the config declares a [`plugin`](#the-one-config-file), the message names it: that field is a readable note about where the template was expected to come from, so a reader who has not installed it learns that from the diagnostic rather than from a missing-name list. Three surfaces meet this condition — `validate` reports it as a finding, never raising it, and [`generate experiment`](#creation-commands) and [`freeze`](#cli-reference) each raise it as a `ContractError` — and this row governs all three, built from one shared message. The hint naming a declared `plugin` appears at `validate` and `freeze`, both reading the field off an existing config; `generate` never has one to read, since it is the command that writes the file the field would live in | `E-TEMPLATE-UNKNOWN` |
| `data.units.attributes` names a value the source table has no column for, or names any value at all under a `{glob: ...}` source, which yields a key and a path and nothing else, or a value no unit a [resolver](#where-units-come-from) yielded carries — reported for the first such name, and after `E-UNITS-ATTR-RESERVED` under a table, glob, or resolver source; or — for a table source only — names a non-string item. Reported for `data.units.measurements.by` too, when it names no column of the source *and* rows sharing a key were collapsed anyway: the collapse groups on the key alone, so such a `by` averages rows nothing distinguished as measurements of one unit. The source's columns, not `data.units.attributes` — `design-principles.md` § Core vs. plugin lists `measurements.by` beside `attributes` as a parallel namer of an input field, and the fence in [§ What isn't a repeat](#what-isnt-a-repeat) declares no attributes at all. Only when rows were collapsed — the same `by` over an input holding one row per unit names a measurement a [step](#what-isnt-a-repeat) supplies, which no input column carries. **Dual-surface for a resolver source**: a resolver is nondeterministic user code that may yield different attributes on a later call, so every command that enters phases 1-5 — `run`, `draft` and [`dry-run`](#before-you-spend-it) — can meet this same raise, through `cli._prepare_run`, and reports it there too, through the same redacting collector `E-RESOLVER-RAISED` uses | `E-UNITS-ATTR-MISSING` |
...
| `data.units.attributes` names `unit`, `measurement`, or `by` — each already a column of [a per-unit table](#the-per-unit-tables) or the block [a stratified report](#reporting-strata) keys its rows by, so a declared attribute of that name would silently occupy a column that already means something rather than being read back as an attribute. A second code rather than a wider `E-UNITS-ATTR-RESERVED` row above, because the two faults differ on **lifetime**: that code's three names belong to `Unit` itself and can never be freed, where these three belong to a column or a stratified block and could be freed by a later rename — the same principle [`E-APPARATUS-FACT-TYPE`](#errors-core-raises)'s own row states for a code sharing `coerce_scalars` with `E-STEP-RETURN-TYPE` without sharing its fault. **One emit path, not two surfaces**: the same roster resolution `E-UNITS-ATTR-RESERVED` reads is what `cli._prepare_run` checks through `validate_config`, before its own `resolve_units` call, so `run`, `draft` and [`dry-run`](#before-you-spend-it) meet this refusal through `validate`'s gate for every source — table, glob, or resolver alike — never through a second, dual-surface check of its own | `E-UNITS-ATTR-COLUMN` |
...
| The table `data.units.from` names has no data rows, or the `glob` it names matches no files under `input_dir`, or a [resolver](#where-units-come-from) yields no units. **Dual-surface for a resolver source**: a resolver is nondeterministic user code that may yield nothing on a later call, so every command that enters phases 1-5 — `run`, `draft` and [`dry-run`](#before-you-spend-it) — can meet this same raise, through `cli._prepare_run`, and reports it there too, through the same redacting collector `E-RESOLVER-RAISED` uses | `E-UNITS-EMPTY` |
| Two resolved units share the same `data.units.key` value. **Dual-surface for a resolver source**: a resolver is nondeterministic user code that may yield a duplicate key only on a later call, so every command that enters phases 1-5 — `run`, `draft` and [`dry-run`](#before-you-spend-it) — can meet this same raise, through `cli._prepare_run`, and reports it there too, through the same redacting collector `E-RESOLVER-RAISED` uses | `E-UNITS-KEY-DUPLICATE` |
...
| A table `data.units.from` names, or a file a `{glob: ...}` pattern matches, exists but cannot be read as one — a CSV that is not valid UTF-8, or an absolute `glob` pattern, raising `NotImplementedError` from `Path.glob` rather than a `ContractError`. Recoded in `resolve_units`, at the one call site over both sources, into an identifier of its own rather than `E-RESOLVER-RAISED`: that code's own row is a **resolver's** raise, and neither source declares one. **Dual-surface**: `cli._prepare_run` reports the identical raise at `run`, `draft` and [`dry-run`](#before-you-spend-it), through the same redacting collector `E-RESOLVER-RAISED` uses | `E-UNITS-SOURCE-UNREADABLE` |
...
| At [`freeze`](#cli-reference), the run directory already holds a `run.yaml` — that run ended, and its record is never modified once written. Read the record; there is nothing to freeze | `E-FREEZE-RUN-ENDED` |
| At `freeze`, the run directory is missing `config.yaml`, `config.yaml` does not parse to a mapping, `environment/repo_root.txt` is missing or empty, or `environment/repo_root.txt` names a path that is not a directory — the run was started by a build before these artifacts existed, or the directory was edited by hand; it cannot be frozen | `E-FREEZE-NO-CONFIG` |
| At `freeze`, the template `experiment_type` resolves to declares no [`apparatus_probe`](#the-apparatus-core-can-only-observe) — nothing to re-probe, since this experiment does not measure through an apparatus | `E-FREEZE-NO-APPARATUS` |
| At `freeze`, the run directory holds no readable `sweep.yaml` — the file is absent, its YAML does not parse, or it holds no `conditions` list — because the run died before its plan was written, or the directory was edited | `E-FREEZE-PLAN-MISSING` |
| At `freeze`, re-expanding the copied config disagrees with `sweep.yaml`'s own recorded plan on some condition's `index`, `label`, `values`, or `is_baseline` — checked over the full four-tuple in recorded order, because `values` is what determines the cfg a probe is called under and a two-field check could miss it moving under `ablate` or a declared `baseline`. The run directory or the config copy was edited; do not trust either | `E-FREEZE-PLAN-MISMATCH` |
| At `freeze`, a probe is declared but `apparatus/probes.jsonl` holds no `run_start`/`pre_execution` line to replay — the run has not probed yet, so there is no baseline, and probing now would pin a fact the run's own gate never adopted. A ledger file that is simply absent is this code too, since it replays to the same empty baseline | `E-FREEZE-LEDGER-MISSING` |
| A line in `apparatus/probes.jsonl` is not valid JSON, is not a JSON object, is missing `phase`, `condition`, or `facts`, or holds `facts`/`condition` of the wrong type — raised in [`apparatus.replay_ledger`](#the-apparatus-core-can-only-observe), which `freeze` calls before probing. The file was edited or truncated by hand | `E-FREEZE-LEDGER-UNREADABLE` |
| At `freeze`, the resolved template now declares a different `apparatus_probe` than the ledger's own `run_start`/`pre_execution` lines record — `templates/**` was edited mid-run; check out the tree the run started from | `E-FREEZE-PROBE-MISMATCH` |
| At [`resume`](#resuming), the run directory holds no readable `identity.json` — the file is absent, will not parse, parses to something other than an object, or is missing any of its five keys — so the [three identity claims](#how-the-three-are-computed) this run started under cannot be compared. One code for all four faults, because the remedy is the same for each: run again into a fresh run directory. A directory written by a build that predates the artifact and one edited by hand are indistinguishable from here, and the message says so rather than guessing which happened. There is deliberately no fallback to the run id's own 7-hex `code_hash` prefix: it says nothing about `parameters` or the lockfile, so a comparison covering one figure of three would read as three | `E-RESUME-NO-IDENTITY` |
| At `resume`, the config the run executed cannot be found: `environment/repo_root.txt` is absent, empty, or names something that is not a directory; or `identity.json`'s recorded `config_path` is not a non-empty relative string, does not resolve to a file, or resolves **outside** that root. Two readers, one code, because both halves answer one question — *can the config this run executed be found?* — and share one remedy, restoring the directory. The containment check is containment **only**: a forward separator is legal (a recorded path is a relative path with components), while an absolute recorded path and a `..` escape are refused by where the value resolves rather than by inspecting the string. It is not a boundary and is not documented as one — a step can `open()` anything regardless — what it buys is that an edited record fails loudly instead of quietly validating some other file | `E-RESUME-NO-CONFIG` |
| At `resume`, the run directory already holds a `run.yaml` — that run [ended](#what-status-means-and-when-a-run-keeps-going), and a run record is never modified once written. Start a new run instead. Also what a resume that stopped on a [changed apparatus fact](#the-apparatus-core-can-only-observe) meets on a second attempt: that stop publishes the record of what the run did, which ends the run | `E-RESUME-RUN-ENDED` |
| At `resume`, the `code_hash` this tree computes differs from the one `identity.json` recorded — `src/**` or `templates/**` moved while the run was down. The run cannot be continued under different code; start a new run | `E-RESUME-CODE-MOVED` |
| At `resume`, the recomputed `parameters_hash` differs from the recorded one — the project's config was edited while the run was down. Note which config: `resume` re-reads the **project's** file, never the run directory's copy, so an edit to the copy is [`freeze`'s](#cli-reference) `E-FREEZE-CONFIG-EDITED` below and not this | `E-RESUME-PARAMS-MOVED` |
| At `resume`, the recomputed `uv_lock_hash` differs from the recorded one, **in either direction and `null` included** — a lockfile added, removed, or changed while the run was down is a different environment either way | `E-RESUME-LOCKFILE-MOVED` |
| At `resume`, the inputs moved: `manifest/input.json` is absent or unreadable, so what the run read cannot be compared against what is there now; or a fresh manifest of `input_dir` does not hash to what that file recorded. One code, two faults, one remedy — restore the inputs the run started from, or start a new run. **The comparison is [content-addressed](#how-the-three-are-computed) where the policy hashed the file**, so a bare `touch` between the crash and the resume no longer trips it — restoring bytes is enough, and restamping a timestamp is not a reason to discard a crashed run. The comparison is against the **recorded** manifest, and the recorded manifest is what travels into the rest of the run, so `run.yaml`'s `input_manifest_hash` stays the first attempt's figure | `E-RESUME-INPUT-MOVED` |
| At `resume`, the run directory holds no readable `sweep.yaml`: the file is absent (the run died before its plan was written), its YAML does not parse, it holds no `conditions` list, its `order` is not the recorded mode string, or an `execution_order` entry is not a recorded `(condition, repeat)` pair. The plan is read rather than re-derived, so a file that cannot be read as a plan is a refusal and never a re-realization | `E-RESUME-PLAN-MISSING` |
| At `resume`, the recorded plan and this config disagree. Two sites, one code: a condition's recorded entry is not a mapping, its recorded `values` is not a mapping, the recorded and re-expanded condition counts differ, or some condition's `index`, `label`, `values` or `is_baseline` differs — checked over the full four-tuple in recorded order, for the same reason [`freeze`'s](#cli-reference) `E-FREEZE-PLAN-MISMATCH` is; **or** the recorded `execution_order` does not name exactly the `(condition, repeat)` pairs this config plans, once, each. The second arm exists because that order is an input on a resume rather than something core just realized: without it an edited `execution_order` reached `E-RUN-ORDER-MISMATCH` in [§ Errors core raises](#errors-core-raises), a code documented as core's own resolved state disagreeing with itself, from inside the lock | `E-RESUME-PLAN-MISMATCH` |
| At `resume`, a line of `executions.jsonl` cannot be read as a record of an execution: it is not valid JSON, it parses to something other than an object, or it is missing any of `step`, `scope`, `condition`, `repeat`, `status`; **or** a line recording a **completed** execution holds no `returned` or no `recorded_columns` key. Four faults, one code, one remedy — the ledger was truncated, hand-edited, or written by a build older than those two keys. A reader that skipped such a line would report that triple as *never ran*, so a resume would re-execute work already paid for, or reconstitute nothing for it and publish intervals over the remainder. The first three are the ledger reader's, whose required set is those five keys and does not include `returned`/`recorded_columns`; the fourth is checked where those two are needed instead, at the reconstitution of a triple `resume` skips, because the alternative is a `per_repeat` hole and a unit table narrowed to `unit` alone | `E-RESUME-LEDGER-UNREADABLE` |
| At `resume`, a completed triple's `units.parquet` will not decode, or it does not hold every column that triple's own ledger line names in `recorded_columns`. The rows of a skipped execution are what the aggregate phase reads, so a file that cannot supply them cannot be silently treated as empty | `E-RESUME-ROWS-UNREADABLE` |
| At `resume`, a completed triple's ledger line names a non-empty `recorded_columns` and no `units.parquet` exists beside it. Distinct from `E-RESUME-ROWS-UNREADABLE`, which is a file that exists and cannot supply the named columns, and from *no rows at all*: a step that recorded nothing writes no file and records an empty list, which is legitimate and resumes | `E-RESUME-ROWS-MISSING` |
| At `resume`, `allocation.json` cannot be applied to what this tree resolved: the file will not parse, is not an allocation document, has an `arms` block that is not a mapping, records an individual axis as something other than a mapping of level to members, records axes or levels this config does not resolve, names a membership that is not exactly the roster's keys in both directions, disagrees about whether a holdout exists, or records a holdout that is not a partition of the roster. One code for every arm, because the remedy is one — this record describes a different roster, so the run cannot be continued. The file is **read rather than re-drawn**: under a drawn method a second derivation is a second draw | `E-RESUME-ALLOCATION-STALE` |
| At `resume`, `apparatus/probes.jsonl` cannot be replayed — the same faults [`apparatus.replay_ledger`](#the-apparatus-core-can-only-observe) refuses for `freeze` under `E-FREEZE-LEDGER-UNREADABLE`, reported under `resume`'s own code because a `FREEZE` identifier printed by a command that is not `freeze` is a lie about which command found the fault. An **absent or empty** ledger is not this fault and mints no refusal: a run that crashed before its first probe is entitled to set a baseline exactly as its own first probe would have | `E-RESUME-PROBES-UNREADABLE` |
| At [`freeze`](#cli-reference), the run directory's own `config.yaml` copy hashes to a different `parameters_hash` than `identity.json` recorded — the copy was edited after the run started, so a probe now would measure under parameters the run never adopted. **An absent `identity.json` is not this fault**, and neither is one that will not parse or holds no `parameters_hash`: a run directory started by a build that predates the artifact has nothing to compare, and `freeze` behaves exactly as it does without this check. Distinct from `E-RESUME-PARAMS-MOVED` above, which is about the **project's** config: `resume` never reads this copy, so neither command's comparison closes the other's | `E-FREEZE-CONFIG-EDITED` |
| A [`diff`](#cli-reference) operand read as a config — any path that is not a directory and is not named `run.yaml` — does not parse as YAML, or parses to something other than a mapping. If it names a run, pass its directory or its `run.yaml` instead | `E-DIFF-CONFIG-UNREADABLE` |
| At [`reproduce`](#reproducing-on-another-device), the operand is not something the command can act on. **Four sites, one code**, because the remedy is one sentence in every one of them — a run record file or a config file: the path is a *directory* (`resume` is the command that takes one, and giving `reproduce` the same operand for the opposite action is worth refusing by name rather than by fall-through); the file parses to something that is not a mapping; it is a mapping carrying a run record's `provenance`/`results` but **no `run_id`**, so it is an edited or truncated record rather than a config, which is a genuinely different reading and gets its own branch; or it is a mapping that is neither. The discrimination is structural throughout and never by basename — a bundle member is `main.run.yaml`, so a name test would refuse the very form [`study`](#building-one) exists to serve | `E-REPRODUCE-OPERAND` |
| At `reproduce`, the operand is a whole [`study.yaml`](#building-one) rather than one of its members. The message **lists the member names** — a bundle is a mapping under `runs`, and naming them is what makes this refusal one step from the command you meant | `E-REPRODUCE-BUNDLE` |
| At `reproduce`, `provenance.git.remote` is `null`, so there is nothing to clone from. This is the scaffolded default rather than an edge case: a project that never added a remote records `null` and runs to exit `0`. The message names the recorded `git.commit`, so a reader holding the repository already can check it out themselves. **Exit [`1`](#exit-codes-and-diagnostics), not `5`** — `5` is a clone that was *attempted* and failed, and keeping this at `1` is what preserves that reading | `E-REPRODUCE-NO-REMOTE` |
| At `reproduce`, the derived destination already exists. The destination is derived from the remote's last component and the `run_id`, so it does not need naming — and a second `reproduce` of one record derives the same name, which is why this refusal exists rather than an overwrite of a checkout you may already have edited | `E-REPRODUCE-DEST-EXISTS` |
| At `reproduce`, the derived destination resolves inside a git repository — the same walk-up [`input_dir`/`output_dir`](#the-one-config-file) use, performed from the destination's parent rather than from the operand, since it is the destination that must not nest. Nothing is created before this refusal | `E-REPRODUCE-DEST-IN-REPO` |
| At `reproduce`, the clone succeeded and the recorded commit is not in it, so the detached checkout fails — which is what a rewritten or force-pushed history actually leaves behind, caught here rather than by a hash. The message names the recorded commit and says the remote does not hold it. **Exit [`5`](#exit-codes-and-diagnostics)**: git was reached and answered, which is the retry class | `E-REPRODUCE-COMMIT-UNREACHABLE` |
| At `reproduce`, the checked-out tree's `code_hash` differs from the record's — recomputed with **the run command's own predicate**, git-ignore-aware, over `src/**` and `templates/**`. The message names both digests and the file count; the transcript beside it lists the files folded and then the [closed candidate set](#reproducing-on-another-device) of causes this command cannot separate, never one cause. **The checkout is kept.** A [`draft`](#draft-runs) record declines the verification instead and draws nothing here | `E-REPRODUCE-CODE-HASH` |
| At `reproduce`, `provenance.environment.uv_lock_hash` is `null` — the run's environment was never pinned, which is [`W-ENV-UNLOCKED`](#warnings-core-reports)'s own sentence coming true one command later. The checkout is kept, and the transcript says the sync was not run rather than passing over it | `E-REPRODUCE-UNLOCKED` |
| At `reproduce`, the run directory's own `environment/uv.lock` is reachable beside the operand and its sha256 does not match the recorded `uv_lock_hash` — the copy was edited after the run, so it is not the environment those numbers came through. The message names both digests, and the clone's own lockfile is left untouched rather than being used as a fallback: a byte copy that fails its own digest check tells you nothing about which of the two is right | `E-REPRODUCE-LOCKFILE-EDITED` |
| At `reproduce`, no lockfile matching the recorded `uv_lock_hash` is reachable at all. Reached from the [bundle](#building-one) form, which carries no `environment/` directory — so only the recorded digest travels — when the commit's own committed lockfile is absent or hashes differently. The message names the recorded digest and what the checkout actually holds | `E-REPRODUCE-LOCKFILE-UNREACHABLE` |
| At `reproduce`, the config written back into `configs/<name>/config.yaml` does not hash to the record's `parameters_hash`. [`parameters_hash`](#three-hashes) covers neither `metadata` nor the two data paths, so the check is blind to the blanking this write performs and sensitive to a re-serialization that dropped or retyped a key — which is the whole reason it is made. The message names both figures | `E-REPRODUCE-CONFIG-WRITEBACK` |
| At `reproduce`, `configs/<name>/apparatus.expected.json` already exists in the checkout. The file is written once and never rewritten, because it is a file you may edit and replacing it would discard a decision core cannot make for you. The reachable route is a **committed** expectation file — `configs/` sits outside the [hashed trees](#three-hashes), so committing one moves no `code_hash` — and not a second `reproduce` into the same destination, which the destination refusal above stops first | `E-REPRODUCE-EXPECTED-EXISTS` |
...
[`reproduce`](#reproducing-on-another-device) accepts either — a `run.yaml`, and it prepares a checkout of exactly what that run ran; or a config, and it prepares the repository that config sits in. It **stops** either way rather than running.

A run directory also holds a **byte copy** of the config it was started from, written at run start and never modified — see [The other files a run writes](#the-other-files-a-run-writes), which is the section that enumerates everything a run directory holds. Its purpose is to let a mid-run command (`freeze`, `resume`) reach the config a run is executing under *before* `run.yaml` exists. It is not a third thing to edit — editing it does not change the run — and this section is not renamed on its account: "the two files" counts *roles*, what you edit and what you report, not every file inside a run directory.
...
case is `upstream: []`, present and empty rather than omitted, for a run whose steps call
[`io.reuse_from`](#lineage-between-runs) not at all; see that section for the two locator forms and
every refusal along the way.
...
**`failed` means there is nothing to report**, which is why it isn't simply "the plan didn't finish." Three things produce it. A `scope: "run"` step that raises takes every condition with it — there is no shared cohort for them to condition on, so continuing would mean executing a plan whose first premise is missing. The [input manifest failing its re-verification](#steps-and-artifacts) after the last execution fails a run that otherwise reached the end of its plan — the inputs moved underneath it, so every number in it is over a dataset that no longer exists, and there is no honest way to report that as `partial`. And so does the [apparatus](#the-apparatus-core-can-only-observe) itself moving — a changed fact, not merely one that stopped answering — for the identical reason: the executions on either side of the change were measured through two different apparatus states, so nothing in the record is one dataset, and the record is kept anyway rather than discarded.

**The first of those three was prose only until 2026-09-19, and a run in the field is what found it.** A `scope: "run"` step that raised marked its own execution `failed` and the plan **carried on** — every condition executing on the missing premise this sentence describes. It was measured in a study whose pre-sweep apparatus probe raised in 0.007 seconds and whose remaining executions all ran unguarded, and it had been held in place by three of core's own tests asserting `partial` where this section says `failed`. The sentence did not move; the code did. `summary` scope is unaffected, and a repeat-scoped raise still does not stop a plan — that is `limits.max_failed_fraction`'s territory and the distinction the paragraph below turns on.

**This paragraph said `limits.max_failed_fraction` was a fourth, and the code has never agreed.** It stops the run where it stands — unit failures only accumulate, so once the fraction is past the threshold no later execution can bring it back, and spending the remaining compute to confirm that is waste — but what it produces is **`partial`**, not `failed`, and that is the right answer rather than a defect to repair in the other direction: a run whose guard fires with thirty-three executions already recorded has a great deal to report, and `failed` means there is nothing. The stop reason is threaded to `run_status` and deliberately left out of its mapping, which is the code saying exactly this. **Corrected 2026-09-14**, after a run stopped by that guard was read against this sentence and the sentence lost.

**A run that stops early can still be `partial`**, and two things produce that. `limits.max_failed_fraction` firing is one, for the reason just given. The other is the [apparatus](#the-apparatus-core-can-only-observe) becoming unreachable — not the apparatus generally, and not a fact it observed changing, which fails the run. A probe that stops responding altogether — the service down, a credential expired mid-run, which is a different thing from [a fact it returns unanswered](#the-apparatus-core-can-only-observe) — leaves the gate with nothing to compare before the next execution, so the run stops there rather than executing uncertified. Everything up to that point ran under recorded facts, so there is a record worth reading and `failed`'s "nothing to report" doesn't fit it. The exit code is nevertheless [`5`](#exit-codes-and-diagnostics) rather than `3`, per the precedence stated there.
...
**A `partial` run says whether its plan ended.** A plan that stopped early still owes executions, and the record names them: a `truncated` block carries the `reason` core stopped for, how many of the plan were `planned` and `attempted`, and the `outstanding` triples that were never tried. It is absent — never `null` — when the plan reached its end, on `findings`' own rule that an empty disclosure claims a question was asked. Without it a reader met `status: partial` and could not tell a plan that finished with one failure from one that stopped with eleven executions never attempted, and neither could [`resume`](#resuming).
...
A `fold` level adds `partitions` — the unit keys in each fold's train and test side, and the realized fold sizes when [`cluster_by`](#clustered-units) makes them uneven. When the design has cells — [`sweep.groups`](#expansion-modes) axes, or `allocation: between` — the partitions are drawn inside each cell and `partitions_within` names those axes, written only then: each entry's `train` is still every other partition of the whole roster, so a reader recovers the side a step was actually handed by crossing that entry against [`allocation.json`](#allocationjson--who-went-where)'s arms, every condition being narrowed to its own arm first. A `sample` sweep adds the drawn `values` per condition — they are the conditions' own values, not a second copy — and `sample_seed`, the seed they came from. `order: randomized` adds the `order_seed` its shuffle used, beside the `execution_order` that shuffle produced — the seed so the plan is derivable, the order because [what happened is not a thing to re-derive](#resuming):
...
```json
{
  ...
  "holdout": {"train": ["P0002", "P0007"], "test": ["P0011", "P0019"],
              "seed": 3310985422, "strata": ["label"], "within": ["arm"]},
...
**`within` names the group axes the split was drawn inside**, and it is the block's third own key. A [group axis](#expansion-modes) divides the roster into cells and the holdout is drawn inside each of them, to the same proportion — so `train` and `test` stay **flat lists over the whole roster**, their union still a partition of it, and a reader who wants one arm's own split crosses them against `arms` in this same file. What flat lists cannot say is *which question the split answered*: one draw over the roster, or one per cell. That fact travels beside them rather than reshaping them, the way [`weighted_by`](#weighted-samples) travels beside a value whose meaning it qualifies. The document above declares one axis, so it reads `"within": ["arm"]`; a run with no group axis omits the key entirely rather than writing `[]`, which would claim a per-cell draw over no cells — `seed` and `strata`'s own rule one key over.
...
`units.parquet`'s columns are the unit key, then every declared attribute, then every key any row recorded — the union, with a column absent from a row reading as null. A failed unit has no row anywhere, which is [how core counts one](#what-isnt-a-repeat): `failed` is what's left after `completed` and `ineligible` are subtracted from `resolved`.

**A column in `units.parquet` unifies across rows by type, not by declaration.** Every value a step *records* already passes through the same [scalar coercion](#steps-and-artifacts) `io.record`'s values take, so a recorded cell always holds `bool`, `int`, `float`, `str`, or `None` by the time a column is built from it. A declared attribute's value is not run through that coercion today — a resolver may yield one that is not a scalar at all, and this section does not close that gap — but the check below runs over every column alike, attributes included, and the rule for what a *column* may hold, across every row that carries it, is:

- a column holding one type round-trips, `str` and `bool` included;
- `int` beside `float` **promotes to `float`**, in either declaration order — a per-unit metric that comes out whole for some units and fractional for others is the ordinary case, not a conflict, and is not worth losing a run's whole record over. **Not stated as absolute:** an `int` above `2**53` (a nanosecond timestamp is the realistic case) beside a `float` in the same column does not promote cleanly — `pyarrow` raises its own `ArrowInvalid`, uncoded, rather than `E-STEP-RETURN-TYPE`, and the one execution that reached it fails rather than the run stopping;
...
- an empty row set writes an empty table and raises nothing — true of the encoder itself, and unreachable for either per-unit table: `finalize` writes `units.parquet`/`measurements.parquet` only when there is at least one row to put in it, so this clause describes `.parquet`'s general behavior, never something a real run's per-unit table does;
...
This is the rule the code already enforces; it was reachable only by colliding with it before now. Where it costs the most is a column that is genuinely `str` for some units and a number for others — refused today, at the cost of the whole execution's record for a step that already ran. **The read and the write are decided separately, and differently.** On the read side, [`aggregate`](#templates-where-parameters-are-defined) publishes a column as a metric when it has numbers to compute one from: a column that is non-numeric for **every** unit publishes no metric block, because there is no mean of strings, and it still reaches `aggregate`'s table where a template that knows what the values mean can read it directly. A column that is a number for some units and absent — `None` — for others publishes a block computed over the units that carried a number, with the count that contributed reported beside it — and [`W-STATS-COLUMN-THIN`](#warnings-core-reports) where that count falls below `limits.min_reported_n` — because a `None` cell says *this unit has no value for this metric* and counting a unit out is what the rest of this section already does with one. A column that is `str` beside a number is not a case: the write side above refuses it, so no read rule needs to describe it. On the write side, `E-STEP-RETURN-TYPE` is **not** loosened: a genuinely mixed `str`-and-number column written across repeats still refuses, because loosening it would make a column's published-or-not status depend on the data a run happened to produce rather than on its config — one run of a config would publish a metric and the next would not, with no diagnostic distinguishing the two.
...
**A run declaring no `sweep` has exactly one condition, and its key is `"00"`** — the same
`<nn>_<label>` scheme with an empty label, rather than the string `"None"` or a literal `null`:
canonical JSON cannot sort a `None` key beside `str` keys under the [hash](#the-apparatus-core-can-only-observe)'s
`sort_keys=True`. Both the ledger's `condition` and `provenance.apparatus.facts`' key use it, so a
single-condition apparatus-declaring run reads `facts: {"00": {…}}` rather than the `00_<label>`
form every sweep-bearing example above shows.
...
└── ArtifactError                  # an artifact-shaped fault — on a write, or on a read
...
**Three rows in this table are not raises, and the `Type` cell says so.** A command can also refuse
through a fresh `Collector` — the mechanism `validate` uses — at a point where no step exists to raise
into and no `validate` pass is running: the dirty gate, the empty-file-list gate and the input-drift check, all inside
the run command's own sequence — since H9a, in `cli._prepare_run`, which `command_run`
calls. They sit here rather than in [§ Errors `validate` reports](#errors-validate-reports)
because **`validate` does not report them** — it neither hashes nor consults the working tree's git
status — and a reader who meets one at `run` looks for it here. Their `Type` reads
*(no exception; a `Collector` diagnostic)* rather than an exception name, which is the whole of the
difference.

| Raised by | Type · code |
|---|---|
| [`io.write`](#steps-and-artifacts) or `io.path` onto a target that exists | `ArtifactExistsError` · `E-ARTIFACT-EXISTS` |
| A `name` that escapes the directory it is checked against — the step's own on a write, or the target step's on [`io.read_condition`](#steps-that-need-every-condition), [`io.read_upstream`](#step-scope), or a [report override's](#a-report-override-renders-one-experiments-own-figures) own `io.read_condition`, which checks the identical target-step containment through the same `_contained` call — an `io.append` onto anything but `.jsonl`, an extension [no writer claims](#steps-and-artifacts) handed an object that isn't `bytes` or `str`, or a row a `.csv`/`.parquet` writer receives that is not a mapping. Four emit sites for the escape alone: a write checks against the calling step's own directory, and each of the three readers checks against the directory of the step named in the call — one row because the check and the remedy are the same containment rule at all four, never a check on the path's shape | `ArtifactError` · `E-ARTIFACT-NAME`, `E-ARTIFACT-APPEND`, `E-ARTIFACT-UNWRITABLE` |
| [Reading](#steps-and-artifacts) a name whose suffix has a writer and no reader — either registered, or claimed by an installed distribution and resolved at the read. A writer and its reader are [registered as a pair](#creating-a-plugin-publishable-plugin-new), through two entry-point groups, because `io.write` dispatches on the writer side and `io.read_upstream` looks up the reader side — an inversion only while the two hold the same keys. That inversion is checked in one direction only: dispatch is decided from the writer side alone, so a reader the writer side does not answer for is invisible to it and reads back as bytes, the same as a suffix neither side knows. A suffix *neither* side knows is not this fault either: that is the raw-bytes case `io.write` already accepts, and it reads back as bytes | `ArtifactError` · `E-ARTIFACT-UNREADABLE` |
| [Reading a step narrower than the caller](#step-scope) | `ContractError` · `E-STEP-READ-DIRECTION` |
| [`io.read_upstream`](#step-scope) naming a step this run does not have. The message lists the steps it does, because the overwhelmingly likely cause is a misspelling and the fix is one of them. Refused rather than resolved: a step whose scope is unknown has no directory to read from, and the alternative was to read the run-scoped one — which is where the name would land if it were run-scoped, and where nothing is if it is not, so the symptom was a `FileNotFoundError` naming a path no step ever wrote to. **The refusal needs a scope map to be sure of**, so it fires only when this execution was given one; a `StepIO` constructed without scopes has no step set to test against and is unchanged | `ContractError` · `E-STEP-READ-UNKNOWN` |
...
| [`io.reuse_from`](#lineage-between-runs)'s locator: neither a bare `run_id` nor an absolute path (a relative path carrying a separator), or a locator of **either** form resolving inside the git repo the command was given — the relative form's path is resolved before that check too, so a symlink under `output_dir` pointing into the repo is refused the same way an absolute path to it is | `ContractError` · `E-UPSTREAM-LOCATOR`, `E-UPSTREAM-REPO-CONTAINED` |
| [`io.reuse_from`](#lineage-between-runs)'s **relative** form resolving to a run directory whose own `run_id` disagrees with the locator as given — most visibly, a locator of `latest`, which only the absolute form may follow | `ContractError` · `E-UPSTREAM-RUNID-MISMATCH` |
| A run record `read_record_file` is pointed at — a run directory's own `run.yaml` (`read_run_record`'s entry) or a bundle member file directly (a bare `<name>.run.yaml`, never `<dir>/run.yaml`) — is absent, present but unparseable or without a `run_id`, or carries a `schema_version` this build doesn't read. One refusal set, two entries: read through [`io.reuse_from`](#lineage-between-runs)'s upstream `run.yaml`, through [`diff`](#cli-reference)'s own run-record operand, through [`report`](#cli-reference)'s run and bundle-member operands, through `study add`'s copy of a run's own `run.yaml`, and through [`reproduce`](#reproducing-on-another-device)'s own operand, which re-reports whichever of the three the reader raised **under its own code** rather than flattening it into `E-REPRODUCE-OPERAND` — all five call the identical reader | `ContractError` · `E-UPSTREAM-RECORD-MISSING`, `E-UPSTREAM-RECORD-UNREADABLE`, `E-UPSTREAM-RECORD-VERSION` |
| The step [`io.reuse_from`](#lineage-between-runs) names: recorded under a condition (no selector exists to address it — republish it from a `summary` step instead), recorded nowhere in the upstream, or recorded but not `completed` | `ContractError` · `E-UPSTREAM-STEP-SCOPED`, `E-UPSTREAM-STEP-UNKNOWN`, `E-UPSTREAM-STEP-INCOMPLETE` |
| [`io.reuse_from`](#lineage-between-runs)'s own `name`, checked for containment the same way any other artifact read is (its own code rather than `E-ARTIFACT-NAME`, on the precedent `E-RESOLVER-SWEPT-PARAM` already sets), or naming an artifact the upstream step never published under that name | `ArtifactError` · `E-UPSTREAM-NAME`, `E-UPSTREAM-ARTIFACT-MISSING` |
...
| A [returned value core can't record](#steps-and-artifacts) — `io.record`'s values, a step's `run` return, a template's `aggregate` return, or a `.csv` row's own cell that isn't a scalar core can coerce to one — a written `.parquet` row set whose rows [disagree on a column's type once coerced](#the-per-unit-tables), checked only for `.parquet` since a `.csv` write does not unify a column's type across rows today — or any [name collision the record can't hold](#validation): a derived key against a recorded column, a derived key taking the reserved metric name `by`, a recorded column against a unit attribute, a recorded column named `unit`, or one named `measurement`. **Two of these never reach a user as a raised error**: a template's `aggregate` return and a derived key against a recorded column are both caught and re-reported at `run` as [`W-STATS-AGGREGATE-FAILED`](#warnings-core-reports) instead, this code interpolated into the warning's own message rather than propagated as an exception | `ContractError` · `E-STEP-RETURN-TYPE`, `E-STEP-KEY-COLLISION` |
| A [`cfg` path](#steps-and-artifacts) the config doesn't hold, a write or a delete through a `cfg` node, or a write through a frozen [`Unit`](#the-unit-list-is-three-operations-and-the-units-in-it-are-frozen) | `ContractError` · `E-STEP-PARAM-UNKNOWN`, `E-CONFIG-IMMUTABLE`, `E-UNIT-IMMUTABLE` |
...
| A [`data.units.from.resolver`](#where-units-come-from) source yielding something that is not a `Unit`, raised in `units._from_resolver`, the one place a yielded item is checked — `validate` is contracted never to raise, so this is what stands between a resolver's own mistake and an `AttributeError` escaping it. **Widened to a second shape**: a `Unit` whose attribute value `units.resolve_units` cannot coerce to a scalar, checked over every resolved unit at the end of that function — the same fault, on the argument that a `Unit` core cannot build a roster row from is one problem whichever half is wrong, so `E-RESOLVER-YIELD` is widened rather than a second code minted. Coercing there rather than leaving it to [`finalize`'s own coercion](#steps-and-artifacts) is what keeps a structural attribute value from turning a **completed** run into a late `ContractError` after every execution is paid for — the roster is checked before the first execution runs, not after the last one. Both halves [reported by `validate`](#errors-validate-reports) under the same code, which is where a run that validates first meets it | `ContractError` · `E-RESOLVER-YIELD` |
| A README's `publishable:begin`/`end` markers that cannot be paired — a `begin` with no `end` before EOF, an `end` with no `begin`, a second `begin` opening while one is still open, or an `end` naming a region other than the open one. Raised in `docs.regions`, the one parse every caller shares, so the four shapes are one code and one row: what a reader does about each is the same edit. Reported by [`docs`](#operation-commands) through its own `Collector` at exit `1`, and turned into a printed note by any [generator](#generators) merging into a README, which succeeds anyway — the package, config or template file it already wrote may not be lost to a README it could not parse | `ContractError` · `E-DOCS-REGION-UNBALANCED` |
| Two `begin` markers naming one region in one README. Raised in `docs.regions`, beside the row above and kept apart from it because the remedy differs — a duplicate is a region to delete, not a marker to pair — and reported at the same two surfaces | `ContractError` · `E-DOCS-REGION-DUPLICATE` |
| **Two senses, one code, and the row says both.** A marker naming a region core does not manage (raised in `docs.regions` while parsing), *and* a managed region a caller asked to rewrite that this README does not declare (raised in `docs._span`, reached from `body_of` and `rewrite`). The second is a refusal rather than a silent skip: a rewriter that returned the text unchanged for a region it could not find would be indistinguishable from one that worked. No sixth code was minted, because both are the same answer — the region you named is not a region of this file — and `docs`' own top-level pass never raises the second: it rewrites what it finds and [names what it did not](#operation-commands) on stdout | `ContractError` · `E-DOCS-REGION-UNKNOWN` |
| A README declaring none of the four managed regions at all. Raised in `docs.regions`. Not the same fault as declaring some of them: a repository scaffolded before a region existed is the ordinary case and its absences are named, while a file with no region at all gives `docs` nothing it may touch — everything outside a region is hand-written | `ContractError` · `E-DOCS-NO-REGIONS` |
| No `README.md` in the repository at all. Raised in `docs.read_readme`. `docs` never creates one — a README `new` did not write is a file this command has no template for, and writing one would be writing outside every region — and the diagnostic's path column names the **directory**, since the subject is the absence of the file rather than a fault inside it | `ContractError` · `E-DOCS-NO-README` |
| No git repository walking up from the path given, raised once, in `provenance.find_repo_root`. **Every path that reaches it is named here, re-derived by reading `src/` rather than by incrementing this sentence: two uncaught, four caught by code, four caught by type.** `cli._prepare_run` (the run command's phases 1-5, since H9a) and the `generate`/`init` dispatch call it **uncaught**, surfacing it at `main`'s `except PublishableError` printer on stderr, exit `1`. **Four sites catch it by code**: `validate._check_data` returns quietly ("not in a repo, so inside-the-repo doesn't arise"); `study._refuse_if_in_repo` catches it as the pass branch of its own in-repo refusal; [`reproduce`](#reproducing-on-another-device)'s config form catches it and **re-reports it under the same code** through its own `Collector`, at exit `1` — a config outside every repository names no project to restore, and re-reporting rather than re-raising is what keeps the [redaction pass](#secrets--credentials) over it, since `main`'s own handler applies none; and [`docs`](#operation-commands) does the same, for the same reason — a README is that command's entire input, so a walk-up it cannot complete is a refusal rather than a narrowing. The first two are pass branches; the last two are refusals, and that is the difference worth reading. **Four more catch it by type**, testing no code: `reproduce.prepare_checkout`'s walk-up from the derived destination's parent, where a raise IS the ordinary case — a destination outside any repository is what `reproduce` wants, so the exception path is the pass branch and the *quiet* return is the refusal; `validate.validate_config`, under a bare `except ContractError`, leaves `repo_root` `None` so local template discovery is skipped while every other check still runs; `cli._preloaded_experiment`, under `except Exception`, returns `None`; and [`list-templates`](#operation-commands), which leaves `repo_root` `None`, lists core's and every installed claim, and says on its own line that no project-local `templates/` was searched — a shorter list with no explanation would be a silent skip. **Three of the ten walk up from `Path.cwd()` rather than from a path argument**, being the commands with none to walk up from — the `generate`/`init` dispatch, `docs` and `list-templates` — the one place `CLAUDE.md` § Invariants' "a walk-up from the path the command was given, not from the working directory" does not apply, and a reader comparing the two without that sentence would conclude one is wrong; the other seven walk up from a path their command was given. This is why a config outside every repository prints `✓ config valid` and refuses only when a command needs the repository — at `run`, and at `reproduce` | `ContractError` · `E-GIT-NO-REPO` |
| A repository with no commit at all — no `HEAD` to verify. Raised in `provenance.git_provenance`, reached from exactly one place, `cli._prepare_run` (the run command's phases 1-5, since H9a), while computing the `GitInfo` the dirty gate reads next, so this code precedes `E-CODE-DIRTY`: a fresh `git init` over two untracked trees reports this, never the gate's. Checked with `git rev-parse --verify HEAD` rather than the bare form, because plain `git rev-parse HEAD` on a commitless repo writes the literal string `HEAD` to stdout as part of its own usage hint, which `_git`'s `check=False`/`strip()` convention would read back as a commit; `--verify` fails with clean, empty stdout instead | `ContractError` · `E-GIT-NO-COMMIT` |
| [`study new`](#building-one) onto a bundle path that already holds a `study.yaml` — joining the `E-*-EXISTS` family [§ Exit codes](#exit-codes-and-diagnostics) already names, refusing being how a creation command stays safe to re-run. "Existing" means a `study.yaml` FILE is already there, not that the directory exists — `~/papers/x/study` beside a manuscript may well be a directory a person made first | `ContractError` · `E-STUDY-EXISTS` |
| [`generate report`](#generators) onto a path already holding `src/<pkg>/report.py` — the same `E-*-EXISTS` family, for the same reason as `study new`'s row: refusing is how a creation command stays safe to re-run, and a silent overwrite of a hand-edited override is exactly what [`code_hash` covering `src/**`](#three-hashes) makes costly to lose | `ContractError` · `E-REPORT-EXISTS` |
| [`study add`](#what-study-add-redacts)'s `min_reported_n` prompt has a thin metric to ask about and no terminal attached to ask it at — an unattended `study add` sailing past a disclosure warning is the automation this prompt exists to prevent. Prints the offending metrics before refusing, and writes nothing: not even a partial copy of the record | `ContractError` · `E-STUDY-CONFIRM-REQUIRED` |
| [`study add --as <name>`](#building-one) where `<name>` is already in the bundle — checked BOTH against `study.yaml`'s `runs` keys and against the file `<name>.run.yaml` on disk, since the two can disagree (a hand-edited `study.yaml`, or a copy interrupted between the two writes) and the file is the thing whose overwrite loses data. The load-bearing refusal of the whole command: `main.run.yaml` silently becoming a different run is exactly the overwrite [append-only](design-principles.md#design-goals) forbids, and a bundle beside a manuscript is the last place to allow it. Re-add under a new name, or start a new bundle — adding the same `run_id` twice under two different names is permitted, since the refusal is about the name and a paper legitimately reports one run in two roles | `ContractError` · `E-STUDY-NAME-EXISTS` |
| A file under [`data.input_dir`](#the-one-config-file) changed between the manifest built at run start and the re-verification after the last execution — reported once, naming the count and every changed path. Checked at `command_run`'s phase 8, **after** the executions have run and their artifacts are on disk, which is why it marks the run `failed` rather than refusing it: the work is already paid for and the record is what says the inputs moved under it. `verify_manifest` compares content for a file the [manifest policy](#manifestinputjson--what-was-read) hashed and size-plus-mtime for one it did not, so an unhashed file that changed size or timestamp counts too. One emit site | *(no exception; a `Collector` diagnostic)* · `E-INPUT-CHANGED` |
| `src/**` or `templates/**` carries uncommitted changes when a command that executes **and enforces the gate** starts — `run` and `resume`. [`draft`](#draft-runs) executes and does **not** refuse, which is the whole of what that name buys, and [`dry-run`](#before-you-spend-it) does not enforce it either because it writes no record for a `code_hash` to identify; both pass `allow_dirty=True` to the one call site, and neither changes what the gate covers. Checked inside `cli._prepare_run` (the run command's phases 1-5, since H9a), before the empty-file-list gate below and before any step runs, for the reason [§ Three hashes](#three-hashes) gives: a `code_hash` folded over a tree that is still moving names no state anyone can return to. **Untracked files count** — the check is `git status --porcelain` over the two trees, which reports them — while **a file the repo's own rules exclude does not**. That question is asked with `-c core.excludesFile=`, [exactly as the hash asks it](#how-the-three-are-computed) and through the same shared constant, so **the gate and the hash honour one exclude chain**, which they did not before: a file a global, system, or repo-local `core.excludesFile` hides is now dirty here *and* hashed there, rather than clean here and hashed there — a recorded identity covering a file no clone of the commit contains. **It is the rules that converged, not the file sets.** The hash additionally drops [the fixed skip set](#how-the-three-are-computed) whatever git says, so a tracked, modified `src/pkg/step.pyc` is dirty at this gate and is read by no hash. One emit site | *(no exception; a `Collector` diagnostic)* · `E-CODE-DIRTY` |
| Every file [`hashed_files`](#how-the-three-are-computed) would otherwise fold into `code_hash` is absent under both trees, or every candidate git excludes — no file at all under `src/**` or `templates/**`, or `src/**`/`templates/**` present but wholly matched by an ignore rule (a `.gitignore` line of `src/`, say). Checked at `cli._prepare_run`'s single hashing site (the run command's phases 1-5, since H9a), immediately before the file list is folded: `hashed_files` and `code_hash_of` themselves compute the same fold over zero files as over any other, since two tests in `tests/test_hashes.py` use exactly `sha256:e3b0c442…` — the digest of the empty list — as a negative control, so a reader looking for this guard inside `hashes.py` will not find it. **No exception is raised**: `_prepare_run` checks the file list directly and refuses through a fresh `Collector`, the same mechanism `E-CODE-DIRTY` uses at the dirty gate two phases earlier. One emit site, and the sole reachable one, positioned after unit resolution — so a resolver's quota may already be spent when it fires — and before `allocate_run_dir`, so a refusal here leaves no run directory behind | *(no exception; a `Collector` diagnostic)* · `E-CODE-EMPTY` |
| [`unignored_under_hashed_trees`](#how-the-three-are-computed)'s single `git check-ignore` call answering with a returncode that is neither `0` nor `1` — the reachable instance is a candidate path inside a committed git submodule, which `check-ignore` refuses to answer for at all (`fatal: Pathspec '<path>' is in submodule '<name>'`, exit `128`). The message carries git's own stderr verbatim, since what went wrong is git's to say, not core's to guess. **An empty `stdout` is never read as "nothing excluded"** — only returncode `0` or `1` licenses that reading, and this code is what stands between an unreadable answer and a false one. One emit site: the one `check-ignore` call `_prepare_run`'s single hashing site makes, reached through `hashed_files`' `include` callback | `ContractError` · `E-CODE-FILE-LIST` |
| A declared [apparatus probe](#the-apparatus-core-can-only-observe)'s own body raises anything but `KeyboardInterrupt` — a probe is user code, called at [`dry-run`](#before-you-spend-it), at run start, before every execution and at `freeze`, never at `validate`, which dispatches no probe at all. **Except `KeyboardInterrupt`**, re-raised fresh and argument-less, `from None`, so Ctrl-C still stops the command and a `KeyboardInterrupt("…secret…")` a probe body constructed never reaches Python's own printer. The sibling of `E-RESOLVER-RAISED`, the identical shape at a surface `validate` never reaches. Raised in `apparatus.observe_once`, and one of **four** outcomes follows depending on which surface calls it. A run-start raise, before any execution exists, is contained by `_execute_prepared` through a fresh, redacting [`Collector`](#secrets--credentials) and ends the command there — no `run.yaml`, exit [`5`](#exit-codes-and-diagnostics). A mid-plan raise instead stops [`execute_plan`](#what-status-means-and-when-a-run-keeps-going)'s loop — the apparatus **unreachable**, distinct from a changed fact — and reaches the same redacted diagnostic through the record path: `status: partial` and the executions that already ran kept, **once there are any to keep** — a raise on the first `pre_execution` round has none, leaving no `run.yaml` at all — exit `5` either way, taking precedence over `partial`'s own `3`. **A third surface**: [`freeze`](#cli-reference) meets the identical raise from its own call to `observe_once`, before `append_observation` ever runs for that call, so no ledger line is written and no `run.yaml` is at stake either way — contained the same way, through a fresh redacting `Collector`, exit `5`. **A fourth**: [`dry-run`](#before-you-spend-it)'s own round, contained the same way again, and it stops the command before the transcript prints — there is no run directory, no ledger and no record at stake at all, and the exit is `5` on the same reading every other surface gives it | `ContractError` · `E-APPARATUS-RAISED` |
| A probe returns something that is not an `Apparatus`, or an `Apparatus` whose `facts` is not a mapping, or whose `facts` holds a key that is not a `str` — the sibling of `E-RESOLVER-YIELD` above: a probe is the second place user code hands core a value whose shape nothing validated in advance, and `Apparatus.__init__` deliberately checks none of it ([§ The apparatus core can only observe](#the-apparatus-core-can-only-observe) states why), so this is what stands between a probe's mistake and an `AttributeError` or `TypeError` escaping the round that called it — `cli._execute_prepared` at `run` and `draft`, `cli._dry_run_probe` at [`dry-run`](#before-you-spend-it), `freeze`'s own round at `freeze`. Raised in `apparatus.check_facts`, the one place a returned value's shape is checked, before either of the next two rows run | `ContractError` · `E-APPARATUS-RETURN` |
| A fact value a probe returned equals, or **contains**, the value core read for a declared credential — refused rather than redacted, because [`provenance.apparatus` is publishable as-is](#the-apparatus-core-can-only-observe) and a redacted value sitting in a block whose whole contract is that it needs no redaction would still be evidence a credential had been there. Checked by **exact value or substring containment**, never by pattern, against exactly the set [`credential_values`](#secrets--credentials) answers for this config's declared names — the same matching [`redact`](#secrets--credentials) uses over the identical value set, since a credential embedded in a larger value (an endpoint URL carrying `?key=<token>`) is not a pattern guess, it is a fact core already knows. Checked before the scalar walk below, since a credential passing as a plain string must be caught here rather than reaching it. Raised in `apparatus.check_facts`. The message names the fact's **key** and the credential's **name**, never the value, whichever way it matched | `ContractError` · `E-APPARATUS-FACT-CREDENTIAL` |
| A fact value outside the closed scalar set [`coercion`](#steps-and-artifacts) already enforces for a step's own return — `bool`, `int`, `float`, `str`, `None`, or what core coerces to one. Its own code rather than [`E-STEP-RETURN-TYPE`](#errors-core-raises), on the precedent `E-RESOLVER-SWEPT-PARAM` already sets for sharing a mechanism without sharing the fault: `coerce_scalars` is the one scalar walk both use, but that identifier is a step's, reached from a step's own `run` return, and a reader holding it would be sent to a section describing a different fault at a different time. Caught and re-coded in `apparatus.check_facts`, since `coerce_scalars` takes no `code` parameter of its own to pass one through — the same catch-and-re-code `E-RESOLVER-SWEPT-PARAM` makes over `E-STEP-SWEPT-PARAM`'s own raise. The message names the value's **type**, never the value, the same shape `E-STEP-RETURN-TYPE`'s own refusal uses, so a fact value whose `__str__` or `__repr__` carries a credential is never interpolated into it | `ContractError` · `E-APPARATUS-FACT-TYPE` |
| A key [`apparatus_facts`](#the-apparatus-core-can-only-observe) declares that the probe's returned `facts` does not carry at all — the one of the three states that section names as an error, because only this one is the plugin and the template disagreeing about what the probe supplies: a value is recorded as given, a declared key answering `null` warns ([`W-APPARATUS-UNANSWERED`](#warnings-core-reports)) rather than failing, and an **undeclared** key the probe returns is recorded with no check of any kind. Raised in `apparatus.check_facts`, last of its four checks, over the declared names alone | `ContractError` · `E-APPARATUS-FACT-MISSING` |
| A probe's answer for one fact **disagrees with that same `(condition, fact)` pair's own first *answered* observation** — never the previous observation, and never another condition's. `null → value` and `value → null` are not this: a fact answered unevenly is not evidence the apparatus moved, only that it is answering unevenly, so those pass and only a value contradicting a prior value raises. The message names the condition key, the fact, and both values (`condition `00`'s fact `pinned` changed: r1 → r2`), never a credential: `apparatus.check_facts` already refuses a fact value matching a declared credential before this gate ever sees a pair. Raised in `apparatus.check_changed`, after `check_facts`, `append_observation` and `Observations.record` have all already run for the same call — the moving observation is on disk and counted before anything can stop the run. **No policy knob permits this**: [`limits`](#the-one-config-file) is a closed key set, so nothing configurable can soften it. The plan stops where it stands, `status: failed`, the record kept — once there is one to keep, per [§ Exit codes](#exit-codes-and-diagnostics)'s `1` row — exit `4`. **A second surface**: [`freeze`](#cli-reference) meets the identical raise, from the same gate, comparing a fresh probe against the run's own ledgered baseline — there `run.yaml` was never at stake, so the code's exit is `1` rather than `4` | `ContractError` · `E-APPARATUS-CHANGED` |
| A probe's answer for one fact disagrees with **`configs/<name>/apparatus.expected.json`**, the file [`reproduce`](#reproducing-on-another-device) writes out of a record's own `provenance.apparatus.facts`. Read at `run` and `draft`, at the **first** probe round, and only when that file sits beside the config — a run whose config directory holds none reaches this check with nothing to compare against and is unchanged in every respect. On **the gate's own terms**, and that is the point: the comparison is `Observations.record` followed by `Observations.changed` over a second, expectation-seeded `Observations`, so `null → value` passes exactly as it does for a run's own gate — the reproduction pinned something the original didn't, which is more evidence rather than less. Its own code rather than `E-APPARATUS-CHANGED`: that one says a fact moved *during this run*, this one says this run is not measuring through the apparatus the record did, and a reader holding one identifier must not be sent to the other's paragraph. `E-APPARATUS-CHANGED` wins when both would fire, since this check is sited after it. Exit `1` before the first execution and `4` once there are results to keep — [`run_status`](#what-status-means-and-when-a-run-keeps-going)'s own fold, the same split `E-APPARATUS-CHANGED` gets, and it holds for a [`resume`](#resuming) whose run-start round meets it with a prior attempt's results on disk | `ContractError` · `E-APPARATUS-UNEXPECTED` |
...
| A project-local `templates/*.py` fails to load, in any of three shapes: it raises while importing, it imports cleanly but registers nothing, or it registers a class that is not a `BaseTemplate` subclass — [reported for the first such file](#errors-validate-reports) in `discover_local`'s sorted walk, once every file in the directory has been imported. One of the load-time raises, beside `E-STEP-NAME-COLLISION`/`E-STEP-SCOPE-UNKNOWN`, `E-TEMPLATE-COLLISION`, `E-PLUGIN-COLLISION`, `E-PLUGIN-DECORATOR` and `E-PLUGIN-LOAD`, and dual-surface for the same reason `E-TEMPLATE-COLLISION` is: every command resolving a template meets `discover_local` at load — [`reproduce`](#reproducing-on-another-device) included, which resolves the template of the checkout it just made, in that checkout's own `templates/`, and reports this code through its own `Collector` at exit `1` (anything else escaping that resolution is reported under this code too, which is one of four byte-identical places it stands for a fault `discover_local` did not itself name — the others are `report`, `freeze` and `docs`, each resolving a project-local template after the fact). Checked *ahead* of a collision — a collision computed while a file failed to load would be computed over a partial set of claims. Never raised inside an execution, for the same reason `E-TEMPLATE-COLLISION` is not | `ContractError` · `E-TEMPLATE-LOAD` |
...
| [`generate step`](#generators) or [`generate report`](#a-report-override-renders-one-experiments-own-figures) naming an `experiment` package whose `src/<pkg>/` does not exist — a different check from `E-EXPERIMENT-EXISTS`'s, which is `generate experiment`'s own overwrite guard on the same directory once it *is* there | `ContractError` · `E-EXPERIMENT-UNKNOWN` |
| A name a generator cannot write: [`generate step`](#generators)'s `step_name`, or `generate experiment`'s `name` once its `-` become `_`, is not a Python identifier or is prefixed with `__`. Both values reach an **import path and a class name** in a file the generator writes, and neither position can be escaped — quoting produces a valid string literal for any value and a valid identifier for none. **Raised before anything reaches disk**, which for `generate step` is the whole of it: that command rewrites `src/<pkg>/experiment.py`, a file it did not create and which may already declare a working pipeline, so a check made afterwards would refuse a project that no longer imports. One code for one fact across the generator family, the way `E-PROJECT-EXISTS` is shared by `new` and `plugin new`; `generate template` states the same rule at the CLI and exits `2` instead, because every file it writes is one it creates | `ContractError` · `E-GENERATE-NAME` |
| Two shapes, both against [`publishable.resolvers`, `publishable.probes`, `publishable.writers` or `publishable.readers`](#creating-a-plugin-publishable-plugin-new). **One entry-point key claimed by two installed distributions**: decided over the **complete** claim set for the group and reported in **name order**, not in the order the metadata scan happened to walk one — install order is a property of a machine rather than of a design, so it may not decide which fault is reported either. The message names every distribution that claimed the key, as `<distribution> <version>`, which is what a reader uninstalls. **A writer or reader claiming a suffix core itself writes or reads**: decided at registration, when the `@register_writer`/`@register_reader` decorator runs, over no claim set at all — the message names the suffix and core rather than a distribution, and the remedy is renaming the plugin's own claim rather than uninstalling anything. The template groups' equivalent is `E-TEMPLATE-COLLISION` rather than this code, since a template name has a second home — [the project's own `templates/`](#templates-where-parameters-are-defined) — and one row cannot state both sets of providers. A writer or reader claiming a core suffix from inside a plugin's top level raises this at decoration time, which is inside the import [`E-PLUGIN-LOAD`](#errors-core-raises) contains — so a load reports the containing code, the same substitution `E-TEMPLATE-LOAD` already makes for a coded error from a local template's top level | `ContractError` · `E-PLUGIN-COLLISION` |
| A [`@register_*` argument](#creating-a-plugin-publishable-plugin-new) disagreeing with the entry-point key that named it. The entry point is the registration and the decorator is a declaration checked against it, so two spellings of one name with no rule for which is canonical is refused rather than resolved — the [defaults-file argument](#there-is-no-separate-defaults-file) again. Reached at `validate` as well as at `run` for a **resolver source**: `validate` dispatches a declared resolver to resolve the roster, and dispatching means loading — so a resolver's disagreement is `validate`'s finding whether or not the run it would have executed ever starts. An [apparatus probe](#the-apparatus-core-can-only-observe)'s dispatch goes through the identical check, but **never at `validate`**, which calls no probe at all — so a probe's disagreement is never `validate`'s finding, only `apparatus._probe_for`'s, met wherever a command resolves the declared probe to call it: `run` and `draft`, [`dry-run`](#before-you-spend-it), and `freeze`. A **writer or reader** reaches it at neither of those moments and at no command in particular: its claim is resolved by the [suffix dispatch](#steps-and-artifacts) itself, so the surface is any `io.write` whose name a claim wins, and any read of such a name — which puts it inside a step, inside an execution, where it is that execution's failure | `ContractError` · `E-PLUGIN-DECORATOR` |
| An entry point whose module raises while importing, or calls `sys.exit()` at module scope. `SystemExit` is a `BaseException` and so needs its own `except` — a plugin building an `argparse` parser at import would otherwise end the command with the plugin's own exit code and no diagnostic at all. The same containment `E-PLUGIN-DECORATOR` describes — reached wherever a plugin is imported, which is a [resolver source](#where-units-come-from)'s dispatch, at `validate` as well as at `run`, and an [apparatus probe](#the-apparatus-core-can-only-observe)'s dispatch, but — unlike a resolver's — **never at `validate`**, which calls no probe to reach it; a probe's own top level raising this way is met at every command that dispatches one, through `apparatus._probe_for` — `run`, `draft`, [`dry-run`](#before-you-spend-it) and `freeze`. A **writer or reader**'s top level is reached by the [suffix dispatch](#steps-and-artifacts) instead, so its surface is any `io.write` whose name a claim wins and any read of such a name, inside a step and therefore inside an execution — that execution fails and the run record is still written, the shape `E-ARTIFACT-UNWRITABLE` already had there. The fault names the entry point and the distribution rather than the module, since a distribution is what a reader uninstalls or pins | `ContractError` · `E-PLUGIN-LOAD` |
...
| [`io.read_condition`](#steps-that-need-every-condition) naming a condition index this run did not resolve, or naming a repeat-scoped step with no `repeat=` to say which copy — raised at the one shared `_resolve_condition_step_dir`, called from both `StepIO.read_condition` and a [report override's](#a-report-override-renders-one-experiments-own-figures) own `ReportIO.read_condition`, so either surface meets the identical two faults | `ContractError` · `E-STEP-READ-CONDITION-UNKNOWN`, `E-STEP-READ-REPEAT-REQUIRED` |
| A run directory's `lock` is held, so a second writer is refused. **Reachable from [`resume`](#resuming) and from nowhere else**, structurally rather than by luck: `run` and `draft` claim their directory with `allocate_run_dir`'s own `mkdir`, so a directory they just created cannot already hold a lock, and `dry-run` takes no lock at all. Every site: the raise in `RunLock`'s own claim, when the exclusive create finds the file there (the message names the recorded holder — host, pid and start time); and, inside `resume`'s takeover, one raise when the exclusive `lock.takeover` token already exists — another `resume` is deciding this directory right now — and one when the liveness test cannot prove the holder gone. **A holder is provably gone only when `os.kill(pid, 0)` raises `ProcessLookupError` for a pid recorded against this machine's own hostname**; every other state holds the lock, a lock recorded on another node included, since core cannot see another machine's process table. The recorded start time is a diagnostic and is deliberately **not** consulted, so a recycled pid reads as alive and refuses. `resume` reports this through [its own collector](#exit-codes-and-diagnostics); every other command meets it through `main`'s handler, and both print the same identifier at exit `1`. **The residual, stated rather than hidden**: a takeover killed with `SIGKILL` inside its own two-syscall window leaves `lock.takeover` behind and every later `resume` refuses until that file is removed by hand, which the message says | `ContractError` · `E-RUN-LOCKED` |
| All 26 suffixed names for one run id are already taken — 26 runs of the same code in the same second. Raised in `allocate_run_dir`, so it belongs to `run` and `draft`, the two commands that allocate a directory; `resume` re-enters an existing one and never reaches it. Reported at exit `1` through `main`'s handler | `ContractError` · `E-RUN-ID-EXHAUSTED` |
| Core's execution plan disagreeing with the state core resolved beside it — a repeat execution whose seed or whose resolved config isn't there, a [realized order](#a-batch-says-when-not-what) naming a pair the plan doesn't hold, a declared fold core can't pair with the units it partitions — a repeat label carrying no fold component, a fold with no resolved roster, drawn partitions with no fold level to label them, a [cell](#expansion-modes) decomposition that does not partition the roster the folds are drawn inside — or a condition on a [group axis](#expansion-modes) core can't pair with its arm, the same disagreement one level over: a condition index missing from the resolved arms. A [`data.units.from.resolver`](#where-units-come-from) source reached with no `cfg` to hand it joins the same family for the same reason: a resolver sees the same `cfg` a `scope: "run"` step does, so this is core's resolved state disagreeing with itself rather than the config being wrong — reached only from a caller that skips the `cfg` decision 6 makes every production call site thread, never from `validate` or `run` | `ContractError` · `E-RUN-SEED-MISSING`, `E-RUN-CFG-MISSING`, `E-RUN-ORDER-MISMATCH`, `E-REPL-ORDER-UNRESOLVED`, `E-RUN-FOLD-UNRESOLVED`, `E-RUN-ARM-UNRESOLVED`, `E-RUN-RESOLVER-UNCONFIGURED` |

**The last row above — core's plan disagreeing with the state core resolved beside it — is the one you can't cause, and it's in the table anyway.** Every other entry is something your declarations or your step code asked for; those seven are core checking its own work, because the plan and the resolved conditions, repeats, seeds, folds, arms, order, and — for a resolver source — the `cfg` handed to it are all derived from one config and must agree. If they ever don't, the run would execute something other than what it recorded, and no result from it is worth reading. They carry a type and a code for the same reason everything above does rather than being an `assert`: `assert` disappears under `python -O`, which is precisely the wrong property for the only guard on a condition nothing else detects. One of them reaching you is a bug to report, not a config to fix — and, like the load-time rows (`E-STEP-NAME-COLLISION`/`E-STEP-SCOPE-UNKNOWN`, `E-TEMPLATE-COLLISION`, `E-TEMPLATE-LOAD`, `E-PLUGIN-COLLISION`, `E-PLUGIN-DECORATOR`, `E-PLUGIN-LOAD`) and like the two run-directory rows just above it (`E-RUN-LOCKED`, `E-RUN-ID-EXHAUSTED`, which you *can* cause and which stop the command before any step exists), none of the seven is raised inside an execution, so it stops the run instead of failing one step and continuing. A plan core can't trust is not a plan to keep walking.
...
**It is dot-access and nothing else**: a mapping in the config is a node with the same behavior, a list is a list whose mapping elements are those nodes, and a scalar is the scalar. There is no `.get`, no `.keys()`, and no method of any kind on a node — which is what makes it safe for a parameter to be named `items` or `values`, since there is nothing for it to shadow. The single exception is the root node's `raw` accessor, [described above](#the-importable-surface) and costing only the one top-level name core already owns; every nested node has none. A path the config doesn't hold raises [`ContractError`](#errors-core-raises) naming the full dotted path and the nearest key it could have meant, exactly as [`E-PARAM-UNKNOWN`](#validation) does at validate time, because against a closed schema a path that misses is a typo by construction. Names beginning with an underscore never resolve as config keys and raise `AttributeError` instead, so `hasattr`, `copy`, and every other protocol that probes an object by attribute keeps working rather than meeting an exception it doesn't expect. A template's [`validate(self, config)`](#templates-where-parameters-are-defined) receives the **parsed document itself** — a plain mapping, not this node — and that is the one place core hands a config to user code in the other shape. The reason is the sentence two clauses up: a node raises on a path the config does not hold, and a cross-block rule's whole question is whether an optional block is *declared*. Five of the paths such a rule asks about — `statistics.contrasts`, `.report_by`, `.resample`, `.null_test` and a `sweep` mode — are absent from what [`init`](#the-one-config-file) writes, so a reader that refused an absent path could not answer the question it exists for. `config.get("statistics") or {}` is the idiom, and [§ Templates](#templates-where-parameters-are-defined) shows it against the rule it was promised for.

**And it is read-only, for the reason a [`Unit`](#the-unit-list-is-three-operations-and-the-units-in-it-are-frozen) is.** A write or a delete through a node raises [`ContractError` · `E-CONFIG-IMMUTABLE`](#errors-core-raises). The failure it prevents is quieter than a unit's: a node is rebuilt on every access, so `a = cfg.analysis; a.method = "kendall"` would have *succeeded*, `a.method` would have read back `"kendall"`, and every fresh `cfg.analysis.method` would have kept returning what the config declares — a held handle and a fresh access disagreeing, with nothing raised anywhere. Core [cannot inspect a step's body](design-principles.md#greenfield-only) to catch that either, so the object refuses at the write, the same way a unit does. The config is the record of what ran; a parameter that needs to change changes in the file.
...
| Method | Behavior |
|---|---|
| `io.write(name, obj)` | Writes into this step's directory. Raises `ArtifactExistsError` if the target exists — no overwrite, no backup-and-replace, no delete-to-make-room. **Atomic**: temp file plus rename, so a crash leaves nothing rather than a half-file that would permanently block a retry. A writer sees only the rows or bytes it was handed, never the artifact's name, so `io.write` prefixes its own name onto a writer's `ContractError` — the writer's own message about the column and the row survives unchanged inside it. |
...
**What a writer takes is what its reader gives back**, so a round trip through an artifact is true by construction rather than by convention — **for every extension but one**, stated in the table's own `.csv` row:
...
| `.parquet` | a sequence of mappings, one per row — a cell may be `bool`/`int`/`float`/`str`/`None` (a NumPy spelling of one coerced to its Python counterpart first, so a column need not agree cell-by-cell on which spelling it arrived in) **or** a structural or `bytes` value, which this format round-trips byte-faithfully | rows, as mappings, each cell exactly the type it was written as |
| `.csv` | a sequence of mappings, one per row, every cell a scalar core can coerce to `bool`/`int`/`float`/`str`/`None` | rows, as mappings, **every cell a `str`** — so the promise above is false for `.csv` even on a legal write, not only on a structural one: `[{"v": 1.0}]` reads back `[{"v": "1.0"}]` |
| anything else | `bytes` or `str`, written verbatim | `bytes` |

Rows as mappings rather than a `DataFrame`, on the same argument [`aggregate`'s table](#templates-where-parameters-are-defined) rests on: accepting a library's table type would make that library part of core's public API, and a step is one `df.to_dict("records")` away from the shape core does take. The column set of a `.csv` or `.parquet` is the union of its rows' keys, a key absent from a row writing empty, exactly as the [per-unit table](#units-the-thing-being-measured) already behaves. Handing a writer anything else — a model, an array, an object whose shape core would have to guess, or a row a `.csv`/`.parquet` writer receives that is not a mapping — raises `ArtifactError` · `E-ARTIFACT-UNWRITABLE`, which is the same refusal as never pickling for you, one step earlier.

**The two row-shaped writers coerce a cell's scalars, and only they.** `.csv` and `.parquet` each run every row through the same [scalar coercion](#steps-and-artifacts) `io.record`'s values take — a `numpy.float64` beside a `float` is one type by the time either writer sees it — but they answer *"a writer accepts what it can give back"* differently, because they differ in what they can give back. `.csv` cannot return a structural or a `bytes` cell at all: before this rule, such a cell wrote silently as its `repr` (`"[1, 2]"`, `"b'x'"`) and read back as that string — a working-looking round trip that was actually silent corruption. `.csv` now refuses it instead, `ContractError` · `E-STEP-RETURN-TYPE`, naming the column, the offending row, and (through `io.write`'s prefix, above) the artifact — converting that corruption into a diagnostic. `.parquet` **keeps accepting** a structural or `bytes` cell, because it *can* return one byte-faithfully; no refusal is added there, and a `.parquet` row mixing a NumPy scalar with its Python counterpart — the shape this section's own worked step produces — **stops raising** `E-STEP-RETURN-TYPE`, a refusal this build no longer has. A row that is not a mapping at all is refused the same way by both formats, `ArtifactError` · `E-ARTIFACT-UNWRITABLE` naming the row — there is no format that can give back a row that was never a mapping to begin with. `.json`, `.yaml`, and `.jsonl` run no coercion at all, because their documented input is any nesting rather than a flat mapping of scalars, and a plugin's own format is the plugin's business, not core's — the rule belongs to the format whose contract states it.

`units.parquet` unifies a column's type across every row it holds, once coerced — [§ The per-unit tables](#the-per-unit-tables) states that rule and why it is checked only for `.parquet`. `io.write` checks the identical thing for any `.parquet` row set it is handed, not only the two core writes itself.

**The extension is the longest registered suffix of the name's last component, compared in lower case.** A compound extension is the ordinary case in several domains — `.fastq.gz`, `.nii.gz`, `.tar.gz` — so splitting at the final dot is not an option: it would hand `reads.fastq.gz` to whatever claimed `.gz` and lose the format in the name. Longest-suffix is also what keeps a plugin's claim safe from a coarser one, since `.fastq.gz` beats `.gz` whichever was registered first. Only the name's last component is examined, and only against suffixes core can actually write — which is every suffix a writer has **registered** plus every suffix an installed distribution **claims** in [`publishable.writers`](#creating-a-plugin-publishable-plugin-new), the claim read from package metadata with no import. So `programs/gpt-4.1__seed29.json` is a `.json`, the dot inside its stem matching nothing, and a plugin's `.fastq.gz` is a `.fastq.gz` before anything has imported the plugin. **A claim wins only by being strictly longer than what is registered**, which is what keeps core's own five unshadowable: a claim on `.csv` never takes it, and — the one silence here rather than a refusal — such a claim is *inert* at the write rather than refused at it, since refusing would make one plugin's bad claim break every core `.csv` write in the process. [`E-PLUGIN-COLLISION`](#errors-core-raises) is what refuses it, at the decoration this claim's module runs whenever anything imports it.
...
**A value that is a scalar in every sense but its type is coerced, and only that.** `pearsonr(...).statistic` is a `numpy.float64`, not a `float`, and a rule that rejected it would reject the return statement in most of this document's own examples. So core coerces anything implementing `__float__`, `__index__`, or `__bool__` to the Python scalar it stands for, keeps that, and raises `ContractError` on everything else — a list, a dict, an array, a `DataFrame`, a fitted model. The line is deliberately at *what the value already is* rather than at what could be talked into serializing: a NumPy scalar is a float that arrived through a library, and an array is a decision about what the metric should have been. A value that is already a `str` by inheritance — `np.str_`, or a `class Color(str, Enum)` member — is that string, coerced with `str.__str__` rather than `str()` so a subclass's own `__str__` override (an enum's, spelling `Color.RED` rather than the value `red`) cannot corrupt it: `str` is one of the four scalar types this module accepts, and the only thing wrong with a value like `np.str_` is that its type is not exactly `str`. **This retires three refusals, and leaves one surface untouched.** An `np.str_` value (or any `str`-subclass value) at `io.record`, a step's return, a template's `aggregate`, or a derived metric — refused as structural before this task, since `np.str_` has `__len__`, the same test that refuses an array — now coerces instead. An `np.str_` apparatus fact, caught and re-coded to `E-APPARATUS-FACT-TYPE`, is no longer refused that way and is recorded instead. And an `Estimate`'s `n`, held to no numeric rule of its own, is no longer refused there either and is kept as the string it names. `np.bytes_` stays refused, on the same ground plain `bytes` already is — `bytes` is not one of the four scalar types, and admitting the NumPy spelling of a type core refuses in its Python spelling would be the divergence this rule exists to prevent. **A resolver-yielded attribute value retires nothing**: nothing coerces a roster attribute today, so that value was never refused and still is not — it already writes cleanly uncoerced, before and after this task. Coercing it is a later payload, on its own decision and its own fixture; this branch had to land first so that coercion meets a value already admitted here, rather than one the `__len__` guard still refuses on a window no test today can see.

**One rule, every surface.** `io.record`'s `values`, a step's return, a template's [`aggregate`](#templates-where-parameters-are-defined), and now the two row-shaped writers all coerce under the same rule — the per-unit value a model hands you is a `numpy.float64` at least as often as a derived metric is, and a table core would reject what a return accepted would be a divergence found on the first line anyone writes. What differs is only where the value lands, and, for `.csv` alone, that a structural or `bytes` cell has nowhere legal to land at all.

**One metric name is reserved: `by`.** [`statistics.report_by`](#reporting-strata) spends it — a stratified block keys its rows by `by`, so a name of that kind has two candidate meanings for one column. A template's `aggregate` returning a derived metric called `by` collides outright and raises `ContractError` · [`E-STEP-KEY-COLLISION`](#errors-core-raises); a step *recording* a column called `by` is not refused the same way, since the retry that would raise it re-runs against executions that already completed — the column keeps its recorded value in [`units.parquet`](#the-per-unit-tables), and its own metric block too under [the same rule any recorded column earns one by](#the-per-unit-tables) — over the units that carried a number, even when that is fewer than all of them — and is reported instead as [`W-STATS-STRATUM-SHADOWED`](#warnings-core-reports), with no contrast delta and no seat in the correction family. The set is stated here rather than left to be discovered by collision, and it is a set of one today; anything added to it is a breaking change to what a template's `aggregate` may return.

**A second, disjoint reservation covers what [`data.units.attributes`](#where-units-come-from) may name — `unit`, `measurement`, or `by`.** It is a different set with a different subject, not a widening of the one above: the metric set `aggregate` may return stays `{by}`, and this one is what an attribute declaration may name, with `by` sitting in both sets for the one reason already given — a stratified block keys its rows by `by` either way. A declared attribute carrying any of the three names would silently occupy a column [a per-unit table](#the-per-unit-tables) already carries, rather than being read back as an attribute — most visibly for `unit`: a declared attribute named `unit` used to replace the unit-key column outright, publishing a table like `[{"unit": "HIJACK", "site": "n", "v": 1.0}]` in place of the identity every pairing and every contrast rest on. `validate` now refuses the declaration, `ContractError` · [`E-UNITS-ATTR-COLUMN`](#errors-validate-reports), and a `run` stops at the same gate before its first execution. **This removes one producer of a `by` column, not the possibility of one**: a step *recording* a column called `by` stays legal, on the argument just made for it, so a recorded `by` column still reaches the record and still has to be told apart from a metric by where it sits, never by its name — this refusal is not license to treat a `by`-named column as safe to identify by name anywhere else.
...
**A run whose plan stopped early is continued rather than refused, and the record it already wrote is not touched.** `run.yaml` means *that attempt* ended, so a directory holding one is never reopened — but a [truncated](#what-status-means-and-when-a-run-keeps-going) plan still owes executions, and losing them is expensive: a metered run stopped by one dropped connection nineteen hours in had no route back at any price. `resume` on such a directory allocates a fresh `run_<id>/` beside it, copies the prior attempt's state into it — everything but `run.yaml`, which the continuation has not earned, and `lock`, because copying one forges a live holder — and continues there. The first record keeps saying `partial` forever, which is true of that attempt; the continuation's `identity.json` carries `continues`, naming the run it started from, so it cannot claim work it inherited.

**Two stop reasons are continuable and one is not, and the difference is whether the state can change.** `max_failed_fraction` fired on units whose executions failed, and an execution that failed for a transport reason recovers on a retry; an [unreachable apparatus](#the-apparatus-core-can-only-observe) is waiting to be brought back. A **changed** fact is the exception: it cannot move back, so a continuation would meet the same gate, truncate again, and leave a second dead record beside the first. That run is over and `E-RESUME-RUN-ENDED` says so, naming the reason.

`resume` refuses if `parameters_hash`, `code_hash`, or `uv.lock` don't match current state. Resuming into a *different* experiment is the failure this guards against; with parameters hashed separately, "I edited the config and then resumed" is caught rather than missed. Each comparison has one recorded operand and one recomputed one, and each has its own code: [`identity.json`](#identityjson--the-claims-a-run-makes-before-it-executes) holds the three figures the run started under, `resume` recomputes them from the tree and the project's config, and a disagreement is `E-RESUME-CODE-MOVED`, `E-RESUME-PARAMS-MOVED` or `E-RESUME-LOCKFILE-MOVED` — the lockfile in **either** direction, `null` included, since a lockfile added or removed is a different environment either way. The inputs are compared the same way against the recorded [`manifest/input.json`](#manifestinputjson--what-was-read) rather than against a fresh manifest of themselves (`E-RESUME-INPUT-MOVED`), and the recorded manifest is what the rest of the run then uses, so `run.yaml`'s `input_manifest_hash` stays the first attempt's figure The manifest comparison asks the same content question `input_manifest_hash` itself asks, so under `hash_all` — and for any file `hash_index` named — a file whose bytes are unchanged passes however its mtime moved.
...
**`data.units.attributes` is a projection, and what it leaves out is gone.** A resolved `Unit` carries
only the attributes the config declares — a column the source table holds, or a value a resolver yields,
that `attributes` does not name is **dropped**, and a step asking for it through `unit.attributes` gets
nothing. The rule is the mirror of the two refusals above it: declaring an attribute the source cannot
supply is [`E-UNITS-ATTR-MISSING`](#errors-validate-reports), and supplying one the config does not
declare is silent, because a source is entitled to carry columns a run has no interest in. It is stated
here because the silence has a sharp edge — a config that declares `attributes: [site]` for a
[`report_by`](#reporting-strata) has, by that same declaration, stopped its own steps from seeing every
other column in the extract, including the label they were written to read. Nothing diagnoses that:
core never reads a step body, so it cannot know which attribute a step will ask for. **Declare every
...
**A declared attribute's value is a scalar**, under the same coercion `io.record`'s values and a step's return already go through: a `bool`, `int`, `float`, `str`, `None`, or what core coerces to one — never a list, a mapping, or anything else structural. Checked once, over every resolved unit's attribute mapping, at the end of resolution — after the source, after [collapsing measurements](#what-isnt-a-repeat), and after the roster's own key-uniqueness check. Before this, a resolver-yielded list or mapping attribute value wrote a list or mapping column straight into `units.parquet`, the published inference base, unrefused. Now it's refused before the first execution runs, under [`E-RESOLVER-YIELD`](#errors-validate-reports) — the code widened rather than a second one minted, since a `Unit` core cannot build a roster row from is one fault whichever half of it is wrong. A NumPy scalar (`np.float64`, `np.int64`, `np.bool_`, `np.str_`) coerces to its Python counterpart rather than being refused — the ordinary shape of a value a resolver computes from a dataframe or a fitted model. **Cost, named so it doesn't have to be discovered**: the `Unit` a resolver constructed is replaced by an equal-but-coerced one. `Unit` is frozen and hashable by `key` alone, so nothing promises object identity — a resolver holding a reference to its own yielded object and expecting core to hand back that same object would be surprised.
...
- **Beside a cell structure the split is drawn within each cell, not across the roster.** Under `allocation: between`, or under a non-empty [`sweep.groups`](#expansion-modes), core draws one split inside every cell that holds units — so each cell contributes the declared `frac` **of itself**, and no cell can come out with a test side of nothing while the roster-wide split looks even. `frac` is bounded by the thinnest populated cell rather than by the roster: a cell that cannot spare a test unit at the declared fraction is [`E-DATA-HOLDOUT-EMPTY`](#errors-validate-reports), which names the cell. A cell no unit falls in is skipped rather than drawn in — it has no side to leave empty. `allocation.json`'s `holdout` block records the axes it was drawn inside under `within`, so a reader can tell a per-cell split from a roster-wide one without crossing the memberships by hand. A `fold` repeat beside the same cell structure is drawn the same way and bounded the same way; see [§ Clustered units](#clustered-units).
...
**Under `allocation: between`, a fold is drawn within each cell rather than across the roster** — the same rule as a [holdout](#a-fixed-holdout-split), for the same reason: one roster-wide partition gives the cells unequal test sizes and, once `k` approaches the smallest cell's size, a fold holding none of that cell's units at all, which is a cell-level metric computed from nothing. Drawing within each cell keeps every fold proportional throughout, and `k` is bounded by the *thinnest populated* cell's unit count — or its cluster count, when `cluster_by` is declared — rather than by the whole roster's, so a `k` that fits the roster and not the thinnest arm is refused ([`E-REPL-FOLD-K-TOO-LARGE`](#errors-validate-reports)) naming that cell. The per-cell partitions are merged index-wise: fold *i* is every cell's fold *i*, so the fold indices are still one set across the run and a design with no cell structure draws exactly what it drew before. `sweep.yaml` records `partitions_within` naming the axes they were drawn inside, which is what tells a reader crossing `partitions` against [`allocation.json`](#allocationjson--who-went-where)'s arms that the split was per cell.
...
**A template can live in three places, and where it lives decides how it's pinned.** Core ships `generic`; a plugin ships its own and arrives as a pinned `uv.lock` entry; `generate template` writes one into `templates/` in your own repo, for a template only this project needs. **The second of those is refused as this project ships**, and it is said here rather than only at the code that refuses it: core resolves an installed template's name from package metadata and never loads the class behind it, so a config naming one earns [`E-TEMPLATE-INSTALLED-UNSUPPORTED`](#errors-validate-reports) at `validate`, at `generate experiment` and at `freeze`. The home is specified, the pinning it would buy is real, and nothing is chartered that would load it — so a shared experiment type is written as a `templates/` file in each repository that runs it, which is the third home paying the third home's price. A plugin's four other registries are unaffected: a resolver, a probe, a writer and a reader all dispatch from an install. The third is code the run's numbers came out of and has no version anyone resolves, so [`code_hash` covers `templates/**`](#three-hashes) alongside `src/**` — editing a local `aggregate` moves the hash exactly as editing a step does, and `run` refuses a dirty `templates/` for the same reason it refuses a dirty `src/`. Discovery importing every file to find its registration writes `templates/__pycache__/`, same as any import, and two different mechanisms keep it out of the way. `code_hash` skips `__pycache__` directories and compiled `.pyc`/`.pyo` files unconditionally, wherever in the hashed trees they sit — that skip is the [first of its four cases](#how-the-three-are-computed) and is applied before git is asked anything, so no ignore file could have done that for it and a *tracked* `.pyc` is skipped too. The **dirty gate** is what the [scaffolded](#scaffolding-publishable-new) `.gitignore` is for: it already excludes both, so `validate` and `run` see a clean tree. A hand-assembled repo whose `.gitignore` omits that line goes dirty at `validate` and fails `run` — while its `code_hash` is unchanged, the fixed skip set being what keeps those files out of the hash whatever an ignore file says. What the omitted line bears on is that gate; what an ignore file bears on in the hash is [the other three cases](#how-the-three-are-computed). Where it lives also decides how it is *found*: the first two are [registered through an entry point](#creating-a-plugin-publishable-plugin-new), and the third — which is installed nowhere and distributed to nobody — is discovered by path from the fixed layout, making its `@register_template` argument the whole of its registration. Every non-dunder-stemmed file the fixed layout finds under `templates/` is read as one of these local templates, and one that fails to load — see [`E-TEMPLATE-LOAD`](#errors-validate-reports) — is a fault rather than a silence; a genuine helper file is named with the `__`-prefix `__init__.py` already uses to be skipped.
...
**What it receives is the parsed document — a plain mapping, and deliberately not the [dot-access node](#the-importable-surface) a step's `cfg` is.** The two methods read different things: `aggregate` reads a condition's **resolved** values, where every parameter has one and a path that misses is a typo, so a node that refuses it is right; `validate` reads **declarations**, and an absent optional block is the answer rather than a mistake. So the idiom is a mapping's, and it never refuses an absence:
...
def validate(self, config) -> list[str]:
    ...
    if not units.get("holdout") and not folds:
        return ["this experiment type fits a model, so it needs a `data.units.holdout` "
                "or a `{kind: fold}` repeat to fit on"]
    return []
```

`or {}` rather than a `{}` default at each step, because a declared-and-empty block parses as `None` as readily as an absent one — `statistics:` with nothing under it is a legal file. Nothing here can raise, which is what makes a cross-block rule safe to write: a template's `validate` raising is reported as [`E-TEMPLATE-RULE`](#errors-validate-reports) *"raised while validating"*, a diagnostic that names the exception rather than the rule and tells the user nothing they can act on.
...
Columns are whatever the step [recorded](#units-the-thing-being-measured) plus every declared unit attribute, so `units.truth` and `units.pred` are the same shape whichever of the two supplied them. A declared attribute is carried through **unchanged** rather than averaged: it comes from the roster rather than from an execution, so unlike a recorded numeric column it has nothing to collapse across a unit's repeats. It is a column here and nothing else — never a metric, and never something `aggregate`'s own return can collide with, which a [recorded column of that name is](#validation). **The names a recorded column may not shadow are the union over the roster, not one unit's.** An attribute only some units carry is still a declared attribute — `validate` admits it by reading the same union, on the same *this column exists* reading a table header gets — so a step recording a column of that name earns `E-STEP-KEY-COLLISION` whichever unit it was writing, and whether or not the first unit resolved happened to carry it. A recorded column that is not a number is carried too, and collapses the way [§ What isn't a repeat](#what-isnt-a-repeat)'s attribute rule already does — "Attributes constant within a key collapse to that value with no rule needed" — extended to a value the step itself recorded: across a unit's repeats it collapses to that value when every repeat agreed and to `None` when they did not, because a non-numeric column has no average to compute and [`data.units.measurements.collapse`](#the-per-unit-tables) governs a declared measurement collapse rather than a repeat one. [Disclosed as a warning](#warnings-core-reports) when repeats disagree, it too is a column here and nothing else — never a metric, for the reason [§ Statistical reporting](#statistical-reporting) gives. A column has one entry per row, in row order, reading [`None`](#the-per-unit-tables) where that unit recorded nothing — which is what makes `pearsonr(units.pred, units.truth)` above pair each unit's prediction with *its own* truth even when the two columns are ragged in different rows. A name no row holds is `E-STEP-COLUMN-UNKNOWN` rather than an empty column.
...
**A `parameter_spec` path is `head.leaf`, exactly two dotted segments — not a constraint on a value, so it earns no row in the closed constraint vocabulary above.** A one-segment path (`"reference_frame"`) or a three-segment one (`"a.b.c"`) is rejected at the same moment a bad `default=None` is: when the class that declares `parameter_spec` loads, in `BaseTemplate.__init_subclass__`, before `@register_template` or anything else ever sees it. That is earlier than `Param.__init__` can check it — the path is the *caller's* dict key, never anything passed to `Param` — so this constraint cannot join that closed vocabulary, whose every entry is something `Param.__init__` itself receives. The remedy is one word: rename. A template declaring `"reference_frame"` fixed this by renaming it to `"frame.reference"` — one segment to nest under, one leaf name. `E-TEMPLATE-PARAM-PATH` names the fault; [§ Errors `validate` reports](#errors-validate-reports) carries the row. This only checks a `parameter_spec` declared as a class attribute — every template this document shows — since `__init_subclass__` sees the class, not an instance; a template that instead builds `self.parameter_spec` inside its own `__init__` still meets `materialize._parameters_block`'s own guard, kept in place for that residual.
...
What a control arm actually designates is a comparison, and a comparison between two arms is a [`statistics.contrasts`](#contrasts-claims-that-arent-condition-vs-baseline) entry naming both conditions by label — `of: arm=treatment`, `against: arm=control` — which says the same thing without claiming one arm is the reference for the whole run. **This build computes that delta**, unpaired, by `welch_t_over_units`/`unpaired_percentile_over_units` and their `_clustered` forms (see [§ Allocation](#allocation-within-subjects-or-between-subjects) and [§ Statistical reporting](#statistical-reporting)) — the `Estimate` route returned by a `summary` step, or two runs joined in a `study`, remains available for anything core does not compute (an interaction, a conditional estimator).
...
| `fold` | `k` — an integer ≥ 2, or `all` for leave-one-out — plus optional `stratify_by` (`str \| None`, naming **one** attribute: a fold balances its folds on a single attribute, unlike the list form [`holdout.stratify_by`](#a-fixed-holdout-split) and [`assign.<axis>.stratify_by`](#allocation-within-subjects-or-between-subjects) each take) |
...
**The failure fraction `run` enforces is against the run level.** A threshold checked per execution would fire on a small fold long before the cohort was in any trouble, and a threshold checked per condition would let a systematically broken fold hide inside nine healthy ones. Run-level is the number that decides whether the complete-case result is interpretable, so that's the one with a threshold on it. The fraction is **units that failed in at least one execution, over `provenance.units.n`** — distinct units on top, the whole resolved roster underneath. Counting failures instead of units would charge a cohort of 240 with 30 failures when one unit failed in every one of 30 executions, and dividing by the executions' summed `resolved` would move the denominator every time you added a fold. Distinct-over-roster is the only reading that means the same thing under `seed`, under `fold`, and under a group axis, which is what a threshold you set once has to do. It's evaluated as the run goes rather than at the end, because failures only accumulate: once the fraction is past the threshold no later execution can bring it back, so core stops there. **What it produces is [`partial`](#what-status-means-and-when-a-run-keeps-going), not `failed`** — this sentence said otherwise in two places until 2026-09-14, and the code has never agreed with either.

**A unit fails when a COMPLETED execution was handed it and settled nothing. An execution that raised contributes none.** A completed execution that recorded and skipped nothing about a unit has demonstrated that the unit admits no answer, which is attrition and accumulates. A raised execution has demonstrated nothing about any unit: the step died, and the same unit may settle perfectly in the next repeat. Counting it is not the conservative reading, it is the wrong one, and it is expensive — one dropped connection raised one execution of forty-five, all 300 of its units were charged against a 600-unit roster, and a metered plan stopped nineteen hours in with eleven executions still to run. This is the same rule the fraction already applied to a step whose *every* execution crashed, which never counts as recording units at all; what changed is that it now holds for a step that crashed in some. **What it deliberately leaves uncovered**: a step that raises part-way through its units on every execution trips no threshold and runs the plan to its end. Core cannot tell that from an outage — both are a raise after some rows — and the alternative is a second threshold with no declared home. Per-execution failures are still recorded and `report` surfaces any execution whose completion rate is an outlier against its condition — an early-stopping signal without a second threshold to tune.
...
class Step(BaseStep):
    def run(self, cfg, io):
        ...
        self.rng                  # this execution's random.Random — see "Randomness"
...
| | Is | Reaches |
|---|---|---|
| `self.rng` | A `random.Random` built from that seed, at every scope | Whatever you draw from it |
...
**`numpy.random.seed` reaches the legacy global `RandomState` and nothing else.** A step that calls `numpy.random.default_rng()` gets a generator seeded from OS entropy, so a pipeline written the modern way is *unseeded* by a core that seeds the legacy global — which is exactly the failure `self.rng` exists to close. Take the generator core hands you rather than constructing one — **and note that it is a `random.Random`, not a NumPy `Generator`**, so the methods are `random()`, `gauss()`, `uniform()`, `choice()` and `shuffle()`; `self.rng.normal(...)` is an `AttributeError`. For NumPy's own API, seed one from [`derive_seed`](#a-step-that-partitions-needs-a-seed-and-derive_seed-is-where-it-comes-from): `numpy.random.default_rng(self.derive_seed("my-purpose"))`.
...
**A metric block's `value` is a number, and its `n` counts what it was computed from.** A column in `units.parquet` earns a block in `aggregated` when it carries a number for at least one unit — [§ The per-unit tables](#the-per-unit-tables)' read-side rule — so a column that is non-numeric for every unit reaches `aggregate`'s table and publishes no block, while a column that is a number for some units and `None` for others publishes a block over the units that carried one. **What makes that honest rather than convenient is the count**: the block reports the number of units that contributed, not the run's `completed`, because an interval over five values published beside a `completed` of two hundred is a claim about a precision it does not have — and it is a claim no later reader can catch from the record. A [`None` cell](#templates-where-parameters-are-defined) counts the unit out of that one metric and out of nothing else, and [`W-STATS-COLUMN-THIN`](#warnings-core-reports) is what says so on stdout when the contributing count falls below `limits.min_reported_n`. This is the `units` basis's own rule; it says nothing about the `repeats` rows below, whose point estimate is whatever the step itself returned, nor about a `summary` step's own `Estimate`, which core stores rather than computes and which carries `reported: true` for exactly that reason.
...
**A contrast's interval is its own construction, never a difference of the two sides' intervals.** Differencing those would discard the covariance that [pairing exists to exploit](#allocation-within-subjects-or-between-subjects) — in the worked example it would report an interval several times wider than the delta itself — so the delta is computed from the per-unit differences, or from a resample that draws for both sides at once. Which row below applies follows from whether the contrast is [`paired`](#allocation-within-subjects-or-between-subjects), whether the metric is a column or a derived one, and whether [`weight_by`](#weighted-samples) is declared:
...
width while looking exactly like a narrow one. It is a property of the draw's **content**, not of how
...
**This refusal is not uniform with every per-condition percentile draw, and the asymmetry is
...
**A unit enters the intersection whether or not it carries the metric's own column — so a *derived* metric's `n_paired` can exceed the number of units that influenced the difference, and a *recorded column*'s cannot.** Nothing about the intersection is per-metric: it is the units both conditions completed, narrowed only by a `within` stratum. A [derived metric](#templates-where-parameters-are-defined) is computed over the whole [unit table](#the-unit-table-is-the-inference-base), so a unit's row is part of what that metric was computed from whether or not the row carries any particular column. A [recorded column](#steps-and-artifacts) narrows again — to the units carrying a real number for *that* column — so its count and its own per-unit difference vector are the same set by construction. The two figures can therefore differ inside one contrast block, and both are right.

**It is the honest figure either way, because it is exactly the pool the interval rests on.** `paired_percentile_over_units` draws over the intersection and recomputes the derived metric on each draw, so narrowing the published count without narrowing that pool would put a count beside an interval computed over a different set of units — the one fault a contrast record must not have. `n_paired` is a fact about the intersection rather than a count of contributing values, the same shape `n_paired_effective` and `n_paired_clusters` already take.
...
scale:  4,800 unit-executions (20 executions × 240 units handed to each)
would create 20 step directories under /secure/results/cohort-pilot/run_.../
  shared/step01_load_cohort
  conditions/00_baseline/step02_fit_model
  conditions/00_baseline/seed17/step03_analyze
  conditions/00_baseline/seed42/step03_analyze
  …                                       (the three remaining seeds, one each)
...
and 9 fixed files in that directory:
  ...
  run.yaml
  sweep.yaml
the /secure/results/cohort-pilot/latest pointer is repointed too; it sits beside
  the run directory rather than inside it
...
**The step-directory count has a rule beside it so you can re-derive it rather than trust it: one directory per planned (step, condition, repeat) triple.** For the worked example that is 20 — `shared/step01_load_cohort` (1, a `run`-scope step running once for the whole sweep) + `conditions/<c>/step02_fit_model` × 3 (3, one per condition) + `conditions/<c>/<seed>/step03_analyze` × 3 × 5 (15, one per condition per repeat) + `summary/step04_compare_methods` (1). A step's scope is what decides which of the three components its directory carries, so a repeat count that multiplies quietly shows up here as directories rather than only as an execution count. **The same rule is why the execution count is 20 and not 15, and the header line's `×` is a description rather than a derivation**: `3 conditions × 5 repeats` names the *shape* of the sweep, while the number after it is the length of the planned execution list — one entry per (step, condition, repeat) triple, so the `run`-scope step's one execution and the `summary`-scope step's one are in it alongside the three condition-scope ones and the fifteen repeat-scope ones. A pipeline of one `repeat`-scope step *would* print 15 here. The [`demo`](#what-demo-walks-you-through) walkthrough is **not** that shape — its three steps sit at `run`, `condition` and `repeat` scope, so `dry-run` prints 19 there for the same reason it prints 20 here — which is why that stop names both figures: 15 is the sweep's arithmetic, `3 × 5`, and 19 is the length of the planned execution list.

**What the list omits, and why, stated where you read it rather than only in the [CLI reference](#cli-reference).** The artifact *files* inside a step directory are not listed and cannot be: their names are the arguments a step passes to [`io.write`](#steps-and-artifacts), [`io.record`](#the-unit-table-is-the-inference-base) and `io.append`, which are strings in step code and are declared nowhere in the config — and [`design-principles.md` § Greenfield only](design-principles.md#greenfield-only) is the rule that core never inspects the body of your Python. Two of them are conditional on facts that do not exist until the run happens at all: `units.parquet` is written only when a step recorded at least one row, and `measurements.parquet` only when one passed `measurement=`. So this is an honest partial manifest rather than an approximate full one, which is why the transcript says so on its own last lines.
...
| Hash | Covers | Answers |
|---|---|---|
| `code_hash` | Content hash of `src/**` and `templates/**` — [which files](#how-the-three-are-computed) | Was the code identical? |
...
**`code_hash` is `sha256` over the sorted list of `(repo-relative path, sha256 of file contents)` pairs** across `src/**` and `templates/**`, taken from the **working tree**. Which files under those two trees it hashes has four cases, and they are enumerated once — in the table below, which every other passage that mentions the rule links to rather than restating. Which repo those paths are relative to is decided by [the walk-up rule](design-principles.md#whose-git-hash-is-this): it starts at the path the command was given, so a hash never depends on the directory you invoked it from. Not a git tree hash, and the reason is [`draft`](#draft-runs): a git tree hash needs the content staged, so a dirty tree has none, and `run` and `draft` would then be computing two incomparable things. Reading the working tree means both compute the same function, and a draft's hash is a real fingerprint of the code that ran — just one no commit reaches, which is exactly what `draft: true` records. File modes, timestamps, and paths outside the two trees are all excluded, so a `chmod` or a touched file changes nothing.

| A file under `src/**` or `templates/**` is | Hashed? |
|---|---|
| in the fixed skip set — `__pycache__`, `.git`, `.ruff_cache`, `.mypy_cache`, `.pytest_cache` as a path component, or the suffix `.pyc`/`.pyo` | **no**, whatever git says — including when it is tracked |
| tracked | **yes**, even when it matches an exclude pattern |
| untracked and not excluded | **yes** |
| excluded by one of the repo's own committed rules — a `.gitignore` at any depth — or by `.git/info/exclude` | **no** |

The fixed skip set is applied **first**, so git is never asked about a path that is skipped anyway. Everything else is git's own answer to *is this path excluded?*, which is why a tracked file matching an exclude pattern stays hashed: git does not skip it, so neither does the hash. The alternative — a second implementation of git's pattern rules living here — would disagree with the [dirty gate](#templates-where-parameters-are-defined) the first time a pattern was subtle, and the two disagreeing is the defect this rule exists to remove.

**What the narrowing removes, and what it leaves.** The exclude question is asked with `-c core.excludesFile=`, a command-line override that beats a global config entry, the XDG default `~/.config/git/ignore` that no config entry names, and a repo-local `.git/config` entry — every route a `core.excludesFile` can take — so a `core.excludesFile` on one machine cannot change what another machine hashes: a rule that does not travel with the tree cannot define the tree's identity, which is this section's opening sentence applied to git's own configuration. The same override is applied to the [dirty gate](#errors-core-raises), so the two ask one git rather than two. **CONTROLLER RULING M (2026-08-23)** made this an override rather than an environment neutralization, and only of `core.excludesFile` and (for the gate) `status.showUntrackedFiles` — the two settings that decide *which files are considered*. Nothing else is touched: see the paragraph below for why.

**What it leaves is not a single residue, and the mechanism is worth stating rather than the list.** `check-ignore` answers from the **working tree**, not from the commit, so *whatever exclude rule the tree holds when the command runs* decides — a `.gitignore` you have edited and not committed decides, and so does one you have not committed at all. `.git/info/exclude` decides too, and is the one no flag can disable: it lives in one clone and travels with nothing. So a run can still publish a `code_hash` that a clone of the same commit does not reproduce, and the [dirty gate](#errors-core-raises) does not close the gap: it is scoped to `src/**` and `templates/**`, so a `.gitignore` **inside** either tree is caught as uncommitted while the repo root's — the usual place the rule lives — is not. A file whose hashing status has to be the same everywhere is a file you **commit**, and so is the rule that excludes it.

**The dirty gate moved with the definition, and that is a second behaviour change to `run` (2026-08-23).** The gate is `git status --porcelain` over the two trees asked with `-c core.excludesFile=` and `-c status.showUntrackedFiles=normal`, so a repository that relied on a rule the tree does not carry to keep a file out of `src/**` or `templates/**` now **fails `E-CODE-DIRTY` where it used to run**: a global, system, or repo-local `core.excludesFile`, the default `~/.config/git/ignore` no config entry names, and a global or repo-local `status.showUntrackedFiles = no`, which used to hide every untracked file from the gate while the hash folded them in. The remedy is the same one this section already gives — commit the rule.

**CONTROLLER RULING M (2026-08-23) narrowed the mechanism itself.** The fix round that closed Ruling F's and Ruling L's pins shipped a total neutralization (`GIT_CONFIG_GLOBAL`/`GIT_CONFIG_SYSTEM=/dev/null`, applied through the environment) rather than the two `-c` overrides above, and disclosed the cost this paragraph now replaces. Ruling F's own ground is about *exclude* rules specifically — a rule that does not travel with the tree cannot define the tree's identity — not about every git setting, and the environment form neutralized far more than that: **`core.fileMode`, `core.autocrlf` and `core.symlinks` are deliberately left alone**, because they answer *how* git compares a file's content or mode, not *which* files the gate or the hash consider, and they are legitimately machine-local — they exist because filesystems differ. Wiping them made an **unedited** tracked file read as dirty: measured, with the repo's own `core.filemode` unset, a `chmod +x` on a tracked file was clean under the machine's config and dirty under the old, total neutralization. Blocking a run on a correct tree is a worse failure than the one Ruling F closes, and it would have landed on the users least able to diagnose it — a Windows checkout with `autocrlf`, a network mount without executable bits. The `-c` overrides also retire, rather than merely narrow, the specific `safe.directory` consequence the fix round filed: that filing was a false negative caused by the neutralization itself, not by `safe.directory` in general — a legitimate global `safe.directory` allowlist entry, which would ordinarily clear the dubious-ownership warning, was discarded by `GIT_CONFIG_GLOBAL`/`SYSTEM=/dev/null` along with everything else in the user's global config, so the check failed anyway and `_git`'s `check=False`/`strip()` convention read the resulting empty, non-zero-exit answer as "clean". Measured: with `GIT_TEST_ASSUME_DIFFERENT_OWNER=1` and a real global `safe.directory` entry for the repository, the old mechanism still produced `fatal: detected dubious ownership` (rc 128, read as clean); the `-c`-only form does not touch global or system configuration at all, so the same allowlist entry is honoured and `git status` succeeds normally (rc 0). A repository with **no** `safe.directory` entry at all still hits the same `fatal:` and the same silent-clean reading under either mechanism — that is `_git`'s own `check=False` convention swallowing a returncode at a call site never designed to refuse, and it is unrelated to Ruling F, L or M; it is not filed here because it is not this mechanism's regression.

**What the rule's arrival costs, stated rather than marked (2026-08-22).** Nothing in a record says which definition produced its `code_hash`, and `run.yaml`'s `schema_version` is deliberately not bumped for it: [`reuse_from`](#reuse_from-addresses-an-artifact-not-the-design-that-produced-it) refuses a record declaring any other version, so a bump would refuse every record already on disk — strictly worse than an unmarked value change. The carrier is the one that already exists. Core's own version is pinned in `uv.lock`, which is precisely why `code_hash` covers the repo's two trees and nothing else, and `provenance.environment.uv_lock_hash` records that lockfile. **The change is visible as a dependency change and is not visible as a hash-definition change**, and a reader comparing two runs across it reads the `uv.lock` row as covering it.

**Exactly one hash's value moves: `code_hash`.** Every record field that carries it:

| Field carrying the moved hash | Where |
|---|---|
| `code_hash` | [`run.yaml`](#the-two-files), top level |
| `run_id` | `run.yaml`, and the run **directory's name** — the short form is the digest's first seven hex characters |
| `provenance.upstream[].code_hash` | **copied** out of an upstream record, so one record can carry two definitions |
| the bundled copy of each of the above | [`study add`](#building-one) copies a run's `run.yaml` into `runs/<name>/` verbatim |
| the `latest` pointer's target | it names the run directory |

The last two rows carry nothing of their own, which is why they need no separate account: a bundled record is a verbatim copy, and the pointer names the directory, so whatever is true of `code_hash` and `run_id` is true of both. The other ten figures a record carries are unmoved, and they are enumerated rather than counted so a reader can check them: `parameters_hash`, `input_manifest_hash`, the per-file `sha256`s in [`manifest/input.json`](#manifestinputjson--what-was-read), `uv_lock_hash`, `units_hash`, `allocation_hash`, [`apparatus.hash`](#the-apparatus-core-can-only-observe), the [design digest](#what-auto-derives-from), the copied upstream `parameters_hash`, and every [derived seed](#what-auto-derives-from).

**So, plainly.** Two runs of the same config over the same data at the same commit, on either side of this change, publish different `code_hash` values and different `run_id`s whenever the repo carries an excluded file under the two trees — which the [scaffolded](#scaffolding-publishable-new) `.gitignore` makes the common case rather than the exotic one. [`diff`](#operation-commands) prints `code_hash DIFFERS` for identical code, and `report study.yaml` over a bundle spanning the change prints [`W-STUDY-CODE-HASH-MISMATCH`](#warnings-core-reports), whose message names three candidate causes and still names three: a build boundary is not among them, and widening that row would document a transient.

**The sharpest form of the cost, which no marker mitigates.** A run that consumes an older upstream through [`io.reuse_from`](#lineage-between-runs) publishes **one record carrying two hash definitions** — its own under the four cases above, and `provenance.upstream[].code_hash` copied verbatim from the older one — with nothing in the record distinguishing which is which.
...
**`parameters_hash` and the [design digest](#what-auto-derives-from) are `sha256` over a canonical JSON rendering** of what each covers: UTF-8, object keys sorted, no insignificant whitespace, floats in shortest round-trip form. Values are hashed **as the config declares them**, with nothing materialized on the way in: a hand-trimmed config and the file `init` wrote are two declarations, so they hash differently and [`diff`](#operation-commands) names the key that differs.
...
**`input_manifest_hash` is content-addressed where content is available, and this is a VALUE CHANGE (2026-08-26).** `manifest_hash` used to digest the manifest whole, `mtime` included, so a bare `touch` of one input file moved the published hash while [`verify_manifest`](#manifestinputjson--what-was-read) — the change *detector* the run itself uses — correctly reported that nothing had changed. The detector was content-addressed where it could be and the hash never was. Now the digest is taken over the manifest with **each hashed file's `mtime` dropped**, and nothing else changes: a hashed file contributes its `size` and its `sha256`, a file the policy left unhashed contributes all three unchanged — size and mtime are the only evidence there is for one of those, and dropping the mtime would make the hash *weaker* than the detector rather than as strong. `policy` stays in the digest, because over an `input_dir` whose every file is an index file `hash_index` and `hash_all` produce identical per-file evidence for two different claims. **What the row above does and does not now say**: *relative paths + content hashes … at the depth `data.input_manifest_policy` asks for* is what the digest covers for a file the policy hashed, and the row is silent about a file it did not — for that one the digest still rests on size and mtime, because there is nothing else to rest it on. So this change does not make the digest content-addressed under `none`; it makes it content-addressed exactly as far as the policy reached.

**What moves, enumerated rather than counted.** `provenance.input_manifest_hash` moves for every past run whose policy hashed at least one file — `hash_all` always, `hash_index` whenever it named one — and the [bundled copy](#building-one) of that field follows it, being a verbatim copy of the record. **Under `none` nothing moves at all**: no file is hashed, so the digest's input is byte-identical to what it was. **[`manifest/input.json`](#manifestinputjson--what-was-read) itself does not move** — the manifest still records every file's `mtime`, because `verify_manifest`'s size-and-mtime fallback reads it — and neither do the per-file `sha256`s inside it. Nothing derived from `code_hash` moves: [`run_id`](#run-identity), the run directory's name and the `latest` pointer are built from that hash alone. Nor do `code_hash`, `parameters_hash`, `uv_lock_hash`, `units_hash`, `allocation_hash`, [`apparatus.hash`](#the-apparatus-core-can-only-observe), the [design digest](#what-auto-derives-from), any [derived seed](#what-auto-derives-from), or [`identity.json`](#identityjson--the-claims-a-run-makes-before-it-executes), which carries no `input_manifest_hash` at all. `provenance.upstream[]` copies an upstream run's `code_hash` and `parameters_hash` and never its manifest hash, so no record can carry two manifest-hash definitions the way one can carry two `code_hash` definitions.

**Why the new value is better, said plainly rather than left to be derived.** The question the row asks is *was the data identical?*, and an mtime is not data — a restore from backup, a `cp -p` that missed, a mount that renormalized timestamps all moved a published identity claim for bytes nobody touched, and a reader comparing two runs saw `input_manifest DIFFERS` with no way to tell that from a real edit. The carrier of the change is `uv.lock`, the same one [the `code_hash` change](#how-the-three-are-computed) uses, and the same limitation applies: **being *able* to derive a hash-definition change from a lockfile digest is not being told**, so [`diff`](#operation-commands) prints `input_manifest DIFFERS` for identical data across this boundary, exactly as it prints `code_hash DIFFERS` across the other. No `schema_version` bump and no marker key, for the reason given there — a bump would refuse every record already on disk.

**One shipped command's behaviour moves with it.** [`resume`](#resuming) compares the recorded manifest against a fresh one and refuses [`E-RESUME-INPUT-MOVED`](#errors-core-raises) when the two hashes disagree. A bare `touch` between the crash and the resume used to trip that gate and discard a whole crashed run — a stop over a fact the run's own detector called clean. It no longer does. The refusal's own three faults are untouched: content that moved, a file that was added, and a `manifest/input.json` that cannot be read.
...
**A changed fact fails the run, with no policy knob.** Same line as a dirty code tree, a lockfile mismatch, or an input file that moved: data gathered under two different apparatus states is not one dataset, and a flag to permit it would only ever be used to paper over the moment a result stopped being interpretable. The run is marked [`status: failed`](#what-status-means-and-when-a-run-keeps-going), and its record is kept rather than discarded — **once there is a record to keep.** A change caught before the first execution runs has none: `run.yaml` is never written and `latest` is never repointed, the same shape a config that fails validation leaves, so that one case exits [`1`](#exit-codes-and-diagnostics) rather than `4`, with the probe ledger alone on disk. Restarting under a changed apparatus is a new run — [`resume`](#resuming) refuses it too, and the ledger keeps both observations so the evaluable earlier period is still reportable, when one exists.
...
That output is the point of the whole mechanism: two runs with identical code, parameters, and data that disagree, and a record that says why. Both header lines read `completed` — the pairing worth noticing rather than wondering about: an apparatus that moved between two *finished* runs, neither of them cut short by the mid-run gate above, is exactly the case this row exists to catch.

**`diff`'s `apparatus` row is a strictly wider net than the run-time gate above, and deliberately so — they answer different questions.** The gate asks whether the apparatus moved *during* this run, so `null → value` is a fact answered for the first time and passes. `diff` asks whether two runs measured through the same apparatus, so that identical transition is a real difference between them and prints `DIFFERS` — a `null` on either side of an arrow here is exactly the case the gate tolerates, not a failure, and reading it as one is the mistake this sentence exists to head off.

**There is a third reading of that same comparison, and it sides with the gate.** [`reproduce`](#reproducing-on-another-device) writes a record's own facts out to `configs/<name>/apparatus.expected.json`, and the next `run`'s **first** probe round compares against it: a contradiction is [`E-APPARATUS-UNEXPECTED`](#errors-core-raises), exit `1` before any execution and `4` once there are results to keep. Stated explicitly so it is not folded into either of the two above. The gate asks *did the apparatus move during this run*; `diff` asks *did two runs measure through the same apparatus*; the expectation file asks *is this run measuring through the apparatus that record did* — three questions, and on `null → value` the expectation file answers the gate's way rather than `diff`'s. That is not a compromise: the file is read through the **same** `Observations.record`-then-`.changed` pair the gate uses, over a second `Observations` seeded from the record, so it inherits the gate's tolerance by construction rather than by a rule written twice. A reproduction that gets an answer the original never got has pinned something the original didn't, which is more evidence rather than less — and a *value* contradicting a recorded value is the substitution this whole mechanism exists to catch. The file is untracked, editable, and sits beside the config; editing it is visible, and the run you produce records the facts you actually observed rather than the ones you claimed.
...
**The two forms are told apart by `Path(locator).is_absolute()` and by nothing else** — not a separator test, not whether the directory happens to exist. A relative path carrying a separator is neither form and is refused (`E-UPSTREAM-LOCATOR`) rather than silently resolving under `output_dir` as if it were a `run_id`, which would otherwise let `provenance.upstream` record a value that is not one. **Both** forms are checked against the same containment invariant `output_dir`/`input_dir` already enforce — [code and data never share a repo](design-principles.md#code-and-data-never-share-a-repo) — refused as `E-UPSTREAM-REPO-CONTAINED` when the locator resolves inside the git repo the config path was given against, with the route named in the refusal itself: copy the upstream outside the repo, or address it by `run_id` under `output_dir`. One check, run on both branches, against the *resolved* path in each: a relative locator inherits `output_dir`'s own already-checked containment for an ordinary subdirectory and not for a **symlink** under it — core writes one itself as `<output_dir>/latest` — so exempting the relative form would let a symlink there read an in-repo run with no check at all. The absolute form resolves symlinks before that check runs, which is what makes `<output_dir>/latest` usable and honest — `provenance.upstream` records the resolved `run_id`, never `latest`, the same rule that a path is a fact about one machine. The relative form cannot follow `latest`: `latest` is not a `run_id`, and the directory it names holds a record whose own `run_id` disagrees with the locator as given, which is `E-UPSTREAM-RUNID-MISMATCH`. That asymmetry is a property of the two forms rather than a bug in one of them — they differ in exactly the way their arguments differ, one being an identity and the other a location.

**Only a `run`- or `summary`-scoped upstream step is addressable, which is the same argument as the missing selector above applied one level down.** The only two artifact locations that carry no condition and no repeat coordinate are `shared/` (a `run`-scoped step) and `summary/` (a `summary`-scoped step), so a read never has to parse an upstream's condition numbering to land somewhere. A step recorded under a condition is refused as `E-UPSTREAM-STEP-SCOPED`, whose message names the republish route the code sample above already shows — including for an *unswept* upstream, where that step in fact writes to one unambiguous place, directly under the run directory: the refusal stays broader than the ambiguity on purpose, because an upstream that is unswept today and gains a sweep level tomorrow would otherwise relocate that same artifact under a downstream read that named it before the level existed, with every hash still matching. A step the upstream's record names nowhere is `E-UPSTREAM-STEP-UNKNOWN`; one that's addressable but whose recorded status isn't `completed` is `E-UPSTREAM-STEP-INCOMPLETE` — a refusal rather than a read, because an artifact from an execution that didn't finish is exactly what "lineage is recorded, not resolved" exists to stop being silently consumed. A `partial` or `failed` upstream *run* is not refused on that account alone — the named step's own recorded status is what this check reads, so a sibling condition's failure doesn't make an otherwise-complete step's artifact unreadable.

**The upstream's own `run.yaml` is read and validated before any of the above.** No file at the resolved run directory is `E-UPSTREAM-RECORD-MISSING` — the run never finished, or the directory isn't a run directory at all. Present but unparseable, not a mapping, or without a `run_id` is `E-UPSTREAM-RECORD-UNREADABLE`. A `schema_version` this build doesn't read is `E-UPSTREAM-RECORD-VERSION` — pin the `publishable` version that wrote it.

**`name` is checked for containment the same way any other artifact read already is** — a `..` escape, an absolute path, or a symlink leading outside the step's own published directory, never the shape of the path itself, so a documented name like `programs/gpt-4.1__seed29.json` still reads. That escape is `E-UPSTREAM-NAME`, the lineage-specific twin of `E-ARTIFACT-NAME` rather than that code itself, on the precedent `E-RESOLVER-SWEPT-PARAM` already sets for one mechanism serving two faults. A name that passes containment but names nothing the upstream published under that step is `E-UPSTREAM-ARTIFACT-MISSING` — one code for "the step directory holds no such artifact" regardless of whether the step ran at all, since the remedy is identical either way.

**A run that consumes nothing records `upstream: []`, unconditionally — the key is always present, never omitted**, so a reader can tell "consumed nothing" from "this build predates the key" by testing membership rather than truthiness.
...
**Republishing is also what makes a run's own conclusions recomputable later, and that is decided before the run.** The section above reads as a design pattern — build a two-run study on purpose — and it is one, but it carries a second consequence that is easy to meet only by accident. A `summary` step's *return* is what the record reports; an `io.write` is what a later run can *read*. So if an analysis defect is found after a run completes, whether it can be re-analysed without re-paying for every metered call depends entirely on whether that step republished the material its own numbers rest on. It is not recoverable afterwards: `reuse_from` refuses a condition- or repeat-scoped step for the reason just given, `run.yaml` is immutable by design, and `reproduce` re-executes. **A run whose summary step published nothing is a run whose conclusions can only be corrected by running it again.** Core cannot warn about this — it never reads a step body — so it is a judgement to make while writing the step, and the question to ask is: *if the analysis in this step turns out to be wrong, what would someone need in order to redo it?*
...
**A hypothesis that reaches no verdict records why, as `unevaluable`.** `observed: null` alone cannot say which fault it was, and the three have different remedies:

| value | what happened | the remedy |
|---|---|---|
| `metric_absent` | the observation site resolved and the run produced no metric of that name | a typo, or a template whose `aggregate` never derived the key |
| `condition_unresolved` | `compare: {to: constant}` named no `condition` on a run whose sweep resolved several | name one in `compare.condition` |
| `value_absent` | the site resolved and the metric block **is there**, carrying no number — its `delta` or `value` is `null` | whatever the run's own [`findings`](#the-two-files) say nulled it |

**`value_absent` is the one a reader is most likely to misread, because everything looks present.** The block the hypothesis names exists, sits beside healthy-looking counts, and carries `null` where its number goes. The commonest way to reach it is a [contrast whose two sides share no units on a metric a template derived](#warnings-core-reports) — core will not recompute an `aggregate` on each side's own resampled table, so `delta` is `null` and `W-STATS-CONTRAST-UNPAIRED-DERIVED` says so — and the second is a metric core computed as `null` because it is undefined on that condition's units, a `sensitivity` where nothing was positive. Core does not re-derive which of those it was: the finding that nulled the number is already in the record, and a second authority on one question is how two answers eventually disagree.

The key is **absent** when there is a verdict rather than present-and-null, on the same rule [`weighted_by`](#weighted-samples) follows: a key that is there and empty reads as *considered, came back nothing*, which is the opposite of what it means. Everything `validate` can see from the declarations alone is refused before a run — an unknown step (`E-HYPOTHESIS-METRIC`), an unknown contrast (`E-HYPOTHESIS-CONTRAST`), an unknown condition label (`E-HYPOTHESIS-CONDITION`) — so these three are what is left, and all three need the run to have happened. [`W-HYPOTHESIS-UNEVALUABLE`](#warnings-core-reports) reports the same fact while the config is still in front of you, naming every metric the step *did* record; the field is what survives into the record for whoever reads it later.

**The verdict records which number it compared, because the [hypothesis family](#sweeps-and-repeats) is corrected separately from the sweep's.** `family_size` and `family` carry it in the same idiom every other family uses, with a single breakout key because a hypothesis family multiplies nothing: it counts the confirmatory hypotheses whose observations core computed, where a sweep's family counts comparisons × metrics. A reader can check the level without re-deriving it, exactly as `family` beside a `vs_baseline` delta is auditable rather than asserted. Correction reaches a verdict only through a bound: a hypothesis evaluating on `observed` compares a point estimate, which has no α to adjust, while one evaluating on `ci95_lower` or `ci95_upper` reads the corrected bound at the level *this* family implies — for a baseline comparison, a declared contrast, or a [`compare: {to: constant}`](#what-a-hypothesis-is-tested-against) observation, each of which core builds a correctable `Member` for wherever the metric's own raw interval exists — under `fdr_bh` the family implies no per-comparison level for any member at all, so there is no bound to read there regardless of what is compared. Under the methods that do imply one, the single exception is a recorded column carried under both `weight_by` and `cluster_by`: core builds no `Member` for that combination, so a bound test on it reads `supported: null` under a real correction method rather than a corrected bound it has no evidence to build — still counted into `family_size`, never silently decided on the raw, uncorrected bound. `verdict_evaluated_on` names which of the three the comparison actually used — spelled out rather than echoing the config's `evaluate_on`, since a record field one letter from a config field is a typo waiting to be read as agreement. So a verdict is never a number a reader has to reconstruct from `evaluate_on` plus a correction rule.
...
family implies, which can be `null` for four unrelated reasons: a family too large for the resample's
...
size — a [`compare: {to: constant}`](#what-a-hypothesis-is-tested-against) observation whose metric
has no raw interval at all leaves it unmet the same way any metric with nothing to correct does, and
one on a recorded column carried under both `weight_by` and `cluster_by` leaves it unmet because core
builds no correctable `Member` for that combination, whatever the sample size; every other
constant-referenced observation gets a `Member` the same way a baseline comparison or a declared
contrast does, wherever the metric's own raw interval exists. The
shape of `observed` is how a reader tells the two routes apart: the first writes `observed:
null` — there
...
hypotheses:
  ...
  - id: above_chance
    ...
    direction: greater
    threshold: 0.0
    evaluate_on: observed                     # a bound would work here too; see below
```
...
**A constant-referenced hypothesis joins `family_size`, and gets a correctable `Member` the same way a `vs_baseline` delta or a declared contrast does, wherever the metric's own raw interval exists** — carrying whatever evidence that raw interval was built from, at the same rank every other family member sorts into. Under `correction: none` its `observed` carries no `ci95_corrected` key exactly like every other member of a family nothing was attempted for. Under `holm` or `bonferroni`, evaluating it on `ci95_lower` or `ci95_upper` reads a real corrected bound, **except in two cases**: a metric with no raw interval at all has nothing to correct, and a recorded column carried under both `weight_by` and `cluster_by` has no `Member` shape to build one from even though its raw interval exists. Either way the bound test reads `supported: null` and `ci95_corrected: null`, the same honest gap [`W-STATS-CORRECTED-THIN`](#warnings-core-reports) reports for a family too large for its resample's draws to support. **`fdr_bh` is not a third case, because it is not about this form at all**: Benjamini-Hochberg implies no per-comparison level, so *no* member of *any* kind — a `vs_baseline` delta and a declared contrast included — carries a corrected bound under it, and a bound-evaluated gate reads `supported: null` there whatever it compares. That is a prior condition on the whole promise above, not an exception a constant reference earns. `evaluate_on: observed` is unaffected either way — correction only ever tightens a bound, never the point estimate — which is why the example above reads on `observed`.
...
Which corrected interval a bound test uses follows the family rules unchanged for a baseline comparison, a declared contrast, or a [`compare: {to: constant}`](#what-a-hypothesis-is-tested-against) observation, wherever the metric's own raw interval exists — `report` shows both, and a confirmatory gate reads the corrected one when a correction is declared. Two standing exceptions: a metric with no raw interval at all, and a recorded column carried under both `weight_by` and `cluster_by`. Neither gets a correctable `Member`, so under `holm` or `bonferroni` there is no corrected interval to read at all, and a bound-evaluated gate comes back `supported: null` rather than a decision on either interval. `fdr_bh` is not among them — it withholds a corrected interval from every member equally, which is the family rules unchanged rather than an exception to them.
...
The bundle is self-contained and device-independent: every reference is relative, `run_id` is a label rather than a locator, and nothing resolves through the original output storage. Zip it, attach it as supplementary material, or deposit it and cite the DOI. `publishable report study.yaml` renders it offline, cross-checking that runs claiming the same code really share a `code_hash` (a notice, not a refusal, when they do not — [`W-STUDY-CODE-HASH-MISMATCH`](#warnings-core-reports)) — and the same for [`provenance.apparatus.hash`](#the-apparatus-core-can-only-observe) ([`W-STUDY-APPARATUS-MISMATCH`](#warnings-core-reports)), since "these runs used one deployment" is a claim a paper makes and a bundle can check — flagging any [draft](#draft-runs) runs, and collecting every declared [hypothesis](#pre-registration) into one table. Both checks compare the two records' own figures and compute neither: `report` never calls `hashes.code_hash` or `apparatus.apparatus_hash`, on the same grounds [`diff`'s own `apparatus` row](#the-apparatus-core-can-only-observe) treats that hash as authoritative rather than reconstructible. Both notices print to **stderr**, never stdout: for this command stdout is the artifact itself — `publishable report study.yaml > report.md` is the ordinary way to use it — so a diagnostic ahead of the render would otherwise land inside the file a reader redirects to.
...
**`provenance.environment.os` and `.hardware` are not in this table, and travel unredacted, on purpose.** Redaction here exists for identity and credentials — a path that can encode a cohort name, a node name that often names an institution or a person. A platform string and a core count name neither: they answer *what platform produced this number*, which is provenance a bundle reader needs, not exposure a bundle reader shouldn't have. The same distinction the table already draws for `input_manifest_hash` surviving without its path.
...
**This table redacts host identity, and says nothing about participant identity — `allocation.json` is where that gap shows.** It is the one run-directory artifact that is a list of unit identities — "which patients were in the treatment arm" — and this section, and the table above, have never named it. **A bundle never carries `allocation.json`, and no option is added to put it there.** `study add` takes a run's `run.yaml` **path**, not a run directory, and the bundle holds that file and `study.yaml` — run records, never a copy of a run directory — so `allocation.json`, a run-*directory* artifact, is not reachable from the argument the command takes; admitting it would mean `study add` reading a sibling of its own argument, quietly turning a record-copying command into a directory-copying one. It is also the one run artifact that is a list of unit identities, in the artifact most likely to be deposited publicly, so adding it would create the gap and then require a redaction rule to close it. `provenance.allocation` and `provenance.allocation_hash`, the two fields of it that do reach `run.yaml`, are a bare filename and a digest — neither disclosing membership, so a bundle already carries a *commitment* to the split without carrying the split. The route for a reader who wants to verify the split, rather than merely trust the hash, is named rather than left implicit: `allocation.json` is one file the author can attach as supplementary material beside the bundle, or hand over with the run directory, and `allocation_hash` is what makes that transfer checkable — a reader who holds both can prove the file is the one the run used, which is exactly the posture this section already takes for `input_manifest`.
...
| `publishable generate` (`g`) | built | generator, name, generator args (`experiment` accepts `--plugin`; `report` accepts `--format`) | `experiment` \| `step` \| `template` \| `report` |
| `publishable init` | built | `--template`, `--name`, `--input-dir`, `--output-dir`, `[--plugin]` | Alias for `generate experiment` |
| `publishable study new` | built | bundle path, `--title` | Creates an empty study bundle, outside any experiment repo |
| `publishable study add` | built | bundle path, run.yaml path, `--as <name>` | Copies a run record into the bundle under that name, with host paths redacted |
...
| `publishable reproduce` | built | run.yaml or config path | Prepares a runnable checkout and **stops**. Given a run record — a run directory's own `run.yaml` or a [bundle member](#building-one) — it clones the recorded remote into a derived directory, checks out the recorded commit, verifies `code_hash`, restores the recorded environment, writes the config back with both data paths blanked, writes `configs/<name>/apparatus.expected.json` when the run measured through an [apparatus](#the-apparatus-core-can-only-observe), and prints what is left for a person to supply. Given a config it does the same from step 4 onward, in the repository the config sits in, and names what it did not verify. It executes nothing and writes into no run directory — see [Reproducing on another device](#reproducing-on-another-device) |
...
| `publishable docs` | built | *(none)* | Rewrites every `publishable:begin/end` managed region this repository's README declares, from the repository itself, and **names on stdout** every managed region it did not find — a README missing some of the four is the ordinary state of a project scaffolded before those regions existed, not a fault. Touches no byte outside a region. Walks up from the working directory, having no path argument. A README holding none of the four, one whose markers cannot be paired, and a missing README are refusals with their own codes |
| `publishable list-templates` | built | *(none)* | Every template name this build knows, in name order, with its provenance (`core`, `local`, `installed`) and its provider. Prints the full `parameter_spec` for a core or project-local template; an installed one prints its distribution and one line saying its spec is **not readable** ([`E-TEMPLATE-INSTALLED-UNSUPPORTED`](#errors-validate-reports)) — core resolves an installed name from package metadata without importing the package, so there is no class here to read a spec off. Walks up from the working directory, having no path argument; outside a repository it lists core's and every installed claim and **says on its own line** that no project-local `templates/` was searched. A name two providers claim is [`E-TEMPLATE-COLLISION`](#errors-validate-reports), the same answer `validate` gives |
...
| `1` | The thing you asked about is wrong — a config that fails [validation](#validation), a `resume` whose hashes moved, or a changed [apparatus fact](#the-apparatus-core-can-only-observe) caught before the first execution ran, which leaves nothing to mark `failed` at all |
...
**`5`'s *"a clone or `uv sync` that failed"* clause has its readers**, and no code is minted for them either: [`reproduce`](#reproducing-on-another-device) reaches `5` at three sites — a `git clone` that failed, a recorded commit the clone does not hold ([`E-REPRODUCE-COMMIT-UNREACHABLE`](#errors-validate-reports)), and `uv sync --locked` failing in either operand form — and the first and third report `E-IO-FAILED`, since a path core was handed and could not use is what that identifier already means. Naming the sites rather than a count is deliberate: `5` was never unread, [`E-APPARATUS-RAISED`](#errors-core-raises) having reached it from four surfaces already, so *"first reader"* would be right about the clause and wrong about the code.
...
|---|---|---|
| 1 | *(`demo` itself)* | Writes 240 synthetic units to `~/publishable-demo-data/input/`, scaffolds `templates/correlation.py` — a [project-local template](#templates-where-parameters-are-defined), because core's `generic` defines no `aggregate` and so derives nothing — plus `src/correlation_pilot/` and `configs/correlation-pilot/config.yaml`, then `git init` and a first commit. Explains why the data is [outside the repo](#why-not-in-the-repo) |
...
The `<!-- publishable:begin ... -->` regions are **managed**: a generator that populates one leaves everything outside it untouched. `generate experiment` merges a row into the experiments table and any new `required_env` into the credentials table; `generate template` writes its parameter table into the `templates` region. **A region is rewritten, never created**: a generator whose region the README does not declare says so and writes nothing, which is the state of every repository scaffolded before these four regions existed. Most scaffolding tools write a README once and leave it; here the README is meant to be part of what a reviewer reads to reproduce the work, which is why the regions are regenerated from the live specs rather than typed once.
...
|---|---|---|---|
| `experiment` | built | `publishable g experiment cohort-pilot --template generic --input-dir ~/data --output-dir ~/results` | A fully-populated `configs/cohort-pilot/config.yaml`, `src/cohort_pilot/` (thin `experiment.py` declaring step order, plus `steps/` with one **working** starter step). Refuses if either path resolves inside the repo, and refuses a name whose `-`-to-`_` form cannot be a package and a class (`E-GENERATE-NAME`). Merges a row into the README's managed `experiments` region and any of the template's `required_env` into its `credentials` region, and says so when the README declares neither |
| `step` | built | `publishable g step cohort-pilot analyze` | Next-numbered `src/cohort_pilot/steps/step03_analyze.py` with a `BaseStep` stub, registered in order. Takes a name that can be a module and a class — an identifier, not `__`-prefixed — refusing anything else with `E-GENERATE-NAME` **before it writes**, since registering the step rewrites an `experiment.py` this command did not create |
...
`publishable report` renders any run without configuration — **which run it is**, the condition table, the deltas, the hypothesis verdicts, the attrition. The first of those is a `Run` block carrying `run_id`, `status`, the three hashes and the commit, copied out of the record as written: a rendered report that cannot say which run produced it is the one thing a renderer could get wrong about a tool whose whole pitch is handing someone one file. It is the single-run sibling of the `run_id`/`status` line [a bundle already gives every member](#building-one). What it can't produce is the figure your paper actually needs, because that is domain work. `generate report` writes a subclass into your repo for it:
...
`run` is the parsed `run.yaml` and `io` is a `ReportIO` — `conditions`, `repeats`, `read_condition`, `read_input` — the read half of the `StepIO` a [`summary` step](#steps-that-need-every-condition) gets, so an override can reach any condition's artifacts. `generate report`'s `--format` writes that attribute and does nothing else — the class is the source of truth from then on, exactly as `--input-dir` seeds a config field it doesn't afterwards own. It **adds and reorders sections; it cannot change a number** — the values it renders come from `results`, which core computed and [attributed](#estimate-carries-your-interval-without-core-claiming-it) before the renderer existed. That's the same line [`aggregate`](#templates-where-parameters-are-defined) sits on, one stage later: a renderer decides what a reader sees, never what the record says.

`self.section(title, body=...)`'s `body` is a `str` — one markdown block, rendered verbatim (escaped inside a `<pre>` under `html`) — or a mapping, rendered as a table. **The table-body convention, used by all four standard sections and available to an override's own**: a mapping carrying a `rows` key whose value is a `list` renders one table row per list entry, with one column per key any entry carries, ordered by each key's *first* appearance across the rows. A mapping with no such `rows` list renders instead as a single two-column `key`/`value` table over the mapping's own items — for an override handing over an arbitrary summary without building a row list itself. Anything `body` could be besides these two shapes is refused; see [`E-REPORT-BODY`](#errors-validate-reports).
...
1. Reads `provenance.git.remote` and `.commit` from the file — no existing checkout needed. **Which file is decided structurally, never by name**: a mapping carrying `run_id` is a run record, whether it is a run directory's own `run.yaml` or a [bundle member](#building-one) such as `main.run.yaml`; a mapping carrying `experiment_type` and no `run_id` is a config, and takes the form described below. Anything else is `E-REPRODUCE-OPERAND`, naming what a usable operand would be — a run *directory* included, since [`resume`](#resuming) is the command that takes one and giving `reproduce` the same operand for the opposite action is worth refusing by name. A whole [`study.yaml`](#building-one) is `E-REPRODUCE-BUNDLE`, which lists the bundle's members so you can name one.
...
3. Verifies the checked-out tree's `code_hash` matches the recorded one, and on a difference prints **which input moved and the causes it cannot separate** — never a cause. `E-REPRODUCE-CODE-HASH` names both digests and the count and list of files folded, and then names the closed candidate set: the code at that commit really is different (a rewritten or force-pushed history); the record predates a redefinition of *which* files are hashed, which no key in `run.yaml` can date; or this machine's git materialized the tree differently — `core.autocrlf`, which the clone neutralizes, or a tracked `.gitattributes`, which it may not, because that file is tracked and so is part of what the commit says the tree is. Naming a closed set is not a verdict; picking one would be. **The checkout is kept**, because a stop that discards its own artifacts leaves nothing to look at. A [`draft`](#draft-runs) record declines this verification instead of failing it: the record says the tree was never reachable from any commit, so there is nothing to compare against, and that is the one cause `reproduce` does name — because the record names it.
4. Restores the environment and runs `uv sync --locked`, failing loudly on lockfile mismatch. Plugin versions come along automatically. **The recorded `uv_lock_hash` is the authority and neither source is preferred silently**: the run directory's own byte copy of `uv.lock` is the preferred carrier and is used when its digest matches the record (`E-REPRODUCE-LOCKFILE-EDITED` when it doesn't — the copy was edited after the run, so it is not the environment those numbers came through); when that copy isn't reachable — from a [bundle member](#building-one), which carries no `environment/` directory — the commit's own committed lockfile is used **if and only if** its digest matches, and `E-REPRODUCE-LOCKFILE-UNREACHABLE` otherwise. Every absence and every disagreement is printed rather than resolved quietly, including a `pyproject.toml` in the run directory that differs from the commit's, which is reported and not gated on. A record whose `uv_lock_hash` is `null` — the state [`W-ENV-UNLOCKED`](#warnings-core-reports) warns about at run time — is `E-REPRODUCE-UNLOCKED`, with the checkout kept: that warning's own sentence coming true.
5. Writes the embedded config to `configs/<name>/config.yaml`, re-serialized from `run.yaml`'s own `config` mapping rather than copied out of the run directory, with `data.input_dir` and `data.output_dir` blanked and marked `# REQUIRED: set to your local copy`. **The write is self-checked**: [`parameters_hash`](#three-hashes) covers neither `metadata` nor those two paths, so hashing the written file reproduces the recorded figure exactly unless the re-serialization lost or retyped something — and if it differs, `E-REPRODUCE-CONFIG-WRITEBACK` names both figures rather than leaving you a config that isn't the one that ran. It also reports whether the commit's own committed config matches the recorded parameters, which is a real difference to know about and not a refusal. **The inline comments [`init`](#cli-reference) wrote are lost**, and the transcript says where they still live: `run.yaml`'s `config` is the parsed mapping, so no comment survived into the record for one to come back out of. When the run had an [apparatus](#the-apparatus-core-can-only-observe), it also writes `configs/<name>/apparatus.expected.json` — the recorded facts, which the first probe is checked against. `reproduce` writes that file once and never rewrites it — an existing one is `E-REPRODUCE-EXPECTED-EXISTS` rather than a replacement, because replacing it would discard a decision core cannot make for you; you may edit it, and that asymmetry is the whole design — see below.
...
**Two facts step 2 states in one clause each, spelled out here because each carries a refusal.**
**Derived is not unique**: a second `reproduce` of the same record derives the same name, and rather than
overwrite a checkout you may already have edited it refuses — `E-REPRODUCE-DEST-EXISTS`, naming the
directory — and it refuses a destination resolving inside a git repository too
(`E-REPRODUCE-DEST-IN-REPO`), for the reason [`input_dir`/`output_dir` may never](#the-one-config-file).
The destination is created under the directory you are standing in. **And *"the only git operation"* is
right about what you typed, while the count underneath it is two** — a clone and a detached checkout, each passing
`-c core.autocrlf=false` on the **clone**, which writes the setting into the new repository's own
`.git/config`, so the detached checkout that follows needs no flag of its own and passes none — the tree
is materialized the way the recorded hash expects either way. A recorded commit the remote no longer holds — what a
...
to clone from at all: `E-REPRODUCE-NO-REMOTE`, naming the recorded commit so you can check it out
yourself in a repository you already have.
...
**Given a config rather than a `run.yaml`, there is nothing to clone and nothing to check against.** A config names no commit, no remote, and no recorded hash, so the first three steps have no input at all: what `reproduce` does with one is prepare the checkout it is already standing in, from step 4 onward — `uv sync --locked`, the `.env.example` copy, the list of what needs values, and the same closing instructions, with step 5 moot because the config is already where it would have been written. The repository is found by walking up from the **config path**, never from the working directory, and a config outside every repository has no project to restore: [`E-GIT-NO-REPO`](#errors-core-raises). Nothing is created and no destination is derived, so step 2's two refusals are unreachable from this form. **It names the three things it did not verify** — `code_hash`, the input manifest, and the apparatus — rather than reporting a match it never made, which is the same posture [`diff`](#cli-reference) takes for a config side. There is a fourth absence, and it is printed too: step 4's ranking has no authority here, because a config records no `uv_lock_hash` for the repository's own lockfile to be checked against, so that lockfile is named and reported as unranked rather than silently trusted. That form is for the case where someone handed you a config and a repo instead of a record; the `run.yaml` form is the one that reproduces a *result*, and it is the one to prefer whenever both exist.
...
│   ├── cli.py                 # dispatch, and `list-templates`, which is a listing of what the
│   │                          #     registry already answers rather than a surface of its own
│   ├── scaffold.py            # `new`
...
│   ├── docs.py                # `docs`: rewrites managed README regions from live specs
│   ├── demo.py                # `demo`: the guided arc — builds the worked example, then walks it
│   ├── readme_templates/      # the shipped README/CITATION.cff/LICENSE scaffolds, as files
│   ├── sourceimport.py        # importing one file by path, recompiled rather than served stale
...
│   │                          #     read_condition, reuse_from, exists/resumed/recorded_keys;
│   │                          #     ReportIO (read half, for report overrides): conditions,
│   │                          #     repeats, read_condition, read_input; ResolverIO (read-only,
│   │                          #     for resolvers): read_input
...
├── tests/
├── examples/generic/                    — not yet built
└── pyproject.toml
```
