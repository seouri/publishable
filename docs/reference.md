...
| No git repository walking up from the path given, raised once, in `provenance.find_repo_root`. Six paths reach it. `cli._prepare_run` (the run command's phases 1-5, since H9a) and the `generate`/`init` dispatch call it uncaught, surfacing it at `main`'s `except PublishableError` printer on stderr, exit `1` — the creation commands walk up from **`Path.cwd()`** rather than a path argument, being the commands with none to walk up from — the one place `CLAUDE.md` § Invariants' "a walk-up from the path the command was given, not from the working directory" does not apply, and a reader comparing the two without that sentence would conclude one is wrong. Two sites catch it **by code**, as the pass branch of a check of their own: `validate._check_data` returns quietly ("not in a repo, so inside-the-repo doesn't arise"); `study._refuse_if_in_repo` catches it as the pass branch of its own in-repo refusal. Two more catch it **by type**, testing no code: `validate.validate_config`, under a bare `except ContractError`, leaves `repo_root` `None` so local template discovery is skipped while every other check still runs; `cli._preloaded_experiment`, under `except Exception`, returns `None`. This is why a config outside every repository prints `✓ config valid` and refuses only at `run` | `ContractError` · `E-GIT-NO-REPO` |
...
| `publishable reproduce` | built | run.yaml or config path | Clones the recorded commit into a new checkout and prepares it to run — see [Reproducing on another device](#reproducing-on-another-device) |
