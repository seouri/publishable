...
| [`study new`](#building-one)'s bundle path resolves inside a git repository — a study is a publication artifact, sibling to the paper, and belongs beside the manuscript rather than inside the code repo it cites (§ Why not in the repo). Implemented as the walk-up `provenance.find_repo_root` performed over `input_dir`/`output_dir` **succeeding**: `find_repo_root` raises `E-GIT-NO-REPO` rather than returning `None` when none is found, so that one code is caught as the pass branch and every other `ContractError` it might raise propagates | `E-STUDY-IN-REPO` |
