...
| Under `method: by_attribute`, the column `data.units.holdout.from` names does not resolve to exactly `train` and `test` over the resolved roster — the same set equality [`validate` reports](#errors-validate-reports) for the identical fault, raised here because `units.arms_of` is the single authority for a column-read partition and a run that validates first meets it at the draw rather than a second time | `ContractError` · `E-DATA-HOLDOUT-VALUES` |
| Under `method: random`, a realized holdout draw leaves the train or the test side empty — the same fault [`validate` reports](#errors-validate-reports) for the unstratified, unclustered case, raised here for the two draws `validate` cannot check itself: a stratified draw, whose per-stratum sizes only the run's own draw produces, and a clustered draw, whose empty side depends on the seed the same way a clustered arm assignment's does. `units.holdout_for` is the single authority, so the run meets it where it draws | `ContractError` · `E-DATA-HOLDOUT-EMPTY` |
