...
| `experiment_type` names a template neither core nor this project's own [`templates/`](#templates-where-parameters-are-defined) registers — the message also disclaims an installed plugin's, but no entry point is resolved in this build, so there is no plugin registry yet for a name to be found in or missing from | `E-TEMPLATE-UNKNOWN` |
