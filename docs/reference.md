...
| `src/**` or `templates/**` carries uncommitted changes when a command that executes starts. Checked inside `command_run`, before the empty-file-list gate below and before any step runs, for the reason [§ Three hashes](#three-hashes) gives: a `code_hash` folded over a tree that is still moving names no state anyone can return to. **Untracked files count** — the check is `git status --porcelain` over the two trees, which reports them — while **a file the repo's own rules exclude does not**. That question is asked with the machine's own git configuration neutralized, [exactly as the hash asks it](#how-the-three-are-computed) and through the same two constants, so **the gate and the hash honour one exclude chain**, which they did not before: a file a global `core.excludesFile` hides is now dirty here *and* hashed there, rather than clean here and hashed there — a recorded identity covering a file no clone of the commit contains. **It is the rules that converged, not the file sets.** The hash additionally drops [the fixed skip set](#how-the-three-are-computed) whatever git says, so a tracked, modified `src/pkg/step.pyc` is dirty at this gate and is read by no hash. One emit site | *(no exception; a `Collector` diagnostic)* · `E-CODE-DIRTY` |
...
**What the narrowing removes, and what it leaves.** The exclude question is asked with the user's global and system git configuration neutralized, so a `core.excludesFile` on one machine cannot change what another machine hashes — a rule that does not travel with the tree cannot define the tree's identity, which is this section's opening sentence applied to git's own configuration. The same neutralization is applied to the [dirty gate](#errors-core-raises), so the two ask one git rather than two.
...
**The dirty gate moved with the definition, and that is a second behaviour change to `run` (2026-08-23).** The gate is `git status --porcelain` over the two trees asked with the same configuration neutralized, so a repository that relied on a rule the tree does not carry to keep a file out of `src/**` or `templates/**` now **fails `E-CODE-DIRTY` where it used to run**: a global or system `core.excludesFile`, a `core.excludesFile` in the repo's own `.git/config`, the default `~/.config/git/ignore` no config entry names, and a global `status.showUntrackedFiles = no`, which used to hide every untracked file from the gate while the hash folded them in. The remedy is the same one this section already gives — commit the rule. **The neutralization is total, not exclude-only, and that reaches one further class**: a setting deciding *how* a tracked file is compared rather than which files are considered is discarded too, so a global `core.fileMode = false` — or `core.autocrlf`, which Git for Windows writes to the system config — can report a file nobody edited as modified. Measured on 2026-08-23: with the repo's own `core.filemode` unset, a `chmod +x` on a tracked file is clean under the machine's config and dirty under the neutralized question. Most repositories are unaffected because `git init` writes `core.filemode` into the repo's own `.git/config`, which outranks the global one. One consequence points the other way and is filed rather than fixed here: `safe.directory` is read from global and system configuration too, so on a repository git considers dubiously owned the gate now gets no answer and reports the tree **clean** — measured, and the run stops two phases later anyway, because the hash asks its own question through the same neutralization and that one fails loudly as [`E-CODE-FILE-LIST`](#errors-core-raises). No record is published either way; what is filed is a gate that answers *no* to a question it could not ask.
