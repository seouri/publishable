...
| The table `data.units.from` names has no data rows, or the `glob` it names matches no files under `input_dir`, or a [resolver](#where-units-come-from) yields no units. **Dual-surface for a resolver source**: a resolver is nondeterministic user code that may yield nothing on a later call, so `command_run` can meet this same raise at `run` and reports it there too, through the same redacting collector `E-RESOLVER-RAISED` uses | `E-UNITS-EMPTY` |
| Two resolved units share the same `data.units.key` value. **Dual-surface for a resolver source**: a resolver is nondeterministic user code that may yield a duplicate key only on a later call, so `command_run` can meet this same raise at `run` and reports it there too, through the same redacting collector `E-RESOLVER-RAISED` uses | `E-UNITS-KEY-DUPLICATE` |
...
| A [`@register_*` argument](#creating-a-plugin-publishable-plugin-new) disagreeing with the entry-point key that named it. The entry point is the registration and the decorator is a declaration checked against it, so two spellings of one name with no rule for which is canonical is refused rather than resolved — the [defaults-file argument](#there-is-no-separate-defaults-file) again. Reached at `validate` as well as at `run` for a **resolver source**: `validate` dispatches a declared resolver to resolve the roster, and dispatching means loading — so a resolver's disagreement is `validate`'s finding whether or not the run it would have executed ever starts. An [apparatus probe](#the-apparatus-core-can-only-observe)'s dispatch goes through the identical check, but **only at `run`**: `validate` calls no probe at all, so a probe's disagreement is never `validate`'s finding, only `apparatus._probe_for`'s, met wherever `command_run` resolves the declared probe to call it | `ContractError` · `E-PLUGIN-DECORATOR` |
| An entry point whose module raises while importing, or calls `sys.exit()` at module scope. `SystemExit` is a `BaseException` and so needs its own `except` — a plugin building an `argparse` parser at import would otherwise end the command with the plugin's own exit code and no diagnostic at all. The same containment `E-PLUGIN-DECORATOR` describes — reached wherever a plugin is imported, which is a [resolver source](#where-units-come-from)'s dispatch, at `validate` as well as at `run`, and an [apparatus probe](#the-apparatus-core-can-only-observe)'s dispatch, but — unlike a resolver's — **never at `validate`**, which calls no probe to reach it; a probe's own top level raising this way is met only at `run`, through `apparatus._probe_for`. The fault names the entry point and the distribution rather than the module, since a distribution is what a reader uninstalls or pins | `ContractError` · `E-PLUGIN-LOAD` |
