...
| [`report study.yaml`](#building-one) — the bundle form. Not yet built: the run form (`report run.yaml`) is, and this is `report`'s OWN refusal for the form that isn't, at exit `1` — never the CLI's generic "specified but not built" diagnostic (exit `2`, reserved for an invocation fault), which would be a false claim about a command whose run form really is built. Retired wholesale, not narrowed, the day the bundle render lands | `E-REPORT-BUNDLE-UNSUPPORTED` |
