...
| No git repository walking up from the path given, raised once, in `provenance.find_repo_root`. Six paths reach it. `command_run` and the `generate`/`init` dispatch call it uncaught, surfacing it at `main`'s `except PublishableError` printer on stderr, exit `1` — the creation commands walk up from **`Path.cwd()`** rather than a path argument, being the commands with none to walk up from — the one place `CLAUDE.md` § Invariants' "a walk-up from the path the command was given, not from the working directory" does not apply, and a reader comparing the two without that sentence would conclude one is wrong. Three sites catch it **by code**, as the pass branch of a check of their own: `validate._check_data` returns quietly ("not in a repo, so inside-the-repo doesn't arise"); `validate.validate_config` catches it under a bare `except ContractError`, leaving `repo_root` `None` so local template discovery is skipped while every other check still runs; `study._refuse_if_in_repo` catches it as the pass branch of its own in-repo refusal. `cli._load_experiment_for` catches it under `except Exception`, returning `None` for `validate_config` to report as `E-ENTRYPOINT-IMPORT` instead. This is why a config outside every repository prints `✓ config valid` and refuses only at `run` | `ContractError` · `E-GIT-NO-REPO` |
