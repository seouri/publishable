...
| A `data.units.assign` block's `method` is `blocked` and its declared, non-`"auto"` `block_size` cannot fill each arm's share — **one code over the whole malformed-value family**: it is not a positive `int` at all (`0`, a negative number, `2.5`, `"four"`, `null` — a bare `range(0, len(keys), block_size)` in `units.assignment_for` either raises a bare `ValueError` or silently produces no blocks, and neither is a diagnostic), or it is a positive `int` that is not a whole multiple of `ratio`'s sum — [§ Allocation](#allocation-within-subjects-or-between-subjects): "an explicit value must be a whole multiple of that sum so every block fills each arm exactly." `ratio`'s sum is read the same way `E-DATA-ASSIGN-RATIO`'s own accept path establishes it usable — an absent, empty, or malformed `ratio` (already `E-DATA-ASSIGN-RATIO`'s own fault to report) falls back to the level count, equal allocation's implied sum. Not reported for `block_size: "auto"` itself, which resolves to twice that sum at draw time and so is a whole multiple of it by construction; not reported under `random`, where `block_size` means nothing — `from`'s own reason, the discriminator deciding which of a block's other fields are read. Skipped when the axis's declared `levels` do not resolve to a non-empty list of strings, `sweep.groups`'s own shape fault. Reported beside `E-DATA-ASSIGN-DRAWN`, not instead of it — the two say different things about the same block | `E-DATA-ASSIGN-BLOCK-SIZE` |
  ...
  units:
    ...
    assign:
      arm:                         # keyed by axis name — one block per declared axis
        ...
        block_size: auto           # blocked only; auto = twice the ratio's sum, or twice the level count when ratio is {}
