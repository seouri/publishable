...
This is the rule the code already enforces; it was reachable only by colliding with it before now. Where it costs the most is a column that is genuinely `str` for some units and a number for others — refused today, at the cost of the whole execution's record for a step that already ran. **The read and the write are decided separately, and differently.** On the read side, [`aggregate`](#templates-where-parameters-are-defined) publishes a column as a metric only when every value carried for it is a real number — one string in a column of floats costs that column its own metric block and nothing else, over however the mixture arose. The column still reaches `aggregate`'s table regardless, where a template that knows what the mixture means can read it directly. On the write side, `E-STEP-RETURN-TYPE` is **not** loosened: a genuinely mixed `str`-and-number column written across repeats still refuses, because loosening it would make a column's published-or-not status depend on the data a run happened to produce rather than on its config — one run of a config would publish a metric and the next would not, with no diagnostic distinguishing the two.
...
**A metric block's `value` is a number.** A column in `units.parquet` earns a block in `aggregated` only when every value carried for it is a real number — [§ The per-unit tables](#the-per-unit-tables)' read-side rule — so a column that is `str` for even one unit, or that a repeat-level disagreement [collapsed to `None`](#templates-where-parameters-are-defined), reaches `aggregate`'s table but publishes no metric block of its own. This is the `units` basis's own rule; it says nothing about the `repeats` rows below, whose point estimate is whatever the step itself returned, nor about a `summary` step's own `Estimate`, which core stores rather than computes and which carries `reported: true` for exactly that reason.
