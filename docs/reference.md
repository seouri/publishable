...
Each of those raises a [`ContractError`](#errors-core-raises) carrying its own identifier, except the hypothesis row — which is a fact about the record rather than an error in the step, and so is recorded rather than raised.
...
**What a step returns is the same flat mapping of scalars `io.record` takes.** Keys are strings; values are `str`, `int`, `float`, `bool`, or `None`; nothing nests. `run.yaml` is the file a paper attaches and a reviewer opens in ten years, and [`per_repeat` is *exactly what the step returned*](#the-two-files) — so a returned value that plain YAML can't hold honestly is one the record can't hold either, which is the argument above about never pickling for you, arriving at the other end of the step. Nesting is refused for a second reason: a [hypothesis](#pre-registration) addresses a metric as `step03_analyze.r`, and a nested return leaves that path with more than one reading. The rule is identical for the values a template's [`aggregate`](#templates-where-parameters-are-defined) derives, with one exception at one scope — an [`Estimate`](#estimate-carries-your-interval-without-core-claiming-it), whose whole purpose is to carry a structure core will render as an interval.
...
**`self.rng` is the execution's default stream; [`derive_seed`](#a-step-that-partitions-needs-a-seed-and-derive_seed-is-where-it-comes-from) is how you get a second one** — draw from the first until you need to name the second. Where there is no repeat, the two are the same construction: the execution's seed is `derive_seed` of the step's own name, so `self.rng` there is exactly `default_rng(self.derive_seed("<this step>"))`, and any purpose but that one lands on a stream of its own. It sits beside `self.condition` and `self.repeat` rather than in `io` because it is a fact about this execution rather than about where its artifacts go — but unlike them it never raises, since every execution has a seed and there is no scope at which it would have nothing to hand over.
