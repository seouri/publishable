...
| A `sweep.grid`, `sweep.paired` or `sweep.ablate.override` value cannot be rendered into a condition label — every value `label_for` renders, and a label is also a directory segment, so a value carrying `/` would resolve outside the condition directory. A `sweep.baseline` value is exempt, since it is never rendered into one, and so is a `sweep.sample.ranges` bound, the *drawn* value being what a label carries | `E-SWEEP-VALUE-UNNAMEABLE` |
