...
| `statistics.resample.stratify_by` names a value that varies within a cluster under a declared `data.units.cluster_by` — the same constancy [§ Clustered units](#clustered-units) already requires for `fold`, `holdout` and `assign`, checked here through `units.stratum_varies_within_cluster`, the single authority *Fold strata survive clustering* and *Holdout strata survive clustering* also read. A resample draws whole clusters, so a cluster carrying two stratum values can be dealt to neither. **Dual-listed, the shape `E-DATA-WEIGHT-INVALID` has**: `stats.percentile_over_units_clustered` raises the same code at run time, under the same message, because it is handed the stratum vector and the membership map directly and cannot silently choose one over the other | `E-STATS-RESAMPLE-STRATIFY-VARIES` |
...
| `stats.percentile_over_units_clustered` given both `strata` and `membership`, where a cluster's units disagree about the stratum they carry — the composition [§ Clustered units](#clustered-units) states, "a stratum must be constant within a cluster", taken again for `resample` rather than invented a second time. Dual-listed the same way `E-DATA-WEIGHT-INVALID` above is: `validate` reports the declaration form of this from the roster, through `units.stratum_varies_within_cluster`, and this is the run-time half, raised because a public function handed both vectors directly cannot silently pick one | `ContractError` · `E-STATS-RESAMPLE-STRATIFY-VARIES` |
