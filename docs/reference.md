...
| A `name` that escapes the directory it is checked against — the step's own on a write, or the target step's on [`io.read_condition`](#steps-that-need-every-condition) or [`io.read_upstream`](#step-scope) — an `io.append` onto anything but `.jsonl`, or an extension [no writer claims](#steps-and-artifacts) handed an object that isn't `bytes` or `str`. Three emit sites for the escape alone: a write checks against the calling step's own directory, and the two readers each check against the directory of the step named in the call — one row because the check and the remedy are the same containment rule at all three, never a check on the path's shape | `ArtifactError` · `E-ARTIFACT-NAME`, `E-ARTIFACT-APPEND`, `E-ARTIFACT-UNWRITABLE` |
