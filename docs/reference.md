...
corner — which branch such a contrast takes is recorded in `docs/superpowers/spec-defects.md` rather
than promised here — but `n_paired_clusters` itself is decided even there: it travels beside `n_paired`
whether or not that branch could compute a `delta`, because both are facts about the paired
INTERSECTION rather than about whether a construction ran, and `n_paired` already keeps that shape
when a draw is too thin or degenerate to publish.
