...
size — and a [`compare: {to: constant}`](#what-a-hypothesis-is-tested-against) observation on a
recorded column carried under both `weight_by` and `cluster_by` leaves it unmet because core builds
no correctable `Member` for that combination, whatever the sample size; every other constant-referenced
observation gets a `Member` the same way a baseline comparison or a declared contrast does. The
...
**A constant-referenced hypothesis joins `family_size`, and gets a correctable `Member` the same way a `vs_baseline` delta or a declared contrast does** — carrying whatever evidence the metric's own raw interval was built from, at the same rank every other family member sorts into. Under `correction: none` its `observed` carries no `ci95_corrected` key exactly like every other member of a family nothing was attempted for. Under a real method (`holm`, `bonferroni`, `fdr_bh`), evaluating it on `ci95_lower` or `ci95_upper` reads a real corrected bound, **except for a recorded column carried under both `weight_by` and `cluster_by`**: no `Member` shape exists for that combination, so a bound test there reads `supported: null` and `ci95_corrected: null`, the same honest gap [`W-STATS-CORRECTED-THIN`](#warnings-core-reports) reports for a family too large for its resample's draws to support. `evaluate_on: observed` is unaffected either way — correction only ever tightens a bound, never the point estimate — which is why the example above reads on `observed`.
...
Which corrected interval a bound test uses follows the family rules unchanged for a baseline comparison, a declared contrast, or a [`compare: {to: constant}`](#what-a-hypothesis-is-tested-against) observation — `report` shows both, and a confirmatory gate reads the corrected one when a correction is declared. The standing exception is a recorded column carried under both `weight_by` and `cluster_by`: core builds no correctable `Member` for that combination, so under a real correction method there is no corrected interval to read at all, and a bound-evaluated gate comes back `supported: null` rather than a decision on either interval.
