...
Two warnings the fixture also produced are left out of the table above because they are properties of the fixture, not of these designs: `W-DATA-CLUSTER-UNDECLARED` on `age_band`, an artifact of the synthetic table's own three-band shape rather than of `growth-screen`'s real roster; and, on E5 specifically, `W-REPL-DETERMINISTIC`, an artifact of the demo entrypoint's step not declaring `nondeterministic = True` where the real `step03_screen` is described as declaring it (§ E4 — Reasoning-effort operational benchmark). Neither bears on `E-DATA-HOLDOUT-UNSUPPORTED`, `E-DATA-RESOLVER-UNSUPPORTED`, or `E-DATA-WEIGHT-CONTRAST`.

**Under the table-roster substitution, the generous count is three, not six.** E1, E2 and E5's transplanted blocks validate with zero errors. E3, E4 and E6's transplanted blocks validate equally clean under the same substitution — the same zero-error result — **but still cannot run**: each reads its frozen compiled program through `io.reuse_from`, and `grep -rn "reuse_from" src/publishable/` returns nothing in this build. A block validating clean is not a config that can execute when the method its steps call does not exist; § E5 — Binary-output repeatability and the `io.reuse_from` paragraph above are what this depends on, not anything H3d touched. So "six of nine, one substitution away" is not the number this measurement supports — three are one substitution away from `validate`, and the other three need that same substitution plus a method this build does not have. The shortcut runs' confirmation set has the identical `io.reuse_from` dependency (it reads the development run's fitted artifact), so C1–C3 do not become three more even once `E-DATA-WEIGHT-CONTRAST` lifts — see the next section.
