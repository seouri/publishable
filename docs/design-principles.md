...
- **Code and parameters are hashed separately.** `code_hash` covers `src/**` and `templates/**`; `parameters_hash` covers the parameter block as declared, one hash per run. This is what makes "same code, different parameters" a provable claim rather than a hopeful one. See [Three hashes](reference.md#three-hashes).
