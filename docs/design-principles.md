...
- **`uv` is not optional.** Environments are captured and rebuilt through `uv`, so "reproduce this" means `uv sync --locked`. "Not optional" describes `reproduce`'s obligation, not `run`'s: a run whose repository holds no lockfile proceeds with `W-ENV-UNLOCKED`, and it is `reproduce` that refuses, because that is the command with nothing to `--locked` against. Promoting the warning would refuse every run of every scaffolded project today, which is a bootstrapping fact about this package's publication state rather than a principle.
