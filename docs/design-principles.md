...
- **Code and parameters are hashed separately.** `code_hash` covers `src/**`; `parameters_hash` covers the parameter block as declared, one hash per run. This is what makes "same code, different parameters" a provable claim rather than a hopeful one. See [Three hashes](reference.md#three-hashes).
...
- **Not crossed group axes.** `sweep.groups` is one axis, so a between-subjects factorial whose factors are both properties of the units — sex × arm, site × cohort — can't be expressed as a crossed design. This follows from the refusal above it rather than standing alone: core computes no main effects or interactions, so a second group axis would name a decomposition core declines to perform and render four arms as peers, which reads as a 2×2 that was never analyzed as one. Randomize one factor and balance on the other with `assign.stratify_by`, or name the cells as levels of a single axis. See [experimental-designs.md § What core will not do for you](experimental-designs.md#what-core-will-not-do-for-you).
