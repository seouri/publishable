...
They should be equal. **The build is bit-reproducible from the tag, across platforms**, and both halves of that were measured. On 2026-08-26 a rebuild in a detached worktree at `v0.1.0` reproduced both published digests on the same machine. On 2026-08-27 the stronger form held: 0.1.2 was built by `verify` on `ubuntu-latest` and published from there, and both digests match a local macOS `uv build` of the same tag exactly. That is what makes the artifacts attached at step 4 the same bytes the workflow publishes, and it means anyone can check a published artifact against the tag on any machine. It held a third time on 2026-08-27 for 0.1.3: both digests published by `verify` on `ubuntu-latest` equal a local macOS `uv build` of `v0.1.3`. And a fourth on 2026-08-29 for 0.2.5, where the local macOS digests were read out before `gh release create` and compared against pypi.org afterwards — `5370678…` for the sdist, `38759a0…` for the wheel, equal in both places. A fifth on 2026-09-18 for 0.2.6, the same way — `7ca3c41…` for the sdist, `7aade6f…` for the wheel, equal in both places. A sixth on 2026-09-20 for 0.2.7, where the local build was additionally repeated *after* the squash merge and reproduced the branch's own digests before the tag was cut — `dcb3798…` for the sdist, `9cb7320…` for the wheel, equal locally, on the branch, and on pypi.org.
