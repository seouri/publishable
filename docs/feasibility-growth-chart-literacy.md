...
shipped in `v0.2.0`; the corrected bound described below landed after it and is unreleased — see
...
working tree rather than a release — which is what makes the measurements below current, and is also the
caveat: **`081efc0` is 32 commits past the released `v0.2.0`**. The `compare: {to: constant, value: N}`
form itself shipped in 0.2.0, but the correctable `Member` that gives it a real corrected bound did not, so
an install of `publishable==0.2.0` evaluates such a hypothesis on a bound as `supported: null` under a
declared correction method. Read every claim here against the commit, not against the version number.
