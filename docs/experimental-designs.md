...
| **Two identical measurements reported as two arms** | A [`groups`](reference.md#allocation-within-subjects-or-between-subjects) axis narrows each condition to its arm through one shared roster, and a condition's arm is a subset view of that one roster, never a re-resolution — so declaring the axis without `allocation: between` is refused (`E-DATA-ALLOCATION-WITHIN-ARMS`) rather than quietly measuring every unit under every arm label and reporting one set of values as if it were two. The mirror is refused too: `allocation: between` with no axis naming what the arms are (`E-DATA-ALLOCATION-NO-ARMS`). And a contrast that crosses arms is refused outright rather than reported paired over an empty intersection (`E-DATA-ALLOCATION-CONTRAST`) — the two sides hold disjoint units by construction, so there is no shared unit left to difference. `E-DATA-ALLOCATION-WITHIN-ARMS`, `E-DATA-ALLOCATION-NO-ARMS` and `E-DATA-ALLOCATION-CONTRAST` all read the *within*-versus-arms question, and two more close the routes that answer it correctly and still repeat themselves: a level declared twice on one axis (`E-SWEEP-LEVEL-DUPLICATE`), where the two conditions carry the same label and the same units because the two arms are equal rather than overlapping; and a `sweep.baseline` fixing a level of the axis (`E-SWEEP-BASELINE-GROUP`, or `E-SWEEP-ABLATE-BASELINE-GROUP` where `ablate` is declared), where that level is rendered both as the baseline row and as its own axis's product row — the arms are peers, and designating one as the reference is a [`statistics.contrasts`](reference.md#contrasts-claims-that-arent-condition-vs-baseline) entry rather than a baseline |
