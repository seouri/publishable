...
Three choices sit *below* an answer rather than beside it. `assign.method` (`random` | `by_attribute` | `blocked`) only applies once allocation is `between`, and it answers a narrower question than the axis does — the `groups` axis says what the arms are, `assign.method` says how a unit reaches one. `cluster_by` sits below all three rather than beside them — it declares that units aren't independent, which widens the intervals. It is *not* independent of the other answers, though: because a cluster is indivisible, declaring it also constrains how `fold` splits and how `assign.method: random` assigns, so whole clusters stay on one side of every partition — `blocked` beside a declared `cluster_by` is refused instead, since a block fills to an exact unit count and a cluster can't be split to fit one. See [Clustered and hierarchical data](#clustered-and-hierarchical-data) and [Matched case-control](#matched-case-control). `data.units.holdout` sits below the third question without being an answer to it — a single train/test split divides the units once and repeats nothing, so it's a declaration about the table rather than a repeat kind. See [Train-test holdout](#train-test-holdout).
...
This design is shown with `method: random` because a trial drawing its own arms, rather than reading someone else's, is the case being illustrated; `method: by_attribute` is what a trial whose arm was already decided somewhere declares instead, and both execute. See [reference.md § Allocation](reference.md#allocation-within-subjects-or-between-subjects).

The realized allocation lands in `allocation.json`, hashed. The arm-to-arm comparison is unpaired, because the two conditions differ on the `groups` axis — but it does not exist until you declare it: `sweep.baseline` may never fix a level of a group axis (`E-SWEEP-BASELINE-GROUP`, since the arms are peers and none of them is the reference), so a `statistics.contrasts` entry naming both arms is the only route to it. If you compose a parameter axis on top, contrasts *within* an arm stay paired, because they're the same patients analyzed two ways. The two conditions share a `parameters_hash`, which is the point: same code, same parameters, different units. `statistics.null_test` with `shuffle: arm` tests that declared contrast directly: permuting the attribute that *defines* the arms is a test of the difference between them, not of either arm's own estimate, and core attaches the p-value to the contrast for that reason — see [reference.md § What isn't a repeat](reference.md#what-isnt-a-repeat).
...
Every cell is a condition, so cells get their own `values` and their own labels. A fold or a holdout is drawn *within* each cell: core partitions each cell separately, so every cell contributes the declared fraction — or the declared number of folds — of **itself**, and `k` and `frac` are bounded by the thinnest populated cell rather than by the whole roster — see [reference.md § A fixed holdout split](reference.md#a-fixed-holdout-split). `limits.min_units_per_cell` is checked per cell, which is where a 2×2 over a fixed roster starts to bite. **Neither axis can be the reference**: `sweep.baseline` may not fix a level of a `groups` axis, because the fixed level would be rendered both as the baseline row and as its own axis's product row, giving two conditions the same units. The arms are peers, and the comparison between two of them is a [`statistics.contrasts`](reference.md#contrasts-claims-that-arent-condition-vs-baseline) entry naming both cells — a delta this build refuses to compute over disjoint arms, so it lands in a `scope: "summary"` step's `Estimate` alongside the main effects below. See [reference.md § Expansion modes](reference.md#expansion-modes).
...
Which contrasts those are follows from how much `sweep.baseline` fixes, and the rule is the same one the between-subjects case uses: **the baseline expands over whichever axes it doesn't fix**, group axes and parameter axes alike — with only a parameter axis ever on the fixed side, since a baseline may not fix a group level at all. So when one axis is the parameter under test and the other is a stratum you're reporting across — two prompts across six model deployments, one normalization setting across three assay lots — fix the first and leave the second free, and you get one baseline per stratum with every contrast differing in exactly one place. Fixing a value on both axes instead gives a single reference condition and marks the contrasts that cross both [`confounded: true`](reference.md#allocation-within-subjects-or-between-subjects). See [reference.md § Expansion modes](reference.md#expansion-modes).
...
Core reports each arm with intervals clustered on `match_set`; the contrast between them needs a declared `statistics.contrasts` entry naming both, since `sweep.baseline` may never fix a level of a group axis (`E-SWEEP-BASELINE-GROUP` — the arms are peers, so none is the reference). That accounts for the matching in the intervals; it is not a conditional analysis, and if your field expects conditional logistic regression or a stratified estimator, that's a `scope: "summary"` step — see [what core will not do](#what-core-will-not-do-for-you).
...
| **Two identical measurements reported as two arms** | A [`groups`](reference.md#allocation-within-subjects-or-between-subjects) axis narrows each condition to its arm through one shared roster, and a condition's arm is a subset view of that one roster, never a re-resolution — so declaring the axis without `allocation: between` is refused (`E-DATA-ALLOCATION-WITHIN-ARMS`) rather than quietly measuring every unit under every arm label and reporting one set of values as if it were two. The mirror is refused too: `allocation: between` with no axis naming what the arms are (`E-DATA-ALLOCATION-NO-ARMS`). `E-DATA-ALLOCATION-WITHIN-ARMS` and `E-DATA-ALLOCATION-NO-ARMS` both read the *within*-versus-arms question, and two more close the routes that answer it correctly and still repeat themselves: a level declared twice on one axis (`E-SWEEP-LEVEL-DUPLICATE`), where the two conditions carry the same label and the same units because the two arms are equal rather than overlapping; and a `sweep.baseline` fixing a level of the axis (`E-SWEEP-BASELINE-GROUP`, or `E-SWEEP-ABLATE-BASELINE-GROUP` where `ablate` is declared), where that level is rendered both as the baseline row and as its own axis's product row — the arms are peers, and designating one as the reference is a [`statistics.contrasts`](reference.md#contrasts-claims-that-arent-condition-vs-baseline) entry rather than a baseline. A contrast that crosses arms is not this mistake at all: it is computed, unpaired, by `welch_t_over_units`/`unpaired_percentile_over_units` and their `_clustered` forms |
...
| **Resampling clustered rows as if independent** | A declared `cluster_by` makes the cluster the bootstrap draw, so 300 cells from 10 animals give a 10-draw interval rather than a 300-draw one that looks several times more precise than the data supports — √(1 + (m−1)ρ) times, which for 30 cells an animal at ρ = 0.6 is about four |
| **A permutation that shuffles away the matching** | `null_test` shuffles at the level the shuffled attribute lives at — within clusters when it varies inside one, whole clusters when it doesn't — so a matched design gets a conditional test instead of a free relabelling. A shuffle whose attribute varies within some clusters and not others has no level either reading fits, and `validate` refuses it (`E-STATS-NULLTEST-LEVEL`) rather than guessing one |
...
| **The apparatus changed underneath you** | Where a template declares an apparatus probe, it is called per condition at `dry-run`, at run start, before every execution, and at `freeze`, and its facts are recorded from run start onward — `dry-run` probes but records nothing, having no run directory to record into; a revision, firmware, or calibration observed to differ from its own first *answered* observation — never against the most recent observation, and never against another condition's — fails the run rather than pooling two states as one dataset. A fact the apparatus didn't answer is recorded as `null` and counted rather than read as a change, so an unevenly-reported pin stays declarable and warns, wherever a probe runs, instead of the run failing. See [reference.md § The apparatus core can only observe](reference.md#the-apparatus-core-can-only-observe) |
| **A typo'd parameter silently using a default** | The schema is closed and `validate` checks every key against it — `init` materializes each parameter the template declares with its default beside it — so an unrecognized key is a typo by construction and fails validation instead of falling back to a default |
