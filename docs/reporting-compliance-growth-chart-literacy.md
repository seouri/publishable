...
| Version pinning of a hosted model | TRIPOD-LLM; `reporting-standards.md` § 7 | `apparatus_facts = ["model_version", "system_fingerprint"]`, probed at `dry-run`, at run start, before every execution and at `freeze`, with a moved fact failing the run. **The two facts are answered in different places and the split is worth knowing**: the model version is read by the probe and sits under that gate, and the fingerprint is returned only on a completion, so it is logged per call in the request transcript instead — which is where the plan asks for it, and which is what TRIPOD-LLM item 18's drift concern actually needs |
...
| Indeterminate results | STARD | An unparseable response is a declared policy, `scoring.parse_failure`, hashed with everything else — **and it is now reached only after the retry the plan requires**, with the combined unparseable-plus-refusal rate and its 5% threshold recorded per repeat, which is what turns STARD's indeterminate-results item from a sentence into a number a reader can check. Its one deviation across the fourteen — [E5b](feasibility-growth-chart-literacy.md#e5b--the-graded-negative-control) routing it to `negative` rather than to `ineligible` — is visible in `diff` rather than buried in a footnote |
...
**The apparatus gate answers a question STARD-AI cannot ask, and as of 2026-09-10 it has an observation to compare against.** A hosted deployment re-tuned midway through a cross-model arm is normally invisible, and would be reported as between-model heterogeneity. Here it fails the run. That is the difference between reporting a version string and being able to state that the version did not move — and the fact it compares is the *resolved* one: the roster's `gpt-5.6-sol` resolves to `gpt-5.6-sol-2026-07-09`, so a deployment repointed at a newer snapshot changes it while the name in the config does not. **This claim was overstated twice before it was true** — once while the probe returned an environment variable, and once while it asked an endpoint that resource does not serve — which is worth knowing about a property this document rates highly.
...
**Assessed on 2026-08-30** against `docs/feasibility-growth-chart-literacy.md` as re-derived that day, which read the plan at `growth-chart-literacy@e6b43ab` and now reads it at `7dafaeb`, and against `../standards/` as last verified in August 2026. **Its build claims were re-measured three times on 2026-08-31, again on 2026-09-04, and again on 2026-09-10** and none of this assessment moved: the same fourteen configs, the same warnings, the same two executed arms. What did move is two Tier A rows above — the prompt is now two messages carrying the child's sex and the reference frame with the envelope recorded, and the apparatus probe now reads a real metadata endpoint where it had returned an environment variable with a default. Three rows in all: the prompt, the apparatus probe, and the indeterminate-results row above, whose retry and rate the contract required and the code did not perform. The fourth round moved no row here at all — it implemented E5a's monotone interpolant, which is a dataset-provenance fact rather than a reporting one, and it is covered by risk 1 below. The fifth round is the first in which the **plan** moved rather than the tooling — an artifact profile of the augmented layer, a restated generator calibration, and reworded negative strata — and one of its findings reaches this assessment directly: the pooled 53.8% height-availability figure is a **mixture over encounter types**, from 98.2% at well visits to 8.6% at walk-ins, and weight is recorded at 98.6% of *telephone* encounters where a physical measurement is impossible. That does not change a verdict here either, but it sharpens risk 1 below: a reader granting clinical realism to these stimuli should know the availability rate they are matched to is a property of how a record looks rather than of what was measured. All of these strengthen a disclosure rather than changing a verdict, and one is worth re-reading: this document called the apparatus gate a property the standards cannot ask for, at a time when it was comparing a constant against itself. Every claim here about what `publishable` *does* is inherited from that document's [§ Executability on this build](feasibility-growth-chart-literacy.md#executability-on-this-build) and is no fresher than the commit pinned there; every claim about what it *specifies* is from `reference.md` and `design-principles.md`. Guideline versions move — `reporting-standards.md` § 8 lists QUADAS-AI, CLAIRE and BRIDGE-AI in development, any of which could change the triage above — so re-verify against the EQUATOR library before submission rather than against this file.
