...
## Correction 23, appended 2026-08-24 by the whole-branch fix round — task 16's "thirteen `E-RESUME-*` codes" is fourteen

**Appended, never edited.** § Task 16's pointer and its § Errors instruction both say *"the thirteen
`E-RESUME-*` codes and `E-FREEZE-CONFIG-EDITED`"*. The code raises **fourteen** `E-RESUME-*` codes —
`grep -rho 'E-RESUME-[A-Z-]*' src/publishable/*.py | sort -u | wc -l` → 14 — eleven as a literal
`code="E-RESUME-…"` and three (`-CODE-MOVED`, `-PARAMS-MOVED`, `-LOCKFILE-MOVED`) through a loop
variable, which is the under-count a keyword-form grep produces. **Fifteen codes minted in total,
fifteen § Errors rows, one per code.** The figure came from design Decision 17's prose, which its own
table already contradicted, and it reached the feasibility analysis' dated § Executability entry; both
of those carry their own appended corrections. Task 16 wrote fifteen rows regardless, so **nothing
built from this plan is wrong** — the count in the instruction is.
