...
- `runner` or `lineage` gains the reader that counts a triple's records off `executions.jsonl`;
  **grep for an existing `executions.jsonl` reader before writing one** and report the grep. If one
  exists, use it.
...
**Must not touch:** `stats.py`, `runner.attrition`, `run_record`'s blocks — you produce the input those
read, and changing them would be answering the same question twice. No `*.md`. No arm.
