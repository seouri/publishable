...
| [`docs/reporting-compliance-growth-chart-literacy.md`](docs/reporting-compliance-growth-chart-literacy.md) | **Non-normative.** Whether the study [`growth-chart-literacy`](docs/feasibility-growth-chart-literacy.md) specifies could comply with the reporting guidelines in `standards/`: the triage worked to one primary and two adjuncts, then every requirement family sorted into what the run record already answers, what needs a config or template edit, and what is prose no configuration can hold. It quotes **no item number** — the item lists are not in `standards/`, and a fabricated one is worse than an absent one — so what it checks is the requirement families those files enumerate. Dated, and every build claim in it is **inherited** from the analysis it reads rather than re-measured, so it is no fresher than that file's own pinned commit |
...
- **Every declarable field has a reader.** A field either changes what executes and something reads it, or it is *carried* — `metadata`, `replication.rationale`, a bundle's `title` and `authors`, kept by `run.yaml` embedding the config verbatim — and a field that does neither is decoration in a file a reader trusts to describe the run. Carriage admits nothing on its own: `rationale` justifies a declared number three lines above it and can be caught disagreeing with it, while a string justifying nothing adjacent belongs in the manuscript. **Core cannot check this and never will** — the *Greenfield only* bullet below means it never reads a step body, so for a plugin's `parameter_spec` there is nothing to inspect — which is why the check is manual and specific: grep the new parameter's call sites for a literal sitting where the value should be. (`design-principles.md` § Every declarable field has a reader)
...
| A parameter **added, documented, and wired to a constant** | Four slices in a row shipped one. **The docstring is not the wiring**, and *an unread parameter is an unbuilt reader of a shipped surface* — the rule is `design-principles.md` § Every declarable field has a reader, and this row is how one is caught. Grep every new parameter's call sites for a literal |
...
| A field proposed because a standard or a source project asks for it, with nothing reading it | `design-principles.md` § Every declarable field has a reader. An analysis proposes a plugin, and a `Param` that reaches no step still validates, still expands the sweep and still enters `parameters_hash` — every condition renders the same thing and the manipulation's measured effect is zero. `feasibility-growth-chart-literacy.md` found four in one template, by running rather than by reading |
